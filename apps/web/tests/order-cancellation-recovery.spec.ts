...
for (const width of [1440, 390])
  for (const previouslyRefunded of [0, 100, 500])
    ...
    INSERT INTO payment_intent(id,purpose,subject_type,subject_id,customer_id,amount_minor,currency,status,idempotency_key,version,created_at,updated_at)
      VALUES ('${id}','GROCERY_CHECKOUT','checkout_quote','${id}','${id}',500,'PHP','${previouslyRefunded === 0 ? "SUCCEEDED" : previouslyRefunded === 500 ? "REFUNDED" : "PARTIALLY_REFUNDED"}','${id}',1,${now},${now});
  ...
  `);
      if (previouslyRefunded > 0)
        executeAdminE2eSql(
          `INSERT INTO payment_refund(id,payment_intent_id,amount_minor,currency,status,reason,idempotency_key,version,created_at,updated_at) VALUES ('prior-${id}','${id}',${previouslyRefunded},'PHP','SUCCEEDED','Prior adjustment','prior-${id}',1,${now},${now});`,
        );
      ...
      expect(responses[0]).toMatchObject({
        ...
        value: {
          ...
          cancellation: {
            status: previouslyRefunded === 500 ? "COMPLETED" : "REQUESTED",
            requiredRefundMinor: 500 - previouslyRefunded,
          },
        },
      });
      ...
      await expect(
        page
          .getByText(previouslyRefunded === 500 ? "Canceled" : "Cancellation Requested", {
            exact: true,
          })
          .first(),
      ).toBeVisible();
