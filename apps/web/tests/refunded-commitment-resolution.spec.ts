...
// Full refund and failed commitment state is an explicit fixture; resolution/replay use actual Web/Core/D1.
for (const width of [1440, 390])
  test(`Confirm a refunded commitment at ${width}px`, async ({ adminPage: page }, testInfo) => {
    test.setTimeout(60000);
    const id = crypto.randomUUID(),
      now = Date.now();
    const paymentId = `refunded-payment-${id}`,
      caseId = `refunded-case-${id}`;
    executeAdminE2eSql(`
 ...
 `);
    ...
    await expect(target.getByRole("button", { name: "Review refunded outcome" })).toBeEnabled();
    const unlinked = page
      .getByRole("listitem")
    ...
    await target.getByRole("button", { name: "Review refunded outcome" }).click();
    await expect(page.getByRole("heading", { name: "Confirm refunded commitment?" })).toBeVisible();
    await page
      .getByLabel("Confirmation reason")
      .fill("Confirmed the full refund without a committed Order");
    ...
    expect(JSON.parse(writes[0].body ?? "{}")).toMatchObject({
      expectedVersion: 1,
      reason: "Confirmed the full refund without a committed Order",
    });
    ...
    await expect
      ...
      .toBe(true);
    await page.screenshot({ path: testInfo.outputPath("refunded-commitment.png"), fullPage: true });
  });
