...
for (const width of [1280, 390]) {
  test(`Warehouse stock to two destinations with partial acceptance at ${width}px`, async ({
    ...
    for (const target of locations.slice(1)) {
      ...
      await expect(
        page.getByRole("button", { name: "Accept checked goods", exact: true }),
      ).toBeVisible();
      for (const quantity of [5000, 15000]) {
        ...
        await page.getByRole("button", { name: "Accept checked goods", exact: true }).click();
        await expect(
    ...
    await page.screenshot({
      path: testInfo.outputPath(`warehouse-received-${width}.png`),
      fullPage: true,
    });
  });
}
