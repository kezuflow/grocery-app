...
- `pnpm run dev` starts vinext and the Core auxiliary Worker in one Vite runtime at `http://localhost:3000`.
...
`/orders` and `/orders/[order-id]` render Core-owned summaries, immutable detail/timeline, current-state reorder results, typed issue intake, exact cancellation/refund previews, and eligible additive-amendment payment actions. `/orders/[order-id]/transaction-summary` prints the Core-owned provisional record with `NOT AN OFFICIAL BIR INVOICE`; Web does not infer provider success, refund completion, official invoice issuance, or legal actions.
