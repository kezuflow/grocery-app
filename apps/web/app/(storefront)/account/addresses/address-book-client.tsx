...
export function AddressBookClient({
  ...
  const loadAddresses = useCallback(async (confirmedAddressId?: string) => {
    ...
    try {
      if (confirmedAddressId) await checkoutBootstrap.invalidate();
      const bootstrap = confirmedAddressId
        ? await accountAddresses.refresh()
        : await accountAddresses.read();
      if (generation !== addressLoadGeneration.current) return;
      setAddresses(bootstrap.addresses);
      setProfile(bootstrap.profile);
      setSelectedAddressId((current) => {
        const nextId = confirmedAddressId ?? current ?? bootstrap.profile.defaultAddressId;
        return bootstrap.addresses.some(
          (address) => address.id === nextId && address.serviceable === true,
        )
          ? (nextId ?? undefined)
          : undefined;
      });
