import { adminSkuAvailabilityBodySchema, idempotencyKeySchema } from "@freshmarkets/validation";
...
async function PUTHandler(request: Request, context: { params: Promise<{ "sku-id": string }> }) {
  ...
  const body = await readBoundedJson(request, adminSkuAvailabilityBodySchema, { maxBytes: 8192 });
  ...
  return adminJson(
    await coreClient(env.CORE).setAdminSkuAvailability({
      ...body.value,
      skuId,
      requestId: webRequestId(request),
