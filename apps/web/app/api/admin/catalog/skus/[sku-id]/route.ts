import { adminSkuUpdateBodySchema, idempotencyKeySchema } from "@freshmarkets/validation";
...
async function PATCHHandler(request: Request, context: { params: Promise<{ "sku-id": string }> }) {
  ...
  const body = await readBoundedJson(request, adminSkuUpdateBodySchema, { maxBytes: 8192 });
  ...
  return adminJson(
    await coreClient(env.CORE).updateAdminSku({
      ...body.value,
      skuId,
      requestId: webRequestId(request),
