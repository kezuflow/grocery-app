...
import { GET as getMap } from "./route";
import { GET as getDetail } from "./detail/route";
import { POST as previewRoute } from "./route-preview/route";
import { GET as getRiders, POST as createBatch } from "../delivery-batches/route";
...
describe("GET /api/admin/delivery-map", () => {
  ...
  it("forwards one generated request identity, only safe headers, Instant context, and optional filters", async () => {
    ...
    expect(call).toEqual({
      ...
      headers: { cookie: "session=private", origin: "https://freshmarkets.ph" },
      locationId: "location-1",
      fulfillmentMode: "INSTANT",
      cycleId: null,
