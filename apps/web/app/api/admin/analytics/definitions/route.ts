...
async function GETHandler(request: Request) {
  ...
  if (category && !(analyticsMetricCategories as readonly string[]).includes(category)) {
    return invalid(request, "category is not supported");
  }
  if (status && !(metricDefinitionStatuses as readonly string[]).includes(status)) {
    return invalid(request, "status is not supported");
  }
  return adminJson(
...
export const GET = observeAdminRoute("admin.analytics.definitions.get", GETHandler);
