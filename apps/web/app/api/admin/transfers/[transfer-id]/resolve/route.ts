...
import { z, resolveInventoryTransferSchema } from "@freshmarkets/validation";
...
export const POST = observeAdminRoute(
  "admin.transfers.by_transfer_id.resolve.post",
  async (request: Request, context: { params: Promise<{ "transfer-id": string }> }) => {
    ...
    const parsed = resolveInventoryTransferSchema.safeParse({
      ...body.value,
      transferId,
    ...
    return adminJson(
      await coreClient(env.CORE).resolveInventoryTransfer({
        ...parsed.data,
