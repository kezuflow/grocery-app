...
import { z, inventoryTransferCommandSchema } from "@freshmarkets/validation";
...
export const POST = observeAdminRoute(
  "admin.transfers.by_transfer_id.dispatch.post",
  async (request: Request, context: { params: Promise<{ "transfer-id": string }> }) => {
    ...
    const parsed = inventoryTransferCommandSchema.safeParse({
      ...body.value,
      transferId,
    ...
    return adminJson(
      await coreClient(env.CORE).dispatchInventoryTransfer({
        ...parsed.data,
