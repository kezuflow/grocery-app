...
const core = vi.hoisted(() => ({ getPublishedPromotionMedia: vi.fn() }));
vi.mock("cloudflare:workers", () => ({ env: { CORE: core } }));
import { GET } from "@/app/media/promotions/[media-id]/[version]/route";
const context = { params: Promise.resolve({ "media-id": "image-1", version: "1" }) };
beforeEach(() => core.getPublishedPromotionMedia.mockReset());
it("checks Core on cache misses before honoring an ETag", async () => {
  core.getPublishedPromotionMedia.mockResolvedValue({
    ok: true,
    requestId: "r",
    value: { bytes: new Uint8Array([1, 2]).buffer, mimeType: "image/jpeg", etag: '"image-etag"' },
  });
  const request = new Request("https://freshmarkets.test/media/promotions/image-1/1", {
  ...
  core.getPublishedPromotionMedia.mockResolvedValue({ ok: false, error: { code: "NOT_FOUND" } });
  const removed = await GET(request, context);
  expect(removed.status).toBe(404);
  expect(removed.headers.get("cache-control")).toBe("no-store");
  expect(core.getPublishedPromotionMedia).toHaveBeenCalledTimes(2);
});
it("rejects malformed versions before calling Core", async () => {
  ...
  expect(core.getPublishedPromotionMedia).not.toHaveBeenCalled();
});
