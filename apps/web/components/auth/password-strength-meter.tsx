...
const segmentColors: Record<FilledLevel, string> = {
  weak: "bg-oklch(0.577 0.245 27.325) dark:bg-oklch(0.704 0.191 22.216)",
  fair: "bg-amber-500",
  good: "bg-sky-500",
...
export function PasswordStrengthMeter({ password, className }: PasswordStrengthMeterProps) {
  ...
  return (
    <div className={cn("flex flex-col gap-1.5", className)}>
      ...
      <div aria-hidden="true" className="flex gap-1">
        {STRENGTH_SEGMENTS.map((segment) => (
          <span
            key={segment}
            className={cn(
              "h-1 flex-1 rounded-full bg-oklch(0.97 0 0) transition-colors dark:bg-oklch(0.269 0 0)",
      ...
      <p aria-live="polite" className="text-oklch(0.556 0 0) text-xs dark:text-oklch(0.708 0 0)">
