...
export function readMediaUpload(db: D1Database, scope: string, key: string) {
  return db
    .prepare(
      "SELECT id,promotion_id,object_key,content_digest,status,lease_token,lease_expires_at,version FROM promotion_media_upload WHERE command_scope=? AND idempotency_key=?",
    )
    .bind(scope, key)
    .first<MediaUpload>();
}
...
export async function storeMediaUpload(
  ...
  if (upload.status === "STORED" || upload.status === "UNKNOWN" || upload.lease_token) {
    ...
    if (stored) {
      ...
      const claimed = await db
        .prepare(
          "UPDATE promotion_media_upload SET status='STORED',lease_token=NULL,lease_expires_at=NULL,version=version+1,updated_at=? WHERE id=? AND version=? AND status IN ('PENDING','UNKNOWN','STORED')",
        )
  ...
  const claimed = await db
    .prepare(
      "UPDATE promotion_media_upload SET status='PENDING',lease_token=?,lease_expires_at=?,version=version+1,updated_at=? WHERE id=? AND version=? AND status IN ('PENDING','UNKNOWN','STORED') AND (lease_token IS NULL OR lease_expires_at<=?)",
    )
  ...
  try {
    const result = await bucket.put(upload.object_key, bytes, {
      ...
      customMetadata: {
        promotionId: upload.promotion_id,
        mediaId: upload.id,
  ...
  const result = await db
    .prepare(
      "UPDATE promotion_media_upload SET status=?,lease_token=NULL,lease_expires_at=NULL,version=version+1,updated_at=? WHERE id=? AND lease_token=? AND status='PENDING'",
    )
...
export function mediaCleanupIntent(
  ...
  return db
    .prepare(
      "INSERT INTO promotion_media_cleanup(id,promotion_id,object_key,status,available_at,created_at,updated_at) VALUES (?,?,?,'PENDING',?,?,?) ON CONFLICT(object_key) DO UPDATE SET status='PENDING',available_at=excluded.available_at,version=version+1,updated_at=excluded.updated_at",
    )
    .bind(crypto.randomUUID(), promotionId, objectKey, now, now, now);
}
...
export async function cleanPromotionMedia(
  db: D1Database,
  bucket: R2Bucket,
  now = Date.now(),
  limit = 20,
  ...
  const uncertain = await db
    .prepare(`SELECT id,object_key,content_digest,version FROM promotion_media_upload
  ...
  for (const upload of uncertain.results) {
    ...
    await db
      .prepare(`UPDATE promotion_media_upload SET status=?,lease_token=NULL,lease_expires_at=NULL,version=version+1,updated_at=?
  ...
  const abandoned = await db
    .prepare(`SELECT id,promotion_id,object_key,version FROM promotion_media_upload
    WHERE status='STORED' AND lease_token IS NULL AND updated_at<? ORDER BY updated_at,id LIMIT 20`)
    .bind(now - 86400000)
    .all<{ id: string; promotion_id: string; object_key: string; version: number }>();
  for (const upload of abandoned.results) {
    try {
      await db.batch([
        db
          .prepare(
            "UPDATE promotion_media_upload SET status='ABANDONED',version=version+1,updated_at=? WHERE id=? AND version=? AND status='STORED' AND lease_token IS NULL",
          )
          .bind(now, upload.id, upload.version),
        requireMediaEffect(db),
        mediaCleanupIntent(db, upload.promotion_id, upload.object_key, now),
        requireMediaEffect(db),
      ]);
    } catch {
  ...
  const rows = await db
    .prepare(`SELECT id,object_key,version,attempt_count FROM promotion_media_cleanup
  ...
  for (const row of rows.results) {
    if (row.attempt_count >= 5) {
      await db
        .prepare(
          "UPDATE promotion_media_cleanup SET status='FAILED',last_error_code='CLEANUP_ATTEMPTS_EXHAUSTED',lease_token=NULL,lease_expires_at=NULL,version=version+1,updated_at=? WHERE id=? AND version=?",
        )
    ...
    const lease = crypto.randomUUID();
    const claimed = await db
      .prepare(`UPDATE promotion_media_cleanup SET status='PROCESSING',lease_token=?,lease_expires_at=?,attempt_count=attempt_count+1,version=version+1,updated_at=?
      WHERE id=? AND version=? AND NOT EXISTS (SELECT 1 FROM promotion_media WHERE object_key=promotion_media_cleanup.object_key AND status='active')
        AND NOT EXISTS (SELECT 1 FROM promotion_media_upload WHERE object_key=promotion_media_cleanup.object_key AND status<>'ABANDONED' AND status<>'ATTACHED')`)
    ...
    const updated = await db
      .prepare(
        "UPDATE promotion_media_cleanup SET status=?,available_at=?,last_error_code=?,lease_token=NULL,lease_expires_at=NULL,version=version+1,updated_at=? WHERE id=? AND lease_token=? AND status='PROCESSING'",
      )
      .bind(
