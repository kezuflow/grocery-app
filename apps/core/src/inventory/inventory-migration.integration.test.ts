...
async function insertEvent({
  ...
  await env.DB.batch([
    ...
    env.DB.prepare(
      "INSERT INTO receiving_event (id,receiving_record_id, procurement_requirement_id, location_id, inventory_pool_id, accepted_delta, rejected_delta, reason, idempotency_key, occurred_at) VALUES (?, ?, ?, 'location-cebu-central', 'pool-red-onion', ?, ?, ?, ?, ?)",
    ).bind(
      crypto.randomUUID(),
      receivingRecordId,
      requirementId,
