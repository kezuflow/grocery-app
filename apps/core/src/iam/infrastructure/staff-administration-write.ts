export function requireStaffWrite(database: D1Database): D1PreparedStatement {
...
export function beginStaffAdministrationWrite(
  database: D1Database,
  input: {
    staffId: string;
    authUserId: string;
  ...
  return [
    database
      .prepare(`INSERT INTO commitment_abort(id) SELECT -35 WHERE NOT EXISTS (
      ...
      WHERE s.id=? AND s.auth_user_id=? AND s.status='active' AND p.code='staff.manage')`)
...
export function completeStaffAdministrationWrite(
  database: D1Database,
