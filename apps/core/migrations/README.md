...
`0097_fixed_delivery_package.sql` preserves nullable SKU/line shipping evidence while allowing exact
...
## 0078 initial administrator evidence

`0078_initial_administrator.sql` adds an empty singleton setup receipt with immutable evidence and restricted references. It seeds no user, password, role or grant and preserves existing data. Core's one-use configured-email setup command owns enrollment eligibility. The migration verifier compares every pre-0078 retained row and grant across the upgrade. Follow [initial administrator setup](../../../docs/operations/INITIAL_ADMINISTRATOR_SETUP.md) after applying the reviewed migration through the environment's normal upgrade process.

## 0100 administrator ownership-transfer evidence

`0100_administrator_ownership_transfer.sql` adds the immutable singleton receipt used by the explicitly authorized production correction. It changes no identity or grant by itself. The reviewed one-time Core command created the verified target Staff identity, reused the exact active initial-administrator role, moved Global scope, suspended and stripped the synthetic source identity, removed its Better Auth sessions/accounts, tombstoned its email, and committed audit/idempotency evidence atomically. The temporary bearer-gated transport and deployment secret were removed after the receipt was confirmed; this migration does not establish a general recovery surface.

## 0085 - Canonical promotion delivery benefits

Generated by `node scripts/generate-promotion-benefits-migration.mjs` from an in-memory 0084 baseline; `pnpm migration:check` verifies reproducibility. The forward migration expands benefit enums and converts legacy delivery-percentage definition labels with a version increment. It retains every row/rowid in the six affected tables, all dependent rules/applications, and the original financial evidence type/JSON. It recreates existing indexes and verifies the full FK graph in tests. No existing local/shared/remote database has been reset by implementation. Apply through the migration runner after retained-deployment backup/validation; the clean browser database is separately disposable.
