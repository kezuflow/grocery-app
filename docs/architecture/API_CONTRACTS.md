...
- Current navigation correction: Core owns the ordered list and scope metadata. Web preserves the Core order and renders every Core-provided entry with a section icon fallback; it does not maintain a second route allowlist. Procurement requires `procurement.read` and appears in Global and Location. Receiving requires `procurement.manage` and appears only in Location. They are independent Operations destinations so a receiving-only role is not sent to an unreadable Procurement page. Warehouse transfers requires `transfers.read` and appears in Global and Location. The Global Products submenu includes Catalog overview for its controlled-unit reference. Direct routes and RPCs reauthorize independently. This current rule supersedes the older generic navigation-code examples above, including retired `commerce-configuration` navigation.
