...
Initial home and search reads use the composite storefront methods so category/banner support data does not require separate Web-to-Core invocations. A successful browsing confirmation sets an opaque Core-signed context carrying only read-scoping evidence. Catalog RPC validates it and derives the exact location; invalid or expired context falls back to location-unaware browsing. The token cannot authorize Cart mutation, checkout, price acceptance, stock use or payment. A one-release coordinate-cookie bridge may resolve older pre-context selections; newly confirmed selections do not repeat polygon resolution on each catalog navigation.
...
The anonymous browsing confirmation uses `GeocoderPort.reversePermanent` through Google Geocoding, requires the selected entrance to be inside an active Global service area, and re-resolves the nearest current fulfillment-location pin. Its minimal public result contains the finalized display address, confirmed coordinate and location-assignment result; no provider reference, signing material or raw payload. Core additionally issues an opaque signed read context which Web stores in an HttpOnly same-site cookie and removes from the JSON response. Web calls the private-body, no-store `/api/commerce/browsing-location` endpoint only on Deliver here, and remembers only a successful, still-current assignment. Provider errors, an outside-area coordinate, lost location eligibility, editor closure and a moved selection cannot persist temporary or stale browsing data. Search and editor pin autofill remain temporary. Browser storage uses new confirmed-location keys; old unconfirmed keys are discarded rather than migrated. This does not create a saved checkout address, claim courier coverage, or waive checkout revalidation. Actual Google Maps Platform account, billing, API enablement, key restrictions, storage terms, and attribution compliance remain activation requirements.
...
`GEOCODER`, `USER_PIN`, or `DEVICE_LOCATION`. `DeliveryInstructions` contains only nullable
`deliveryInstructions`, bounded to 1,000 characters, the strictest enabled-adapter instruction limit
(Lalamove permits 1,500). Core compatibility
reads join distinct non-empty legacy building/unit, landmark, gate/guard, delivery-note and
recipient-instruction values in their original field order. Private `notes` never enter this value. The
...
authoritative nearest-location assignment, while legacy service-area/delivery-zone labels,
resolution version, status, and other compatibility fields are server-derived. The address view
...
Global location setup uses `listAdminLocations`, `createAdminLocation`, `updateAdminLocation` and `transitionAdminLocation`. Reads require `locations.read`; commands require `locations.manage`, Global scope and a reason. Creation generates the location ID and starts inactive. Purpose, market and code are fixed after creation; detail changes and activation/deactivation require the current version. Core atomically guards current staff/grants, market and location version with capabilities, audit and exact replay result. List responses include active/inactive sites, named markets, currency/timezone, validated structured address or an explicit missing-confirmation state, coordinates and capabilities, with bounded keyset pagination. The optional validated `locationId` query narrows this same authorized read to the location detail, independent of where the record appears in the list; a missing ID returns an empty list. New capabilities are explicit role grants, never automatically assigned.
...
Cart creation now follows an explicit coordinate selection. `cart.get()` returns `DELIVERY_LOCATION_REQUIRED` when no active Cart exists; it never chooses a default site. `cart.selectCartLocation({ latitude, longitude, expectedVersion, idempotencyKey })` resolves the nearest geographically eligible site in Core, creates a version-one Cart when expectedVersion is zero or changes its location under the current version, and returns the immutable `{ cartId, version, locationId }` receipt plus the current `CartView` read projection in the same Core invocation. The browser reads Cart first, avoids serviceability for an unauthenticated/missing Cart unless creation is actually needed, and does not issue a trailing Cart GET after selection. Current geography, customer and Cart state guard the complete transaction. A location change atomically supersedes and releases unstarted Quote/hold evidence; any started Payment blocks the mutation. Existing lines remain for current-price/unavailable-state review; paid Order and started-payment snapshots are unchanged. This browsing choice grants no checkout eligibility or inventory reservation. A Quote records its Cart version internally. Successful payment commitment completes that Cart only while its version still matches; newer edits and retained Quotes with unknown Cart versions are preserved. Completed Cart rows remain historical. The next Cart or Buy again uses current location selection, while payment replay returns the existing Order and leaves later Carts intact.
...
Checkout quotes and new payment admission use one fixed 20,000 g Motorcycle service envelope for every nonempty courier Order. Per-line shipping grams remain nullable logistics snapshots and are not summed for admission. Missing or larger metadata never disables an option or blocks quotation, payment, paid commitment, exact Scheduled demand, procurement, an addition, or booking. Exact replay and existing financial recovery remain available. Operations keeps the actual packed parcel within provider limits; no customer packaging allowance or fit inputs are required.
...
The authoritative service validates selling `OPEN`; authenticated Customer; cart, exact location/SKU prices, nonempty cart, confirmed address coordinates inside an active Global service area, nearest active/capable/mode-ready fulfillment location; Instant inventory hold or Scheduled cycle/window/cutoff; Promotions eligibility/limits/stacking; provider capabilities and a successful current Lalamove quotation; required store/customer contact and pickup/drop-off coordinates; the fixed courier parcel; and payment readiness. Scheduled performs no stock or capacity query. The Global polygon gates FreshMarkets expansion but does not establish courier route availability; no delivery-zone/location link participates. Missing, invalid-currency, expired, or stale required evidence fails closed. The approved item-sale/full-price-code/delivery stack in PRODUCT GD-D06–07 requires item-level allocation and concurrency-safe allowance accounting before exposure. Preserve exact component totals, usage claims and committed history; apply the approved overlap, depletion and eligible-cancellation restoration rules in PRODUCT GD-D07.
...
Immediately before a new payment creation, Core first resolves an exact idempotency replay, then recalculates selling state, exact store/SKU prices, discounts, Instant stock/hold or Scheduled window/cutoff, serviceability and fulfillment eligibility without persisting or superseding another Quote. An accepted external-provider quotation with more than 30 seconds remaining is reused because it is already bound to the same guarded Cart/address/routing snapshot; near-expiry or expired evidence is refreshed once. The accepted Quote version, price-acceptance version, currency, `expectedTotalMinor`, every explicit component, and current provider quotation evidence must agree; otherwise Core returns `PRICE_CHANGED` without creating a payment and the browser must request/present a replacement Quote for explicit acceptance. Identical replay retains the original accepted version and subject; a replay with changed accepted components is an idempotency conflict.
...
- Phase 12 extends each permitted navigation entry with a closed section code, selected-scope applicability, and optional parent code, and adds stable subview codes for the approved Product, Category, Order, Customer, Procurement/Receiving, Payment, Staff/Role, and Settings destinations. Core emits every visible parent and child; Web may filter, group, and collapse those entries but never manufactures an unauthorized route. Global Customer, Catalog definition, Promotion, Payment/Pricing, and Staff administration is hidden for Market/Location selections. Orders, Analytics, and Audit remain available for their supported scoped reads. Location Product operations, physical Inventory, Fulfillment, and External Delivery are Location-only; selling state and Fulfillment Mode are Global-only.
- Current navigation correction: Core owns the ordered list, Shopify-style presentation section metadata and scope applicability. Web preserves Core's first section occurrence and every authorized entry; it does not maintain a second route allowlist. The sections are Home, Orders, Products, Customers, Discounts, Content, Finance, Analytics and Settings, with Settings pinned in the desktop sidebar. Sales channels/Online Store and disabled Point of Sale/Messaging labels are presentation-only and grant no Admin route or command. Delivery weeks (`/admin/procurement`) requires `procurement.read` and appears in Global and Location under Orders. Receiving requires `procurement.manage` and appears only in Location under Products. These remain independent entries, so a receiving-only role is not sent to an unreadable Delivery weeks page. Warehouse transfers requires `transfers.read` and appears in Global and Location under Products. The Global Products submenu includes Catalog overview for its controlled-unit reference. Direct routes and RPCs reauthorize independently. The previous section codes remain compatible with serialized context fixtures; Core emits the new presentation sections. This current rule supersedes the older generic navigation-code examples above, including retired `commerce-configuration` navigation.
...
- `getMyCustomerProfile` resolves the authenticated Customer and returns customerId, nullable accountPhone/defaultAddressId/preferredLanguage, promotionalEmails and current version. `updateMyCustomerProfile` accepts the existing preferences and optional accountPhone, expectedVersion and a stable idempotency key. Omitted phone preserves stored contact for retained preference requests; provided phone is normalized as a Philippine mobile or explicitly cleared. Phone defaults only a new address editor and never rewrites saved delivery contacts or paid snapshots. Current active Customer/principal/version guard the complete update, audit and frozen receipt. Preferred language is a bounded support preference, not a translation promise; promotional opt-in never mutes transactional updates. Name changes use Better Auth updateUser and its session authority, without copying name/email into customer storage. Global preference commands retain their earlier approved editable fields and preserve the new read-only fields.
- `manageMyCustomerAddress({ action: SET_DEFAULT|REMOVE, addressId, expectedAddressVersion, expectedVersion, idempotencyKey })` derives the Customer from the current session. Only an active owned address may become default or be removed. The complete transaction guards current principal/customer/address state, advances Customer version, changes the pointer, deactivates the address for REMOVE, audits, and retains an immutable result. Removal clears only a matching default and retains the address row and every Order snapshot. Default choice does not establish courier availability: checkout still resolves current location readiness and obtains a Lalamove quotation before selecting it. Exact replay returns its historical result without overwriting later choices; disabled principals cannot use this command, including replay. Web retains the exact unresolved command for retry.
- `createCustomerAddress` and `updateCustomerAddress` require a stable `idempotencyKey`; Web POST/PATCH `/api/commerce/address` forwards the `idempotency-key` header. Core derives customer ownership from the current session and rechecks current access and the edit's expected address version in the complete address/audit/receipt transaction after permanent geocoding. An exact successful retry returns the original public view before another provider call; a reused key with a different body conflicts. Disabled customers cannot replay protected results. The editor keeps an unconfirmed method/body/key and disables edits until explicit retry resolves it. Saved addresses and paid Order snapshots remain separate.
...
`listPublishedPromotionCampaigns` is an anonymous, bounded (20) read of active campaigns within their effective dates, with a valid supported benefit and active image. It supplies structured campaign text/benefit fields and opaque image URLs, never audience customer identities, raw rules, object keys or a promise of customer eligibility. `getPublishedPromotionMedia` rechecks the owner, effective dates and media version before returning bytes, including after the storage read. Same-origin Web adapters cache successful versioned image responses for at most five minutes in browsers and the Cloudflare Cache API, preserving Age on hits and honoring ETags. Cache misses/expiry revalidate Core publication; inactive/obsolete URLs then return unavailable with no-store caching. Version changes use distinct URLs; removal or campaign expiry can leave previously cached bytes available for up to five minutes. Checkout remains authoritative for audience, usage, stacking and final savings.
...
`packages/contracts/src/admin-catalog.ts` publishes `AdminCatalogService` (`listAdminCategories`, `createAdminCategory`, `getAdminCategory`, `updateAdminCategory`, `setAdminCategoryStatus`, `listAdminUnits`, `createAdminUnit`, `listAdminProducts`, `createAdminProduct`, `getAdminProduct`, `updateAdminProduct`, `setAdminProductStatus`, `setAdminProductCategories`, `uploadAdminProductMedia`, `updateAdminProductMedia`, `removeAdminProductMedia`, `createAdminSku`, `updateAdminSku`, `setAdminSkuAvailability`, `setAdminSkuPrice`) and `AdminInventoryReadService` (`listAdminInventory`, `getAdminInventoryLedger`).

- Authorization: Product identity, Category, media, unit, and SellableVariant definition surfaces require `catalog.read`/`catalog.manage` plus global scope. A location Product projection requires both `catalog.read` and `inventory.read` plus operational scope over the requested location. Exact-location price reads/writes require `prices.read`/`prices.manage` plus operational scope over the requested location; Global scope satisfies that location check but does not create a Global price. Local selling activation requires the authorized catalog capability and operational scope. Inventory reads require `inventory.read` plus operational scope over the requested location.
...
- SellableVariant creation validates that the controlled sell unit's dimension matches the Product pool and that integer sell quantity converts exactly to persisted base consumption; Admin derives that value for standard controlled units and Core verifies it. Non-gram variants may record a positive estimated shipping weight per sold unit as optional logistics reference. Variant updates and selling-status upserts are version-guarded (`sku_location_availability` inserts use `expectedVersion 0`). Product status toggles are global, reason-gated, audited, and guarded on current status.
...
- Both modes quote the fixed 20 kg Motorcycle parcel; nullable per-line shipping metadata does not alter eligibility or price admission.
...
- `setAdminProductCategories` requires one to twenty unique existing category IDs in ordered primary-first form. It atomically replaces every membership, updates the compatibility primary category and Product version, and records audit/idempotency evidence. Global Product detail returns the ordered category summaries. Storefront category queries use all active memberships; non-category Product projections continue to present the primary category.
...
- `getPublishedProductMedia({ mediaId, version }) -> PublishedProductMediaContent` is an anonymous Core query restricted to current active published Product media. Web serves `/media/products/{mediaId}/{version}`, caching successful public bytes for five minutes with `public, max-age=300, must-revalidate` and `nosniff`. Cache misses/expiry revalidate publication; unavailable or obsolete responses are not cached. Cache hits retain Age and ETag semantics; removal can remain visible for at most five minutes, while new versions have distinct URLs. Core bounds object bytes and returns only content, validated MIME and ETag, never object keys. Catalog home/search/category/detail projections use these canonical URLs. MarketplaceProductView.images supplies at most five active photos in main-photo/display order for the Product page and quick-view gallery; card and cart projections retain only the main photo. Upload forms retain the complete file/metadata/version/key for an unconfirmed outcome and retry the same intent.
...
The Admin Order list exposes the committed Order number, immutable recipient label, customer email, fulfillment mode, amount, commitment instant, lifecycle status, and operational status summaries through a purpose-built projection. The summaries include the latest Fulfillment and Delivery Job states plus the latest courier-attempt/application-normalized provider progress. Web composes those facts as current preparation and courier progress because those tracks can overlap; `UNASSIGNED` without an actual courier attempt is an internal placeholder, not the displayed Order status. Controlling Order-level results such as cancellation, delivery, expiry or exception override stale operational records. Admin order detail composes the sanitized recipient and delivery-address snapshot, immutable checkout financial and item snapshots, Payments, amendments, fulfillment, delivery, exception, merged timeline, allowed-action, and Audit projections; it is not a raw join response. Historical orders without an authoritative checkout quote explicitly return an `ORDER_TOTAL_ONLY` financial source with unavailable component amounts rather than fabricating a breakdown. Payment detail composes canonical attempts, refunds, provider-safe event metadata, downstream reactions, reconciliation cases, allowed actions, and Audit. Provider references, provider event identifiers, payload hashes/payloads, idempotency records, and reconciliation detail JSON do not leave Core. `CANCEL` and `REQUEST_REFUND` are returned only when both lifecycle policy and the caller's capability authorize the command.

Refund availability subtracts every reserved refund amount in `REQUESTED`, `PROCESSING`, `ESCALATED`, or `SUCCEEDED` before accepting another request. The guarded refund mutation, Audit event, and idempotency completion share one atomic D1 batch, so concurrent requests cannot over-refund or record a false success. Case completion is now a bounded internal operation after lookup redrive: current financial evidence, owning Order/refund cleanup, conditional case version, a distinct system audit and immutable per-case receipt commit together. No staff Close case endpoint remains. Stale versions, new provider work, ignored cleanup or audit writes leave the case open. The grouped attention read is oldest-first, counts groups rather than raw cases, and returns Core-derived legal recovery actions; it never writes. Downstream payment-reaction redrive remains Core-owned scheduled work rather than a second Admin retry authority.

`listAdminPayments` admits only `SUCCEEDED|PARTIALLY_REFUNDED|REFUNDED`, applies its closed status predicate before keyset pagination and returns readable customer and actual linked Order identity. `listAdminPaymentAttention` groups linked cases by Payment, unmatched evidence by exact provider-event identity, and otherwise by retained case identity. The Admin overview consumes this same grouped count. Ordinary waiting/processing and an open case whose verified effects are complete are not staff workload.

Payment detail supplies `canonicalStatus` plus a typed display status, actual Order link, and zero refundable balance outside a refundable captured state. Payment/refund recheck reads and writes require an exhausted/stopped recovery with usable existing identity and no active claim. Exact successful replay is checked before current eligibility, so an unknown response can recover its original receipt after progress.

Background payment lookup distinguishes awaiting-customer, processing, terminal, unavailable/missing identity, mismatch and failed local application. Healthy waiting resets its lookup failure budget and schedules one check at the persisted continuation deadline. A post-deadline exact awaiting-customer observation atomically completes lookup metadata and `PAYMENT.UNPAID_WINDOW_CONFIRMED` audit without changing canonical Payment/Attempt state; later signed or looked-up success remains legal and follows the ordinary exactly-once reaction path.

Operational command/read contracts publish the canonical Fulfillment history (`NOT_STARTED` through `COMPLETED`, with `SHORTED` resolution) and Delivery Job (`UNASSIGNED` through `DELIVERED`, with explicit failure/retry/escalation) states. The generic preparation command exposes only picking, shortage resolution, packing and `MARK_PACKED`; it cannot hand off, complete or cancel custody. Delivery-owned commands and verified observations own those later transitions. Core derives `allowedActions`; the former `START|PACK|SHORTAGE` and `DISPATCH|DELIVER|FAIL` shortcuts are not accepted. Procurement aggregation computes exact paid Scheduled demand without physical-stock netting inside its version-guarded command and permits one requirement per exact procurement run/SKU, including closed requirements. Order issues have no `REOPEN` action: `RESOLVED` is terminal and further work requires a new linked issue.
...
The Admin presentation maps the signed adjustment command to explicit **Add stock** and **Remove stock** actions and shows the server-recorded ledger date. This is the default manual inventory API path. Procurement, Receiving, and Fulfillment contracts remain separate because committed demand, accepted/rejected supply, and paid-order picking/packing cannot be safely collapsed into a stock setter; their authorized Operations routes are visible in the location navigation.
...
Instant first dispatch is system work triggered only after an authorized `START_PACKING` succeeds. Both Worker and alternate operations RPC callers invoke the same automatic command. Its atomic admission requires the committed Instant snapshot, `PACKING|PACKED` fulfillment, `FULFILLMENT_PENDING|FULFILLMENT_READY` Order, unchanged unassigned job, future promise, no cancellation, and no different attempt. It uses `auto-book:{jobId}` and `fm-auto-{jobId}` identities. The minute job recovers the same definitely unsubmitted intent, makes at most three safe submissions, and never replaces `CREATING`, active, uncertain or closed evidence. Packing success is independent of booking outcome. Scheduled first dispatch remains explicit location-scoped staff work after `PACKED`. Existing submitted or uncertain attempts keep their provider recovery and reconciliation paths.
...
immutable customer stop/contact/instructions, and the fixed full-order parcel. It rejects incomplete
profiles or customer delivery facts, stale/assigned jobs, pickup after the committed promise/window,
and providers that are not enabled. Manual execution is a separately guarded ordinary dispatch command for both modes; no internal-fleet assignment, Rider/batch command, or route preview is offered.
...
fail-closed until Cebu/city access, sender profile, fixed-parcel compatibility, webhook authentication,
...
- Test active mass/count units, integer SKU consumption, optional shipping-gram snapshots, fixed courier parcel admission, absence of universal packaging conversion, exact-location nonzero pricing with no fallback, deterministic Promotion component stacking/usage limits, provider quotation snapshots, and complete Quote/Order financial components without Service Fee or processing-fee charges.
- Test selling pause/mode-switch/reopen, Scheduled independence from stock/capacity, available-courier selection and the absence of hub choice, both-mode Lalamove pricing, all four mode/method dispatch combinations after packing, no automatic first provider call, final courier variance absorption, and absence of active Rider/batch/map/mock-payment contracts.
...
`getAdminLocationFulfillment` returns the location name/version, dispatch readiness, nullable Instant promise in integer minutes, setup blockers and management decision under Global `locations.read`. `configureAdminLocationFulfillment` requires Global `locations.manage`, the location expected version, complete readiness/promise settings, reason and stable key. Enabling dispatch requires an active customer site and market, an exact location pin, picking/packing/dispatch capabilities and courier pickup profile; it does not require a per-location service-area association, eligibility link or Instant operating hours. Core rechecks these mutable facts and authority in the same transaction as location/readiness/geography versions, unstarted-Quote invalidation, audit and the frozen result. Warehouses cannot dispatch customer orders. New commerce in both modes requires dispatch readiness and a separately configured Global service area; only Instant requires the minutes promise and configured operating hours. Web retains the full request/key after an unknown response. Later readiness changes or retired capacity fields cannot prevent commitment of a provider-confirmed payment; Instant commitment still requires its exact held quantities and atomic stock reservation.

Cycle setup uses `listAdminDeliveryCycles` (100-row stable ordering-time/identity pages for an explicit bounded visible range), `listAdminCycleDestinations` (50-row internally market-bound location pages), `saveAdminDeliveryCycleDraft`, `scheduleAdminDeliveryCycle`, and `cancelAdminDeliveryCycle`. The calendar read returns every cycle whose ordering-to-last-delivery interval intersects the requested range, including a cycle whose ordering started earlier, and consistently applies optional market, location and lifecycle-status filters. Its opaque cursor is bound to the exact range and filters; Web completes all pages or presents the range as incomplete rather than treating a partial page as empty time. The calendar route always supplies `rangeStart` and `rangeEnd`, keeping the legacy destination-list meaning of a lone `marketId` unambiguous. Reads require Global `fulfillment.read`; commands require Global `fulfillment.manage`. Draft save carries explicit ordering, procurement, preparation and pickup timing, exactly one customer delivery range, participating fulfillment locations, reason and expected version (zero for creation); scheduling and cancellation carry cycle identity, current version, reason and stable key. Market, zone and retained window labels are not operator choices. Core validates the complete write set and current authority/locations atomically, then returns a frozen original receipt. Matching replay precedes later lifecycle rejection; changed intent conflicts. Unknown Web responses retain the complete original request and key for explicit retry within the open editor or details panel.

Scheduled option discovery, quote creation and payment revalidation use the cycle opening, cutoff, pickup and customer delivery range; they do not consult location operating hours. Quote creation still requests the configured courier's supported future-pickup quotation and includes that accepted fee in the payable total. Missing, rejected, expired or out-of-horizon courier quotation evidence remains a checkout blocker. Instant option discovery and quote admission continue to require and transactionally guard the selected location's current operating interval.
...
The Scheduled option's customer delivery range includes an internal configured identity/name and actual start/end instants. Core resolves that identity from the opaque customer option; a browser-supplied cycle/location/provider is not authority. New cycle authoring and customer option discovery expose exactly one range. Quote and payment creation fence the accepted range and planned pickup; the customer Order detail renders immutable committed range evidence when available. Retained Orders without that evidence expose their original date, never a fabricated 24-hour range. Instant operating-hours configuration, subsequent cycle operations and provider acceptance remain separate acceptance work.

`getAdminSkuPrices({ skuId, locationId })` requires `prices.read` plus operational scope over the exact location and returns its currency/market, the currently effective price or null, the latest expected price version, Core's management decision and the latest 25 historical versions with their own currency and effective instants. `setAdminSkuPrice` requires `prices.manage` over that location; catalog permission alone is insufficient. Its complete write batch rechecks current staff/capability/location scope and active location/market/currency before closing a prior price, inserting the successor, auditing and completing idempotency. The fulfillment-location Product preview is the write surface and submits an immediate versioned update when the operator clicks and changes a selling-option price. Global Product preview remains catalog-focused and contains no price editor; role grants remain explicit.
...
- Global location create/update/deactivate, structured pickup-address/pin finalization and capability/schedule/closure setup use `locations.read`/`locations.manage`. Global service-area list/publish/coordinate preview uses the same Global permission boundary and accepts a named area code, name and simple GeoJSON polygon; it accepts no location IDs or nested delivery-zone definitions. Core supplies versions and legal actions. No operator types internal IDs manually.
- Exact-location pricing uses `prices.read`/`prices.manage` plus operational scope over the target location. Global scope may satisfy that operational check, but price editing is presented only inside a selected fulfillment-location Product preview. Local catalog activation remains a separate scoped command.
...
- Preparation start locks Order cancellation atomically. Packed completion consumes the appropriate stock/allocation exactly once and leaves the Order ready for explicit dispatch. No preparation transition creates an external attempt. Normal dispatch and handover reject unpacked goods.
- The delivery queue exposes Core-owned `courierPickup.allowedKinds` and a plain unavailable reason for both modes. Instant permits only immediate pickup. Scheduled permits immediate or future pickup after packing when the requested time remains ahead of the current clock and within the immutable delivery deadline/window. This is delivery readiness, not Scheduled sales capacity, a physical-stock reservation or predicted dispatch engine. Exact successful replay precedes mutable readiness rejection.
- The same scoped queue returns the latest attempt's persisted provider quote amount/currency and final courier payable/currency when recorded. These are operational provider amounts, distinct from the paid customer delivery charge; absent values remain unknown. Manual attempt reads retain the system selection reason, assigned person's name/phone, note, handover time, result, actual cost and currency. Web displays these saved facts after a fresh read and never infers a quote expiry timestamp or provider success from a submitted command alone.
- Manual assignment is ordinary for both modes and requires person name/phone plus definite closure of any prior attempt. Core supplies the audit reason `STAFF_SELECTED_MANUAL`; an operator note is optional. Manual handover/completion/failure is versioned and audited; unknown cost remains unavailable.
- `manageManualDelivery` requires current scoped `delivery.manage`, job/location, stable key and expected version. `ASSIGN` uses the job version and requires person/phone with an optional note; `HAND_OVER`, `COMPLETE` and `FAIL` use the selected manual attempt's version. Completion/failure accepts nullable actual cost in minor units; failure also requires a reason. Core derives `manualActions` in the delivery queue. Assignment requires the same shared packed first-dispatch eligibility as Lalamove, or a FAILED job whose latest attempt is definitively CANCELED/FAILED before handover. Recorded handover, returned goods and out-for-delivery custody require separate recovery; pending cancellation and active/unknown attempts block replacement. Handover requires packed fulfillment and a ready Order; completion requires this attempt's recorded handover. Custody, job/stop and Order progress, audit and frozen receipt commit together. The ordinary fulfillment command cannot bypass active manual custody. Failed attempts leave delivery FAILED, retaining custody and Order facts for separate recovery; no return, retry or refund is inferred. Unknown financial evidence remains null, while known cost records the accepted Order/addition delivery charges, currency and variance without changing customer charges. Web preserves the exact body/key after an unknown reply. These policies are Core command guards, not schema authorization; execution evidence remains in the active checkpoint.
...
### Global service areas and fulfillment-pin assignment

Global location administration owns each customer-fulfillment location's exact coordinate and courier pickup profile. A separate Global Service areas workspace lists numbered named areas, adds/edits one polygon at a time, and previews whether a customer coordinate is inside the active union plus which nearest pin would own it. Areas have no fulfillment-location selector or nested zones. Core first gates the coordinate against active areas, then assigns it to the nearest active/capable location pin, rechecks area and mode readiness at checkout/payment admission, and never lets stock select or split that ownership. Area or location changes advance the geography revision and invalidate unstarted quotes while started payments retain reconciliation and accepted terms.

`getAdminServiceability` returns active named area versions. `publishAdminServiceArea` publishes one independently versioned area and retires the previous active version of the same market/code. `previewAdminServiceability` returns `serviceable`, `serviceAreaName` and the nearest eligible location name without writing. The current request/response contracts contain no nested delivery zones, location cursor, location list or location links. Scheduled cycle tables may retain a zone identifier as a compatibility routing bucket until a later schema migration; delivery-zone geometry and timed location-link eligibility are never consulted for customer assignment.

Browsing confirmation, checkout options and quotes use the same Global area gate, fulfillment-location pins, customer-site purpose, capabilities and mode readiness. Nearest selection uses exact Haversine distance and stable location ID; the current area code/version is resolution evidence, while saved legacy delivery-zone labels do not authorize routing. Quote evidence includes geography/global-mode/location/readiness versions plus the accepted Lalamove quotation. Relevant configuration changes during courier quotation reject the complete quote batch, and payment initiation rechecks the selected area, location and accepted provider evidence inside its durable-intent transaction before contacting the payment provider. Existing payment-command replay remains available after geography changes.
...
Current Cart reads expose `paymentInProgress` only for retained pre-upgrade carts whose active Quote has a Payment that may still settle. Every Cart mutation, guest merge and location change rechecks that lock before work and again inside its D1 transaction. After Core durably adopts a grocery payment submission, it freezes the quoted Cart as `PAYMENT_PENDING` and creates an empty active successor; this is a cart handoff, not payment success. Customer Orders exposes these not-yet-committed checkouts through an ownership-checked, no-store read with only active unexpired provider actions. Notifications link to that durable view rather than relying on browser session storage. The submitted Cart becomes `CONVERTED` only in the canonical successful Order-commitment transaction.
...
The Problems list, weekly view and receiving form use purpose-built scoped reads and ordinary actions. Their presentation preserves current authorization, transaction and recovery safeguards without exposing technical steps as operator work. Purchase-based customer reports replace conflicting historical metric meanings. The single commerce checkpoint records executed acceptance and remaining provider/release limits; this original gap map is not another implementation ledger.

### Actual count commands (CA-4.3)

Global Product creation accepts `stockTracking` (`SHARED` by default or `COUNTED_SIZES`). Counted sizes use a GRAM bulk receipt pool and fixed one-piece/pack selling options with optional positive shipping-gram reference metadata. Existing omitted/default shared requests retain their original replay identity. The Product detail reports its tracking choice and each dedicated size pool; operational inventory lists distinguish bulk awaiting counts from exact counted sizes.
...
Purchase-demand rows carry nullable recorded shipping weight. If any paid line in a SKU/destination group lacks that metadata, the group's shipping weight is unknown and Web says “Not recorded”; exact paid base quantity and sold-unit totals remain available for purchase review.
...
Omitting `locationId` from `getAdminScheduledWeek` requests a consolidated Order summary or demand view and requires both Global scope and `procurement.read`. Orders and current offers still require an explicit authorized location. Order-summary cursors order SKU/pool/base-unit/immutable-label identities; purchase-demand cursors order SKU/pool/destination identities. Both reset when the selected scope, section or week changes. Demand rows carry the named destination, its exact quantity and whole-SKU/pool totals across the selected scope. These totals are calculated before the bounded destination page, include OPEN exact paid original/addition demand and exclude canceled demand; they never subtract physical stock. A local reader receives only local rows and local totals; omitting the location cannot widen their authority or individual Order access.
...
The location-scoped Fulfillment queue is the operational Orders read, separate from Global finance Orders. Each queue row may carry a location-safe detail composed from immutable paid original and committed-addition snapshots: human Order number and commit time, mode, recipient contact, committed cycle/window timing, Fulfillment/Delivery progress, blockers, exact quantities, and either Instant reservation or Scheduled committed-demand/received-goods evidence. Scheduled `receivedBase` is the shared delivery-week/location/inventory-pool receipt, not an amount received for one order line. It contains no payment, price, refund, email, or finance authority. Core derives legal actions and enforces exact location scope; an Order from another location is never returned. For a Scheduled Order in `PACKING`, the read omits `MARK_PACKED` and identifies receiving as the next step while the exact cycle/location goods are unavailable. The write rechecks goods inside the guarded transaction and returns a receiving-specific conflict for that known prerequisite; another stale claim still reports a stale version. A replay still processing returns `CONFLICT` with `error.details.outcome = RECONCILIATION_PENDING`; Web retains the exact command key and body only for that typed outcome. The closed `ALL`, `NEW`, `PREPARING`, `READY_FOR_DISPATCH`, `UPCOMING`, and `HISTORY` filter is applied in Core before the bounded page. Pages use descending `(COALESCE(committed_at,created_at),order_id)` cursors bound to the exact location, optional Order/cycle, filter and ordering context; a cursor from another query returns `VALIDATION_FAILED`. Delivery pages use descending `(delivery_job.created_at,delivery_job.id)` cursors. Their summary counts each open job once and reports `bookedJobs` only when that job's latest attempt is currently `ACTIVE`, for either accepted external or manual execution. Global finance Order pages use the same committed timestamp in projection, ordering, and cursor predicates.

`listOperationalActivity` is the lightweight, location-authorized refresh feed for the Admin bell and operational pages. It returns the union of the bounded notices permitted by every relevant capability the caller actually holds (`fulfillment.read` and/or `delivery.read`), with stable IDs and ISO occurrence times plus the newest identity. Holding neither capability is forbidden; missing and out-of-scope locations retain the protected read's concealment behavior. Web has one refresh owner for the selected location: 8-second visible-tab refresh, focus/online refresh, bounded backoff, cancellation on scope change, preservation of the last successful data, a stale indication, and session-level notice deduplication. Full Overview data is not polled to discover new Orders. Fulfillment and Delivery revalidate their scoped reads from this shared signal while keeping the mounted page visible.
...
### Standalone storefront banners (2026-09-11)

The storefront now consumes listPublishedBanners, not listPublishedPromotionCampaigns. Historical promotion-media APIs and records are retained for compatibility. listAdminBanners requires Global promotions.read and returns up to 200 banners ordered by priority and stable identity. saveAdminBanner requires Global promotions.manage, expectedVersion and an idempotency key; creation starts at version zero in DRAFT. Core atomically guards current authority/version, image presence when activating, mutation, audit and immutable result. Identical retries return the original receipt; changed payloads conflict. Optional destinations are bounded same-origin paths.

getAdminBannerMedia/getAdminBannerMediaContent require Global promotions.read. uploadAdminBannerMedia/updateAdminBannerMedia/removeAdminBannerMedia require Global promotions.manage and the current media identity/version plus a stable key. They use the same bounded JPEG/PNG/WebP, durable upload, guarded attachment, receipt and cleanup guarantees as retained campaign media, with independent banner ownership. Same-origin adapters are /api/admin/banners and /api/admin/banners/:bannerId/media[/:mediaId].

Anonymous listPublishedBanners returns at most 20 ACTIVE banners with an active image, startsAt <= now and an absent or future endsAt, ordered by priority descending then ID. getPublishedBannerMedia rechecks these conditions and version before and after R2 reads. /media/banners/:mediaId/:version uses the existing five-minute maximum image cache/ETag policy; withdrawal can remain visible in an already-fresh cache until expiry. No discount, customer eligibility or promotion-code dependency exists.

Vinext multipart preflight permits 6 MB (5 MiB upload plus envelope); media adapters still bound each file to 5 MiB. Plain-text HTTP 413 is a definitive upload-size rejection, not an unknown command outcome.

### Google Places address autocomplete

`autocompleteAddress(AddressAutocompleteRequest)` returns temporary `AddressPrediction[]`
(candidateKey/displayAddress only). `resolveAddressPrediction(AddressPredictionRequest)` returns one
validated `AddressSearchCandidate` with coordinates and structured components. Both requests carry a
UUID editor session token. Predictions cannot be used as confirmed coordinates. Web forwards private
POST bodies through the Core Service Binding using no-store responses; Google credentials and raw
provider payloads remain in Core. The token is reused across typing and selected-place details, then
