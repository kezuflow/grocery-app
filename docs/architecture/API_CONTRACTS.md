...
Scheduled option discovery, quote creation and payment revalidation use the cycle opening, cutoff, pickup and selected delivery window; they do not consult location operating hours. Quote creation still requests the configured courier's supported future-pickup quotation and includes that accepted fee in the payable total. Missing, rejected, expired or out-of-horizon courier quotation evidence remains a checkout blocker. Instant option discovery and quote admission continue to require and transactionally guard the selected location's current operating interval.
