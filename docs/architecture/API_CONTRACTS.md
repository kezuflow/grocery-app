...
- `AdminProductListRequest` carries an explicit authorized `pricingContext` (`marketId` plus optional `locationId`) in addition to query/status pagination. Each summary returns primary-media identity, active/priced/available SKU counts, and the resolved minimum/maximum price with currency for that context. The page returns readiness totals for active/inactive Products, missing primary media, missing authoritative prices, and unavailable SKUs. Totals are scoped and filtered by Core; Web never derives completeness from a truncated page.
