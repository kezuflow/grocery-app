...
- Deferred from Slice 5 but required for Phase 12 Admin Operations UI: `getAdminCategory`, `updateAdminCategory`, `setAdminCategoryStatus`, `createAdminProduct`, `updateAdminProduct`, customer-facing product-detail authoring, and canonical product-media association. Category commands include hierarchy, icon, sort order, status, idempotency, and optimistic concurrency; product commands compose identity/category/base-unit setup without exposing raw inventory-pool rows. Product and category lifecycle has no generic delete command. Bulk import remains deferred; purchase/receiving surfaces belong to Procurement/Receiving.
