...
The anonymous browsing confirmation uses `GeocoderPort.reversePermanent` through Google Geocoding and re-resolves the nearest current fulfillment-location pin at the selected entrance. Its minimal result contains the finalized display address, confirmed coordinate and location-assignment result; no provider reference or raw payload. Web calls the private-body, no-store `/api/commerce/browsing-location` endpoint only on Deliver here, and remembers only a successful, still-current assignment. Provider errors, lost location eligibility, editor closure and a moved selection cannot persist temporary or stale data. Search and editor pin autofill remain temporary. Browser storage uses new confirmed-location keys; old unconfirmed keys are discarded rather than migrated. This does not create a saved checkout address, claim courier coverage, or waive checkout revalidation. Actual Google Maps Platform account, billing, API enablement, key restrictions, storage terms, and attribution compliance remain activation requirements.

`ServiceabilityResult` retains the `serviceable`, area, zone and prior-resolution fields for wire and historical-address compatibility. For new pin-based resolutions, `serviceable` means an active capable fulfillment location was assigned, area/zone are null, and `OUTSIDE_SERVICE_AREA`/`OUTSIDE_DELIVERY_ZONE` are not emitted. Customers do not select a location; Core selects the nearest active, capable fulfillment-center pin by exact Haversine distance with stable location-ID tie-break and re-resolves current readiness at checkout. Product stock cannot select, replace or split the owning location. Lalamove's current quotation—not this result—decides route-specific delivery availability and fee before payment.
...
- Global location create/update/deactivate, structured pickup-address/pin finalization, capability/schedule/closure setup and nearest-location assignment preview use `locations.read`/`locations.manage`. Service-area/zone authoring is retired from the active Admin workflow. Core supplies versions, legal actions and eligible scope choices. No operator types internal IDs manually.
...
### Fulfillment-pin assignment and retained area compatibility

Global location administration owns each customer-fulfillment location's exact coordinate and courier pickup profile. The active Admin route edits those location facts; the service-area polygon workspace is retired. Core assigns confirmed customer coordinates to the nearest active/capable location pin, rechecks mode readiness at checkout, and never lets stock select or split that ownership. Location changes advance the geography revision and invalidate unstarted quotes while started payments retain reconciliation and accepted terms.

The retained `getAdminServiceability`, `publishAdminServiceArea`, `previewAdminServiceability`, service-area, delivery-zone and location-link structures exist only for old records, schema relations and immutable snapshots. They are not active customer-address, readiness, quote, courier-route or payment authority and are not exposed as an ordinary Admin workflow. Scheduled cycle tables may retain a zone identifier as a compatibility routing bucket until a later schema migration; no polygon geometry or timed location-link eligibility is consulted for customer assignment.

Browsing confirmation, checkout options and quotes use the same fulfillment-location pins, customer-site purpose, capabilities and mode readiness. Nearest selection uses exact Haversine distance and stable location ID; saved address area/zone labels do not authorize or deny routing. Quote evidence includes geography/global-mode/location/readiness versions plus the accepted Lalamove quotation. Relevant configuration changes during courier quotation reject the complete quote batch, and payment initiation rechecks the selected location and accepted provider evidence inside its durable-intent transaction before contacting the payment provider. Expiry or retirement of a legacy area/link cannot reject that path. Existing payment-command replay remains available after geography changes.
