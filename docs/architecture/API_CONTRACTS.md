...
Current Cart reads expose `paymentInProgress` while an active Quote has a Payment that may still settle. Every Cart mutation, guest merge and location change rechecks that lock before work and again inside its D1 transaction. Web leaves `/checkout` for a stored unexpired payment continuation or payment/Order status, and never presents a delivery re-quote action for this condition. The Cart becomes `CONVERTED` only in the canonical successful Order-commitment transaction; payment initiation alone does not clear it.
