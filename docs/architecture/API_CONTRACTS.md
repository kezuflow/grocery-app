...
Discount preview accepts optional `deliverySubtotalMinor`, required for a delivery benefit; missing courier cost is not zero. The amount calculation is shared with Quote, applies integer floor for percentage discounts and caps only the owned price component. Preview currently checks definition status/time/minimum and estimates the amount; it does not establish customer-target or usage eligibility. Web labels it an estimate. Full customer-aware preview remains an open campaign requirement. Canonical new quote applications use the explicit delivery percentage/fixed types; `DELIVERY_FEE_DISCOUNT` stays representable only for retained quote/payment evidence.
