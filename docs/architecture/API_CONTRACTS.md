...
New first dispatch is explicit staff work after packing; neither fulfillment RPC nor the scheduler creates a Lalamove attempt. The shared eligibility result requires current location-scoped `delivery.manage`, an eligible paid Order in `FULFILLMENT_READY`, `PACKED` fulfillment, a valid delivery deadline, and no active/uncertain attempt or pending cancellation. Both the read model and Manual/Lalamove commands apply that meaning, while each command revalidates the mutable facts atomically. Existing submitted or uncertain attempts keep their provider recovery and reconciliation paths.

The frozen new-dispatch matrix is:

| Mode | Lalamove | Manual |
| --- | --- | --- |
| Instant | Staff requests `IMMEDIATE` after packing using the accepted provider/service snapshot. | Staff assigns a named person and phone after packing; the system records `STAFF_SELECTED_MANUAL`, with an optional note. |
...
The reachable call-site map is: payment admission and trusted admission time in `create-payment.ts` and `apply-checkout-payment-reaction.ts`; preparation through `advance-fulfillment.ts` from both Worker and operations RPC entry points; external dispatch through `book-order-delivery.ts` then `request-provider-delivery.ts`; Manual dispatch/custody through `manage-manual-delivery.ts`; queue decisions through `list-delivery-dispatch.ts`; and retained provider webhook, refresh, inbox and cancellation recovery through their existing Delivery application handlers. The removed automatic helper, minute job registration and both RPC callers are not first-dispatch authority. FDP-3 consumes a location-operational Order projection authorized by `fulfillment.read`; it must exclude payment/refund administration and customer financial authority rather than weakening the existing finance boundary.
