...
Fulfillment-option reads require a Maps-confirmed active Customer address inside an active Global service area, an assignable nearest fulfillment-center pin, and an owned nonempty active cart at the submitted versions. Exactly the one current global `INSTANT` or `SCHEDULED` mode is returned, with controlled location, schedule, courier-quotation and fee availability. The new customer checkout presents only returned `INSTANT` options and never falls back to a returned Scheduled option; an empty Instant set is an unavailable/mode-mismatch state, not evidence that Web changed the operational mode. Each Instant row uses the returned provider code/display/service fields, while the opaque option ID remains command authority. Web may retain provider-code/service-type intent across a refreshed read, but must submit the new matching opaque ID. The option ID is re-resolved by Core at Quote creation; address/cart/global-mode/service-area/location/readiness/cycle or provider-quotation changes fail closed. An identical Quote idempotency replay returns the original immutable Quote even when current routing later changes, while the same key with another option is `IDEMPOTENCY_CONFLICT`.
