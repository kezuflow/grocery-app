...
- Authorization: Product identity, Category, media, unit, and SellableVariant definition surfaces require `catalog.read`/`catalog.manage` plus global scope. A location Product projection requires both `catalog.read` and `inventory.read` plus operational scope over the requested location; its exact-location price and selling-status commands require `catalog.manage` plus that operational scope. Inventory reads require `inventory.read` plus operational scope over the requested location. The guarded `inventory.adjust` command keeps its existing capability + scope + idempotency + version guards.
...
- `admin.fulfillment.getGlobalModeConfiguration({}) -> GlobalFulfillmentModeConfigurationView`
- `admin.fulfillment.activateGlobalMode({ fulfillmentMode, cadence?, expectedVersion, idempotencyKey }) -> GlobalFulfillmentModeConfigurationView`

The activation command is Global-scope only. `SCHEDULED -> INSTANT` returns controlled readiness blockers until outstanding Scheduled commitments are protected and open locations are Instant-ready. Location operational promise/capacity/readiness is configured separately and never selects another customer mode.
