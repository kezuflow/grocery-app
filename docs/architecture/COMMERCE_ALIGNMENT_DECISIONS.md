...
| Price authority | Price commands now require Global `prices.manage`; the full write batch rechecks current access and the active target. `getAdminSkuPrices` supplies exact-location current/history evidence under Global `prices.read`. Global Product detail has the editor; local Product detail retains activation and read-only prices. New permission definitions grant no roles automatically. |
