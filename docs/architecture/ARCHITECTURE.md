...
| Inventory                          | Integer location inventory positions, expiring Instant checkout holds, committed Instant reservations, and append-only movements                                                                                             | Scheduled preorder eligibility/demand, unit/SKU definitions, procurement approval, and fulfillment workflow state                                         |
| Procurement and Receiving          | Exact paid Scheduled demand aggregation, purchasing, receipt/discrepancy state, and supply exceptions                                                                                                                          | Scheduled stock/incoming/safety-buffer netting and direct unexplained inventory mutation                                                                    |
...
| External Delivery                  | Delivery jobs, normalized progress/exceptions, typed provider capabilities, Instant customer-selected provider evidence, Scheduled operations-selected provider/pickup timing, quotation/booking/reconciliation, and provider dispatch observations | Raw order-state mutation, payment/refund policy, internal fleet/Rider/batch execution, customer selection of Scheduled provider, and provider vocabulary outside adapters |
...
Cloudflare Cron Triggers are the approved time-driven execution mechanism. They own no domain state and contain no business rules: a scheduled handler dispatches through an explicit job registry to existing idempotent Core commands such as checkout/hold expiration, introductory-trial expiry, scheduled-cancellation application, provider-inbox/reconciliation redrive, provider-action expiry, delivery-cycle cutoff and advancement/closeout, and reminder scheduling. Every invoked command keeps its normal authorization, idempotency, expected-version, and concurrency semantics. Provider-inbox redrive uses the same conditional lease and normalized-observation application path as webhook delivery. PayMongo Scheduled Subscriptions own paid invoice creation and payment retries; Core schedules no renewal charge attempt.
...
Future scaling options include additional locations and markets, D1 read replication sessions for read-heavy operations, stock transfers, central/local procurement routing, Workflows for long-running orchestration, richer analytics stores, and selective module extraction. Extraction requires a demonstrated independent scaling, security, deployment, or ownership need and must preserve typed contracts and business invariants.

## Customer launch Runtime Completion

The launch customer flow remains one Core-owned transaction spine: selling `OPEN` -> Membership/Promotions eligibility -> confirmed address -> opaque fulfillment option backed by a provider quotation -> immutable Quote and explicit price acceptance -> provider-confirmed PayMongo outcome -> atomic Order commitment -> customer-safe Order follow-up. In `INSTANT`, the opaque option binds one currently enabled external delivery partner/service chosen by the customer. In `SCHEDULED`, it binds only the delivery window and Lalamove quotation; location operations later choose an enabled external provider plus immediate or supported scheduled pickup. Core never silently substitutes an accepted Instant provider and never charges the customer again for final courier variance. Web route handlers are thin same-origin adapters and never select a location, calculate price or courier cost, infer payment success, issue a refund, or mutate an Order directly.
