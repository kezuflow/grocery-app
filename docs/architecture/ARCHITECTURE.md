...
- provider-neutral geocoding ports, permanent provider finalization, Global service-area gating, nearest fulfillment-pin assignment, and courier-quotation delivery availability;
...
| Geography and Assignment           | Markets, independently versioned Global service-area polygons, exact fulfillment-location pins, location capabilities/readiness, confirmed-coordinate gating, and deterministic closest-location selection by Haversine distance                         | Per-location polygons/area links, customer-selected hubs, stock-based rerouting, courier route acceptance/fees, fulfillment execution, and catalog availability                            |
...
| External Delivery                  | Delivery jobs, normalized progress/exceptions, typed provider capabilities, automatic Instant Lalamove first booking at Start packing, staff-selected Scheduled dispatch, guarded recovery choices, execution-attempt timing, quotation/booking/reconciliation, and provider observations | Raw order-state mutation, payment/refund policy, internal fleet/Rider/batch execution, provider vocabulary outside adapters                                                              |
...
The default `pnpm dev` is isolated: D1, R2 and Queue state are local under `apps/core/.wrangler/state`; outbound Email is absent; and payment and delivery providers remain disabled. It cannot mutate deployed storage or create a deliverable row in a deployed notification outbox. `pnpm dev:shared-staging` is the explicit read-oriented opt-in for the remote `freshmarkets-core-staging` D1 and `freshmarkets-product-media-staging` R2. Browser origin/auth remain localhost with a private local signing secret. That mode disables local Email, Queue, cron, PayMongo and delivery-provider effects, but any D1/R2 write still changes shared staging and the deployed staging Core may consume a notification outbox row written there. Its startup warning names that indirect-effect boundary. Build and explicitly selected Wrangler environments retain their own configurations; provider/OAuth secrets are never inferred from remote bindings.
...
Cloudflare Cron Triggers are the approved time-driven execution mechanism. They own no domain state and contain no business rules: a scheduled handler dispatches through an explicit job registry to existing idempotent Core commands such as checkout/hold expiration, safe recovery of an admitted Instant first-booking intent, provider-inbox/reconciliation redrive, provider-action expiry, delivery-cycle cutoff and advancement/closeout, and reminder scheduling. Automatic booking recovery reuses the stable unsent identity, is bounded, and never replaces active or uncertain work. Every invoked command keeps its normal authorization, idempotency, expected-version, and concurrency semantics. Provider-inbox redrive uses the same conditional lease and normalized-observation application path as webhook delivery. No active membership or recurring-billing job is registered.
...
HSTS only for deployed HTTPS environments. The approved Google Maps script, worker, image, and
connect sources stay exact. Production CSP never permits `unsafe-eval`.
...
nonce; it permits neither `unsafe-inline`, `unsafe-eval`, nor `wasm-unsafe-eval` in any environment.
FreshMarkets explicitly selects Google Maps raster rendering because its current map interactions do
not require vector-only features and the vector renderer requires WebAssembly compilation. Static
Next headers do not carry request-varying CSP. Live storefront-auth, Admin, and
Maps/serviceability hydration flows verify the nonce policy while deployed HTTPS environments alone
receive HSTS.
...
The customer transaction spine is selling OPEN -> authenticated enabled Customer -> confirmed address inside an active Global service area -> nearest eligible fulfillment pin -> FreshMarkets promise/window with Lalamove quotation -> immutable accepted Quote -> provider-confirmed payment -> atomic Order commitment and assigned fulfillment work -> picking/checking -> Start packing -> Instant automatic Lalamove booking alongside packing, or Scheduled post-pack staff dispatch -> provider pickup -> delivery follow-up. Payment and picking never summon a rider. Instant automatic booking uses its accepted provider/service snapshot and a fresh quotation; Scheduled staff-selected immediate/future pickup must remain inside the committed deadline/window. Instant uses holds/reservations; Scheduled uses exact paid demand and cycle goods. Web is a thin adapter. The approved recovery choices are documented in PRODUCT; accepted fees never change with actual execution cost.
