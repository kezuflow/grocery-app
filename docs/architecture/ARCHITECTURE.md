...
- provider-neutral geocoding ports, permanent provider finalization, nearest fulfillment-pin assignment, and courier-quotation delivery availability;
...
| Geography and Assignment           | Markets, exact fulfillment-location pins, location capabilities/readiness, confirmed-coordinate assignment, and deterministic closest-location selection by Haversine distance; retained areas/zones are legacy snapshots only                           | Customer-selected hubs, stock-based rerouting, courier route acceptance/fees, fulfillment execution, and catalog availability                                                             |
