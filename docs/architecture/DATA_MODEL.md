...
The area/zone/link indexes remain for retained records and old snapshot relations. Active assignment indexes fulfillment-location market/status/purpose/capabilities/readiness and exact pins. Core ranks eligible location pins using computed Haversine distance then stable location ID, never stored link priority, product stock or route-provider driving time. Lalamove quotation then determines whether the selected pickup/drop-off route is available and its fee.

Phase 2 materializes these retained geography tables and stable seed identities for `METRO_CEBU`, `CEBU_CITY`, `CEBU_CITY_CORE`, and `CEBU_CENTRAL`. The initial GeoJSON is legacy bootstrap data, not an asserted legal/production Cebu City boundary and not an active launch prerequisite. Prior versions remain only for retained foreign-key/snapshot and audit context until a later migration removes that compatibility layer.

Migration 0072 adds Geography-owned `geography_configuration(market_id PK/FK, version, updated_at)`, initialized at version 1 for retained markets. Customer-site pin/readiness updates advance it atomically with dependent effects. New market setup must initialize its revision. New quotes retain the revision and global-mode version in immutable routing evidence; missing legacy revision means baseline 1. This is a concurrency fence owned by Core, not a database workflow-policy trigger. `service_area`, `delivery_zone`, and `location_serviceability` remain retained historical/snapshot compatibility structures; their polygons and eligibility intervals are not current customer-address, dispatch-readiness, quotation, or payment authority.
