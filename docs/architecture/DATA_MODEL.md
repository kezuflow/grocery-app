...
Migration `0091_delivery_promise_revisions.sql` adds immutable `delivery_promise_revision(id PK, delivery_job_id FK, dispatch_id, job_version, previous_promised_at, promised_at, agreement_note, actor_user_id FK, recorded_at)`. A composite foreign key binds the closed attempt to the same job; `(delivery_job_id, job_version)` is unique and supports the latest agreement read. Update/delete guards retain all agreements indefinitely. The original Order fulfillment/window snapshots remain unchanged. The command updates only the job's operational `promised_at` and version alongside the new agreement, audit and receipt. It preserves the job's delivery-status timestamp, so a deadline-only change does not generate another failure notification; the agreement has its own recorded time. Agreement records are not proof that returned groceries have been inspected, nor authority for a new charge or automatic refund.
