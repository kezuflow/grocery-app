...
`checkout_quote.cart_version` records the version used to produce a new Quote, with a transaction-time guard against Cart edits. Migration `0094_checkout_cart_completion.sql` leaves retained versions null rather than guessing from current contents. Paid commitment conditionally completes the matching active Cart as `CONVERTED`, retaining all Cart lines. Newer edits and unknown historical versions remain untouched. This conditional effect and its suppression check share the Order/Quote/reservation/demand/receipt transaction; they never discard newer shopping or block accepted payment merely because the Cart changed.
