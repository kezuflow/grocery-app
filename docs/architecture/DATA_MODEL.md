...
- `inventory_pools(id PK, base_unit_id FK)` where the referenced unit is the dimension's canonical base unit. Current release has exactly one shared pool per Product; no reciprocal Product pointer or independent SKU stock exists. Catalog creates the pool and Product atomically.
- `skus(id PK, product_id FK, code UNIQUE, display_name, merchandising_label NULL, sell_quantity, sell_unit_id FK, inventory_quantity_base, estimated_shipping_weight_grams NULL, status, sort_order, version)` derive their pool through Product. Sell/base quantities are positive integers, the nullable shipping estimate is positive when present, and persisted configuration is authoritative.
