...
PRODUCT's approved follow-up decisions supersede conflicting baseline shapes below. Actual local size counts and bulk receipt describe the same goods; their eventual storage must not derive piece counts from approximate grams or credit both representations as independent inventory. Product-sale quantity, fixed amount per selling unit, overlap prevention, eligible-cancellation restoration and ordinary-stock depletion need atomic records and immutable paid allocations. Courier integration uses one fixed order-level 20,000 g Motorcycle envelope; per-SKU and line shipping grams are nullable reference evidence, without packaging allowance or fit fields. New/unique/repeat customer reports use the approved purchase-based definitions. These are implementation obligations, not assertions that current tables implement them.
...
Active `service_area` versions are Global, market-owned expansion gates. Core accepts a customer coordinate when it is inside the union of those polygons, without assigning an area to a fulfillment location. It then ranks eligible fulfillment pins using computed Haversine distance and stable location ID, never stored link priority, product stock, polygon ownership, or route-provider driving time. Lalamove quotation determines whether the selected pickup/drop-off route is available and its fee. Delivery-zone/location-link indexes remain only for retained records and immutable snapshot relations.

Phase 2 materializes these geography tables and stable seed identities for `METRO_CEBU`, `CEBU_CITY`, `CEBU_CITY_CORE`, and `CEBU_CENTRAL`. The initial service-area GeoJSON is bootstrap data, not an asserted legal/production Cebu City boundary; an administrator must verify/publish launch coverage. Prior service-area versions remain for audit and retained foreign-key/snapshot context. Delivery-zone geometry and location links are not current assignment authority.

Migration 0072 adds Geography-owned `geography_configuration(market_id PK/FK, version, updated_at)`, initialized at version 1 for retained markets. Global service-area publication and customer-site pin/readiness updates advance it atomically with dependent effects. New market setup must initialize its revision. New quotes retain the revision and global-mode version in immutable routing evidence; missing legacy revision means baseline 1. This is a concurrency fence owned by Core, not a database workflow-policy trigger. Current active `service_area` polygons gate customer addresses and checkout. `delivery_zone` and `location_serviceability` remain historical/snapshot compatibility structures and do not assign current fulfillment locations.
...
- `administrator_ownership_transfer(id singleton PK, source/target auth and Staff restricted FKs, role_id FK, source/target email hashes, capability snapshot, revoked-session/removed-account counts, completed_at)` retains immutable evidence of the owner-authorized production correction in migration 0100. The synthetic source Staff row remains suspended and grant-free because the initial-setup receipt restricts its deletion; its Better Auth email is tombstoned and its sessions/accounts are removed. The temporary transfer transport and secret are not part of the runtime surface after completion.
...
Saved-address create/edit uses the existing idempotency store with scopes `customer.address.create` and `customer.address.update`, actor-qualified keys and a hash of the exact command. Current active Customer/principal and reviewed address version guard the whole address/audit/receipt batch after provider finalization. Every required effect must write one row or the batch rolls back. The immutable `customer_address_row_snapshot` receipt stores the normalized saved row, including its protected address data, never a raw provider payload. Replay derives the public address view from that frozen row and does not overwrite later edits. `delivery_instructions_json` stores new writes as one nullable `deliveryInstructions` value; compatibility readers combine distinct retained courier-facing fields without changing old rows. Retained private `notes` remain stored but are not returned or copied to courier/Order instruction snapshots. These safeguards need no schema change and never alter paid Order snapshots.

Migration 0095 adds nullable customer.account_phone and customer.default_address_id referencing customer_address. Retained customers receive null values, never inferred contact or default selection. Core enforces phone formatting and active same-customer address ownership. The default pointer shares Customer version with profile edits; removal atomically clears a matching pointer, changes the address to disabled, advances versions, audits and saves a receipt. The referenced row remains historical. Account phone is an editable default for a newly created address, independent of each address's recipient phone. Better Auth retains the account name and authentication identity. No paid Order snapshot is updated.

Migration 0079 adds nullable `customer.preferred_language` and boolean `customer.promotional_emails` (integer 0/1). Existing rows retain all earlier values and receive no language preference and no promotional opt-in. Core owns preference eligibility; Customer version, required audit and immutable command receipt commit with each preference update. Transaction update delivery has no customer mute column. No Better Auth identity is copied; saved-address contacts remain independently owned snapshots.

- `customer_addresses(id PK, customer_id FK, label, recipient, phone, address_components_json, barangay, city, postal_code, latitude, longitude, geocode_provider NULL, geocode_reference NULL, confirmation_source GEOCODER|USER_PIN|DEVICE_LOCATION NULL, user_confirmed_at NULL, delivery_instructions_json containing canonical deliveryInstructions or retained legacy fields, service_area_id FK NULL, delivery_zone_id FK NULL, resolution_version, serviceable NULL, serviceability_reason NULL, notes retained-private compatibility, address_json compatibility NULL, status, version, created_at, updated_at)`
...
The retained-baseline implementation adds `delivery_cycle_schedule` (one timezone/procurement/preparation/pickup plan per cycle), `delivery_cycle_window` (integer-instant customer arrival evidence with a retained compatibility name) and immutable `order_delivery_window_snapshot` evidence through migration 0082. Existing cycle, Order and payment rows remain untouched; no schedule/window facts are backfilled. A new draft writes zero to retained capacity/allocation columns without using them as policy. Core permits exactly one delivery range for each new or edited cycle and exposes only the earliest valid range from a retained multi-row cycle as a new customer option; committed snapshots remain valid immutable evidence. Composite foreign keys require the committed Order and selected range to belong to the same cycle. These relationships and immutable Order evidence are storage integrity. Core owns temporal ordering, draft lifecycle and fulfillment-location eligibility. Existing databases require the forward migration; changing source files does not migrate them.
...
`checkout_quote.cart_version` records the version used to produce a new Quote, with a transaction-time guard against Cart edits. Migration `0094_checkout_cart_completion.sql` leaves retained versions null rather than guessing from current contents. Durable payment-creation adoption moves the matching active Cart to `PAYMENT_PENDING`, retains all lines, and creates an empty active successor. Paid commitment conditionally completes the submitted Cart as `CONVERTED`; retained pre-upgrade active Carts remain compatible. Newer edits and unknown historical versions remain untouched. These guarded effects never discard submitted evidence, merge it into later shopping, or infer payment success.
...
`payment_lookup_recovery(payment_intent_id PK/FK, status PENDING|COMPLETED|EXHAUSTED, attempts, available_at, lease_token NULL, last_error_code NULL, version, created_at, updated_at)` owns bounded read-only lookup scheduling separately from canonical Payment versions. Migration 0075 adds it without changing retained payment evidence. Its lease token fences late workers. Healthy awaiting-customer evidence resets attempts and uses the retained provider-action expiry as the next availability; verified waiting at/after that deadline completes the recovery with a matching audit while leaving Payment and Attempt state unchanged. A staff recheck requires an existing exhausted record, expected Payment/recovery versions and usable provider identity; it cannot create ordinary recovery work. Exhaustion and its single deterministic case/audit commit together. Lookup recovery never authorizes another charge or fabricates a terminal payment outcome.

Automatic case completion preserves every `payment_reconciliation_case` row. It advances only an eligible open case version after exact financial evidence and owning cleanup, and writes `PAYMENT.RECONCILIATION_AUTOMATICALLY_COMPLETED` plus a `system.payments.reconciliation-completion` idempotency receipt keyed by case ID and expected version in the same batch. The audited unpaid-window path is limited to the deterministic `payment-lookup:<intent>` case. Multiple cases for one Payment keep distinct completion identities even though the Admin read groups them into one attention row.
