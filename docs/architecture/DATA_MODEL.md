...
- `skus(id PK, product_id FK, inventory_pool_id FK, code UNIQUE, display_name, merchandising_label NULL, sell_quantity, sell_unit_id FK, inventory_quantity_base, estimated_shipping_weight_grams NULL, status, sort_order, version)` where sell/base quantities are positive integers, the nullable shipping estimate is positive when present, and persisted configuration is authoritative.
