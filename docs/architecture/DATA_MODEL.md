...
Exactly one global membership price version is current for the paid `CALENDAR_MONTH` offer. Price versions are effective-dated and non-overlapping. Paid enrollment copies the selected price-version ID, amount, and currency to the paid Subscription; renewal reads only that agreed snapshot. A free-trial Subscription has no agreed paid-price snapshot. Closing the current range and adding a new version changes new paid-enrollment pricing without rewriting existing paid Subscriptions. Offers do not contain `trial_days` or another field that grants trial eligibility. The subscription vocabulary is exactly `PENDING`, `TRIALING`, `ACTIVE`, `PAST_DUE`, `PAUSED`, `CANCELED`, and `EXPIRED`; `CANCELED` and `EXPIRED` are terminal.
...
`order_cancellation(id PK, order_id FK UNIQUE, actor_type, cause, reason, status, retained_service_fee_minor, required_refund_minor, currency, version, created_at, updated_at)` is the Orders-owned financial coordination aggregate. `order_cancellation_refund_member(id PK, cancellation_id FK, payment_intent_id FK, required_amount_minor, currency, refund_id FK NULL, status, attempts, created_at, updated_at)` snapshots the exact original/amendment payment set. `(cancellation_id,payment_intent_id)` and non-null `refund_id` are unique. Payments continues to own every `payment_refund`; Orders only advances members from canonical Refund observations.

Migration `0048_membership_and_service_fee.sql` adds global effective-dated membership and Instant Service Fee configuration plus immutable agreed/snapshotted values. Migration `0049_payment_settlement_observation.sql` records immutable provider-neutral gross/fee/net settlement evidence. Migration `0050_coordinated_order_cancellations.sql` adds the cancellation aggregate and exact refund-member set. Transaction summaries are read models over existing immutable snapshots and allocate no official invoice serial.
