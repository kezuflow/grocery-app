...
A recurring membership agreement separate from individual orders. Its canonical states are `PENDING`, `TRIALING`, `ACTIVE`, `PAST_DUE`, `PAUSED`, `CANCELED`, and `EXPIRED`; `STATE_MACHINES.md` is authoritative for legal transitions. Only `TRIALING` and `ACTIVE` may be checkout-eligible, and effective timestamps still apply.
