...
Eligibility and transition prerequisites belong to Core policy and guarded command writes, not permanent schema triggers. In particular, shared both-mode packed dispatch eligibility, manual transition rules, and packed-handover/completion requirements must be enforced and tested in the delivery commands before exposure. Storage retains valid state vocabulary, assignment identity, timestamp consistency, immutable history and protections against overlapping or unresolved attempts; accepting a row directly in SQL is not permission to execute that business operation.
...
Commands include `ScheduleCycle`, `OpenCycle`, `ReachCycleCutoff`, `CloseOrderingEarly`, `BeginProcurement`, `BeginReceiving`, `BeginPacking`, `BeginDispatch`, `BeginDelivery`, `CloseCycle`, and `CancelCycle`.
...
- Reaching the planned cutoff prevents normal procurement-affecting customer modifications. Early ordering closure stops new checkout/payment admission through the cycle state but preserves existing paid Order cancellation rights through their immutable cutoff; procurement remains gated by that original cutoff and unresolved started payments.
...
- A verified checkout transition to `SUCCEEDED` attempts its `COMMIT_ORDER` reaction in the webhook application path. The durable `PENDING` reaction and scheduled redrive remain the recovery owner when that attempt cannot commit; both paths invoke the same idempotent command and immutable success receipt.
- Customer presentation may expose provider success as `FINALIZING_ORDER`, but may expose completed payment/Order success only after the immutable Payment-to-Order reaction link exists. Client polling and animation never advance either state machine.
- Successful Order commitment resolves its matching Orders finance exceptions atomically with per-exception audit evidence. Retained stale exceptions may be repaired by the owning commitment replay or automatic case completion, only from the exact committed Order/payment/reaction link. Case completion also requires all financial and dependent work to be complete and writes its own immutable audit/receipt; it is not an operator assertion.
...
- Expiring the continuation is not a Payment transition. A verified post-window `REQUIRES_ACTION` lookup completes only the lookup recovery and audits the unpaid-window conclusion; canonical `REQUIRES_ACTION` remains compatible with a delayed valid `SUCCEEDED` observation. Pre-deadline waiting is parked at the exact persisted deadline without consuming the bounded failure budget. `PROCESSING` remains pending confirmation and follows bounded automatic recovery rather than becoming routine staff work.
...
Lalamove prices both modes for checkout. Instant creates its first external attempt automatically when authorized staff enter `PACKING`; Scheduled staff explicitly choose Lalamove or Manual after `PACKED`. Payment and picking never start an execution attempt. The Instant trigger and minute recovery use one stable identity and cannot replace active, uncertain or closed evidence. Provider pickup before `PACKED` is retained for reconciliation rather than fabricating custody.

Instant rider search may overlap `PACKING`; finishing packing produces ready-for-pickup without creating another attempt. After `PACKED`, Scheduled pickup may be requested as immediate or future only when the pickup remains ahead of the current clock and within the immutable delivery deadline/window. Booking does not consume additional goods, imply rider assignment, or rewrite the customer promise.

One active/uncertain execution attempt is allowed per job. Manual is an ordinary first-dispatch choice for Scheduled. For Instant it requires definite closure of the automatic/provider attempt plus packed goods; it records name/phone and system audit evidence. Searching is distinct from rider assignment; refresh, webhook and inbox recovery share one normalized applying path. Provider cancellation never cancels a grocery Order.

The existing manual assignment command may move a FAILED job directly to ASSIGNED after the latest attempt is definitively CANCELED or FAILED before handover. The Order must be `FULFILLMENT_READY`, fulfillment must be `PACKED`, and no provider cancellation may be pending. Assignment retains the previous attempt and creates the next numbered attempt atomically. RETURNED attempts, recorded handover and out-for-delivery custody require the explicit inspected-return agreement before another assignment. The queue uses this same policy, without an intermediate operator retry step. Explicit courier retry instead admits FAILED -> RETRY_SCHEDULED together with its new durable attempt before contacting the provider. Searching remains distinct from ASSIGNED, and unknown submission outcomes retain that attempt without another booking. Original commercial promises and customer charges remain unchanged. After definite pre-handover closure, the customer-agreed revised-promise command records a new operational deadline without changing job/Order/fulfillment status; existing courier retry uses that deadline. The same agreement command can explicitly record inspected return: only definitely closed departure/return evidence with PACKED/HANDED_OFF goods and FULFILLMENT_READY/OUT_FOR_DELIVERY Order may restore PACKED/FULFILLMENT_READY. It requires current scoped authority, complete suitable packed goods and customer agreement. HANDED_OFF -> PACKED is reserved for this review; ordinary MARK_PACKED requires PACKING. Stock consumption, old attempts, original promise and payment remain unchanged. Inspection belongs to one closed attempt and cannot authorize a later return.

Manual assignment creates an ACTIVE attempt and ASSIGNED job. Handover keeps the attempt ACTIVE, records its handover time, and atomically advances packed fulfillment to HANDED_OFF, the job/stop to EN_ROUTE and the ready Order to OUT_FOR_DELIVERY. Completion requires that custody evidence and advances the attempt/fulfillment to COMPLETED and job/stop/Order to DELIVERED. Verified external pickup/completion applies the same HANDED_OFF/COMPLETED custody projection. Failure closes the attempt as FAILED and records a reason plus known cost or null; job/stop become FAILED while fulfillment/Order custody remains unchanged. Recovery does not follow automatically. Generic fulfillment handover/completion is unavailable while the manual attempt is ACTIVE.
...
| Paid before Scheduled packing and cutoff or before Instant `FULFILLMENT_PENDING` | Customer request under mode policy | Release Instant reservation or close Scheduled exact demand once cancellation is accepted | Refund the coordinated paid set; retain only an actual documented non-refundable courier charge when the approved customer-caused stage policy permits it |
...
Unsubmitted external booking intents in PENDING or RETRY_REQUIRED, with no provider delivery identity, may be closed as FAILED after five minutes of inactivity by the existing delivery reconciliation job. Their job/stop transition to FAILED and the original command is marked failed atomically with an immutable audit event. The saved attempt remains in history; ordinary explicit retry may create a new attempt. CREATING, active, unknown and reconciliation-required attempts are never released by this rule. No Order, payment, refund or stock state is changed. Current version/state guards ensure that a provider claim and expiry cannot both win; a missing required effect rolls back the entire closure.

Selected-item sales reuse the promotion lifecycle: target definitions change only in DRAFT; activation from DRAFT/INACTIVE rejects overlapping active option/location/date ranges and caps limited quantities to available goods. Deactivation stops new discounts but does not invalidate an already accepted paid sale allocation. Sale quantity is not a second stock lifecycle: uncommitted claims are effective while their physical hold is HELD, commitment consumes the allowance, and eligible cancellation restores the recorded consumption only with its RESERVED-to-RELEASED physical release. Abandoning an unpaid hold frees its claim; a refund alone does not release goods or restore quantity. See PRODUCT GD-D06–07 for the approved business rules and DATA_MODEL for persisted evidence.
