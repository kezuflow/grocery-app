...
Lalamove prices both modes for checkout, while authorized staff explicitly choose Lalamove or Manual after packing for execution. Neither picking nor packing starts a new external attempt. The durable intent, one-attempt safeguards and retained recovery paths still apply. Normal dispatch/handover requires `PACKED`; conflicting retained provider pickup evidence is retained and escalated.

After `PACKED`, Scheduled pickup may be requested as immediate or future only when the pickup remains ahead of the current clock and within the immutable delivery deadline/window. The booking command rechecks packing and timing at admission; it does not consume or reserve additional goods. An admitted booking does not advance preparation, imply rider assignment, or rewrite the customer promise.

One active/uncertain execution attempt is allowed per job. Manual is an ordinary first-dispatch choice for both modes and requires name/phone plus system audit evidence; replacement still requires definite prior-attempt closure. Searching is distinct from rider assignment; refresh, webhook and inbox recovery share one normalized applying path. Provider cancellation never cancels a grocery Order.
