...
- Renewal retry ownership prefers provider-native recurring/retry behavior when the selected provider safely owns retries; application-driven retries must not be layered on provider-managed retries. Where the application owns attempts, scheduling uses approximately +1/+3/+6 days inside the 7-day grace window; the provider integration specification finalizes exact behavior after provider capabilities are verified. Calendar-month billing semantics and the nominal billing anchor are preserved across short-month clamping.
