...
- Operations: Inventory, Warehouse Transfers, External Delivery, and Operational Exceptions. Inventory is the primary Instant stock workspace and presents explicit Add stock and Remove stock actions with dated immutable activity. Exact Scheduled purchasing, Receiving, and Fulfillment remain distinct Core-owned workflows reached contextually from Orders or exceptions.
...
Warehouse Transfers appears in Global and authorized location scopes. Global creates a draft from named active warehouses, destinations and product pools, then dispatches it. The destination accepts checked quantities through an ordinary action; only acceptance credits physical stock. The list filters by status/scope and opens a detail with planned, accepted and outstanding transit quantities, reasons and immutable receipt history. Draft cancellation has no stock effects. Keep retries behind the original action and preserve its request after an unknown response. Checked receipt includes the damaged and missing quantities still remaining after acceptance, including zero-credit checks or rechecking recovered goods. Responsive product cards keep planned, accepted, outstanding, damaged/non-sellable, missing, lost and returned quantities explicit. Global selects an outstanding category and records a reasoned loss or confirms a physically received, inspected sellable return. Fully accounted transfers with dispositions say Resolved, never Received. The Global stock-distribution read side separates central/site/physical, reserved, held and transit quantities with its damaged/missing subsets; it is searchable and paginated. These controls do not add operator recovery workspaces or another authoritative inventory balance.
