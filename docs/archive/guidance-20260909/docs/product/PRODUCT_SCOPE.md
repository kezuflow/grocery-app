...
- Active introductory membership trials are retired. Controlled non-membership promotions use Promotions-owned grant/redemption history and approved usage limits.
...
28. Customer grocery-order cancellation is ownership-, version-, and idempotency-guarded. Instant locks when staff accepts the paid Order (`FULFILLMENT_PENDING`); Scheduled locks strictly at its snapshotted cutoff and coordinates the original payment plus every committed addition. New commerce retains only a documented non-refundable courier cost when allowed, never a Service Fee. Eligible customer cancellations automatically initiate PayMongo refunds and show processing until confirmed. FreshMarkets-caused cancellation refunds in full.
...
33. Additions cannot be canceled independently. Existing unrelated refund activity routes normal cancellation to financial review; post-delivery exception refunds require staff review and global `refunds.manage`, a reason and audit evidence; authorized staff confirms approved refunds in the FreshMarkets dashboard and Core executes them through PayMongo and verifies/synchronizes the result without reopening the customer window. Pre-commit Quote abandonment remains separate.
