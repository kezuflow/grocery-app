...
**CA-7.5 — Confirm provider-derived browsing locations before retention** is the sole active implementation task. Source: [COMMERCE_ALIGNMENT_E2E_PLAN.md](../../product/COMMERCE_ALIGNMENT_E2E_PLAN.md), **Phase 7 — Complete journeys and activation evidence**, PRODUCT first-visit/remembered location and API_CONTRACTS geocoder storage requirements.

Acceptance: temporary search/reverse results remain transient; Deliver here obtains permanent Core confirmation before any cookie/localStorage persistence. Provider failure or a changed selection cannot save stale/temporary values, and the user can retry/skip. Retire existing unconfirmed browser-storage keys without treating them as approved data. Preserve current serviceability, Cart carryover and saved-address behavior. Verify real Web/Core confirmation with an explicit local provider fake, provider-flag/error tests and both browser widths; actual provider account/storage eligibility remains external.
...
CA-7.4 is committed/pushed as c435d720. Complete aggregate (1660 Core/198, 408 Web/102, 68 contracts/19 and all other components), final focused Core 64/3, and four desktop/mobile Instant/Scheduled journeys passed. Exact commands/scope/failures/screenshots are in its existing history entry. No process remains. CA-7.5 is implemented and locally accepted on c435d720 plus its intended draft. Core 74/5, final Web 35/4, contracts 68/19, workspace types, required runtime/static checks and both builds passed. vinext reports 15 supported/0 issues. Both desktop/mobile first-visit/confirmation/carryover journeys passed against the explicit test-only provider transport. Exact commands, scope, two corrected test-harness failures and screenshots are recorded in the CA-7.5 history entry. Owned stack stopped; no test process remains. Ready to commit/push.
...
| CA-7.5 | Active: permanent-geocoder confirmation before remembered-address retention; closes the CA-7.3 provider-storage implementation gap. |
...
**One next action:** commit/push CA-7.5, then activate CA-7.6 customer name/account phone/default-address/deactivation through existing Better Auth and Customers ownership. Preserve snapshots and earlier acceptance obligations.
