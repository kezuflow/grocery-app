...
- Branch `main`; observed HEAD and origin/main `e54d0cb3`, CA-7.1 committed/pushed. No unfinished application files remain from CA-7.1. CA-7.2 starts from this revision; preserve all excluded owner changes. Preserve all unrelated/protected changes.
...
CA-7.1 is locally accepted and pushed as e54d0cb3. CA-7.2 is locally accepted in this commit: Product/option names in both existing Cart projections and actual current-price Buy again with existing-quantity preservation and immutable paid Order. Focused Core 8/2, Web typecheck, diff check and desktop/mobile local-provider journeys 2/2 passed. Exact commands, failed assumption, tested scope and screenshots are in the existing history. No process is running. No provider/deployment acceptance.

CA-7.3 preflight only; no application edits yet. Confirmed paths:
- `apps/web/components/storefront/address/delivery-address-dialog.tsx` defaults to Cebu City; saves only session address/coordinates and does not propagate the resolved site. Existing AddressEditor returns ServiceabilitySelection including Core serviceability and already supports optional device location.
- `apps/core/src/catalog/service.ts` falls back to LAUNCH_LOCATION_ID in searchCatalog/getProduct (inspect marketplace home in the same service). Web home calls getMarketplaceHome without location; API catalog proxies already accept locationId.
- `apps/core/src/checkout/application/cart.ts` provisions a new Cart using activeFulfillmentLocationId/activeMarketCode. Existing Cart/contracts/BFF currently have no explicit browsing-location command. Read the actual resolution/schema/quote path before changing it; do not replace Core authority with a browser-selected site ID.
- `apps/web/lib/storefront/cart-client.ts` guest merge posts per line with fresh keys and replaces same-SKU quantities; only an empty-server-Cart mock test exists. Verify partial failure, existing quantities and sign-in carryover coherently.
- Paid commitment consumes the Quote but leaves the Cart active. Quote/Cart version and intervening-edit preservation need explicit lifecycle evidence before cleanup.
- `customer-launch-journey.spec.ts` and some Admin finance/visual mocks still assert retired membership features; align obsolete tests while preserving valid post-payment coverage. Do not restore retired screens from tests.
...
| CA-7.3 | Active: first-visit location and guest Cart carryover; paid-Cart lifecycle gap remains visible. |
...
| IDs | Result / commit |
| --- | --- |
| CA-7.2 | Cart names/current-price Buy again locally accepted in this commit; Core 8/2, Web typecheck and both real local browser widths passed. |
...
**One next action:** commit/push verified CA-7.2, then implement CA-7.3 across the existing geography/catalog/Cart contracts and Web. Start with the current serviceability and Cart/quote storage boundaries; keep earlier Phase 6–7/CA-0–2 gaps assigned. No business decision currently blocks independent work.
