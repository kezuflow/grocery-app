...
Working-tree verification: `pnpm --filter @freshmarkets/web test:e2e tests/announcement-popup.spec.ts` with `E2E_START_STACK=1` and disposable `E2E_STATE_NAME=e2e-announcement-close-right` passed **2/2**, including the close-position assertion, Escape dismissal and mobile action reachability. The first run failed only because the new position assertion required a gap where the grid columns meet; its corrected inclusive boundary passed on rerun. `pnpm --filter @freshmarkets/web typecheck`, targeted `oxfmt --check`, targeted `oxlint` and `git diff --check` passed. A repository-wide `pnpm format:check` reported unrelated active Core files; the full aggregate was not run against the concurrent incomplete Scheduled-order changes. Browser evidence is local, not a production deployment. Completion level: **1 of 1 presentation slices locally browser-verified; 0 production releases requested or performed**. Next action: commit and push only the announcement files and this checkpoint, then continue the separate Scheduled-order obligations recorded below.
