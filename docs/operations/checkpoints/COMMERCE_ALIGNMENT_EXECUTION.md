...
**CA-7.6 — Customer account details and saved-address management** is the sole active implementation task. Source: [COMMERCE_ALIGNMENT_E2E_PLAN.md](../../product/COMMERCE_ALIGNMENT_E2E_PLAN.md), **Phase 7 — Complete journeys and activation evidence**, section E and PRODUCT GD-D01.

Acceptance: customer name edits use Better Auth without copying identity fields into customer storage; account phone belongs to Customers and supplies an editable default for a new address. Customers can choose a default address and remove saved addresses by deactivation. Only their active addresses can become default; deactivation clears that default, retains records, and never rewrites paid Order snapshots. Core rejects stale, disabled and wrong-owner commands without partial business/audit/receipt effects; exact retries remain safe. Verify storage upgrade, scoped/versioned/replay/race cases and desktop/mobile real account/address flows. Account recovery and closure evidence plus retired-membership browser expectations remain assigned even if split.
...
CA-7.4 is committed/pushed as c435d720. Complete aggregate (1660 Core/198, 408 Web/102, 68 contracts/19 and all other components), final focused Core 64/3, and four desktop/mobile Instant/Scheduled journeys passed. Exact commands/scope/failures/screenshots are in its existing history entry. No process remains. CA-7.5 committed/pushed as 9decade6. Core 74/5, final Web 35/4, contracts 68/19, types, required runtime/static checks, both builds, vinext and both first-visit/confirmation/carryover browser journeys passed. Exact evidence and test-harness corrections are in the CA-7.5 history entry. No process remains. CA-7.6 is implemented and locally accepted on 9decade6 plus the intended draft. Migration checks, Core 133/4 plus final permission 112/2, Web 25/3, contracts 68/19, types/static/runtime checks and both builds passed. Both account/name/phone/default/address-edit/removal browser journeys passed (2/2); final safeguards and their verification scope are explicit in the CA-7.6 history entry. Intended code/spec/transaction diffs reviewed. No test process remains. Ready to commit/push; no actual provider/deployment acceptance.

Earlier acceptance gap discovered during this slice: existing createCustomerAddress/updateCustomerAddress need a targeted current-principal/required-effect review around permanent provider finalization; they predate the new guarded default/removal command. Do not claim complete saved-address CRUD transaction acceptance solely from the new default/removal tests. Account recovery/closure and retired-membership browser obligations remain open.

Earlier obligations preserved: actual permanent-geocoding account/storage acceptance and the eligibility of retained pre-fix provider-derived records remain external; no existing saved records were rewritten. Customer name/account phone/default address and saved-address deactivation are approved but absent from the current preferences/address UI. Customer-launch/storefront/Admin mocks with retired membership/default-location expectations still need alignment. Earlier phase/provider obligations remain below.
...
| CA-7.6 | Active: account name/phone and saved-address default/deactivation with retained paid snapshots. |
...
**One next action:** commit/push verified CA-7.6, then activate CA-7.7 saved-address create/edit transaction and exact-retry safeguards. Continue account recovery/closure and retired-membership browser obligations afterward.
