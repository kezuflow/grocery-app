...
## Current presentation follow-up — GUEST-CART-NOTE-REMOVAL-1 (2026-09-26)

Active plan: `docs/product/COMMERCE_ALIGNMENT_E2E_PLAN.md`, **Phase 7 — Complete journeys and activation evidence**, with Storefront cart presentation in `docs/design/DESIGN.md`. The owner requested removing “Your cart is saved on this browser. Sign in only when you are ready to check out.” and its container from the guest cart drawer. Acceptance: neither message nor note container appears for a guest with cart items; the cart, promotion entry, summary and Sign in to checkout action remain usable.

Started on `main` at `8b6065b0` with unrelated in-progress Scheduled-order/Core/Web files and an untracked ZIP preserved. Removed the single guest-only note paragraph from `apps/web/components/storefront/marketplace/cart-drawer.tsx`; no commerce authority, persistent data or provider path changed. The implementation and focused browser assertion were committed as `d162b96c` and pushed to `origin/main`. Verification against that working-tree scope: cart-drawer Vitest **12/12** passed; Web typecheck, targeted formatter/lint and `git diff --check` passed; managed local Web/Core/D1 Playwright `storefront-home.spec.ts --grep "guest cart remains visible and asks for sign-in before checkout"` passed **1/1** with disposable `E2E_STATE_NAME=e2e-cart-note-removal`. This browser test observed the guest cart content, absent message and centered checkout action. No production browser check or deployment was requested; no actual provider operation was involved. The repository aggregate was not run against concurrent incomplete Scheduled-order changes. Completion level: **1 of 1 cart presentation slices locally browser-verified and pushed; 0 production releases requested or performed**. Next action: continue the separate Scheduled-order obligations below.

## Current presentation follow-up — ANNOUNCEMENT-CLOSE-RIGHT-1 (2026-09-26)

Active focused plan: `docs/product/STOREFRONT_ANNOUNCEMENT_POPUP_PLAN.md`, **Storefront announcement popup — plan and assets**, within `docs/product/COMMERCE_ALIGNMENT_E2E_PLAN.md`, **Phase 7 — Complete journeys and activation evidence**. The owner requested moving the welcome popup Close button from left to right. Acceptance: Close is visibly right of the centered FreshMarkets brand, remains keyboard-accessible and dismisses the popup, and the existing mobile action remains reachable.

Started on `main` at `cf978b2f` with unrelated active Scheduled-order/Core/Web changes and an untracked ZIP preserved. The announcement component now places the spacer, brand and Close control in left, center and right grid columns; the focused popup plan records the placement. No Core business path, provider operation or production state was changed.

Working-tree verification before commit `586a86ec`: `pnpm --filter @freshmarkets/web test:e2e tests/announcement-popup.spec.ts` with `E2E_START_STACK=1` and disposable `E2E_STATE_NAME=e2e-announcement-close-right` passed **2/2**, including the close-position assertion, Escape dismissal and mobile action reachability. The first run failed only because the new position assertion required a gap where the grid columns meet; its corrected inclusive boundary passed on rerun. `pnpm --filter @freshmarkets/web typecheck`, targeted `oxfmt --check`, targeted `oxlint` and `git diff --check` passed. A repository-wide `pnpm format:check` reported unrelated active Core files; the full aggregate was not run against the concurrent incomplete Scheduled-order changes. Browser evidence is local, not a production deployment. The scoped implementation, test, focused plan and checkpoint were committed to `main` as `586a86ec` and pushed to `origin/main`; unrelated in-progress files remained untouched. Completion level: **1 of 1 presentation slices locally browser-verified and pushed; 0 production releases requested or performed**. Next action: continue the separate Scheduled-order obligations recorded below.

## Current owner follow-up — SCHEDULED-SETTLEMENT-DECISION-1 (2026-09-26)

Active plan: `docs/product/COMMERCE_ALIGNMENT_E2E_PLAN.md`, **Phase 7 — Complete journeys and activation evidence**, with **Phase 5 — Scheduled operations** and the Payments boundary affected. Stable ID: `SCHEDULED-SETTLEMENT-DECISION-1`. The owner approved an exact Scheduled order cutoff, no further QR generation or renewal at cutoff, up to 30 minutes for an already-issued QR, one hour total for settlement, then a paid-demand freeze for purchasing. A later confirmed capture on an uncommitted checkout is flagged and automatically refunded in full; unknown/unpaid attempts are not refunded. Receiving follows a confirmed supplier purchase and actual goods. Acceptance for this design/documentation slice: record the approved rule without claiming it is deployed, and reassess the remaining Scheduled audit findings against it.

Observed synchronized `main`/`origin/main` at `8f1088ca` before this documentation edit, with only unrelated untracked `docs/freshmarkets-shopify-admin-plan.zip` preserved. The current code still allows a pre-cutoff Payment to create an Order without a settlement deadline; Web can renew a QR while its one-hour action remains alive, Core has no server-owned QR-renewal cutoff or PayMongo intent cancellation, and the purchase predicate can block indefinitely on exhausted or verified-unpaid attempts. The current cycle UI defaults purchase planning to midnight and its aggregation guard uses `cutoff_at`, not the new cutoff-plus-one-hour freeze. The cycle vocabulary already contains 11 states; timed QR-settlement/reconciliation phases can be derived without inventing new persistent cycle statuses. The five retained production payment cases require financial reconciliation even when purchase becomes eligible under the new rule. No production write, provider operation, source behavior change or deployment was performed in this follow-up.

The remaining audit findings were assessed read only: the weekly page has one generic payment-blocker string and a five-second refresh timer; the Fulfillment blocker routes to Receiving even before purchase exists; the mobile week selector has a fixed minimum width that caused horizontal overflow; and Scheduled `START_PACKING` currently lacks the goods guard applied to `MARK_PACKED`, despite closing customer cancellation. These remain implementation obligations. Local/full browser and actual-provider acceptance retain the prior audit results below; this documentation edit does not change them.

Completion level: **1 of 1 owner settlement decisions recorded**; **0 of 4 independent UI/workflow findings repaired**; **0 of 1 settlement implementation slices accepted**. Next action: implement the guarded cutoff/QR/deadline/freeze/late-refund path and resolve or safely isolate the five retained financial cases, then fix the typed operator route, mobile width, weekly refresh and packing admission, followed by full Worker/D1/browser and actual-provider acceptance. No deployment authorization was given for this follow-up.

## Prior owner follow-up — SCHEDULED-CUTOFF-QR-DESIGN-1 (2026-09-26)

Active plan: `docs/product/COMMERCE_ALIGNMENT_E2E_PLAN.md`, **Phase 7 — Complete journeys and activation evidence**, with **Phase 5 — Scheduled operations** and the Payments boundary affected. Stable ID: `SCHEDULED-CUTOFF-QR-DESIGN-1`. After the Scheduled audit, the owner questioned why five uncertain pre-cutoff Payments need resolution if unpaid Orders should be excluded at cutoff, and asked whether all issued QR codes can be invalidated. Acceptance for this read-only design clarification: distinguish Order commitment from money uncertainty, verify current code and the provider's published cancel/expiry capabilities, and identify the rule and implementation change needed for a hard paid cutoff.

Observed synchronized `main`/`origin/main` at `78a8da8b` with only unrelated untracked `docs/freshmarkets-shopify-admin-plan.zip`. The current approved PRODUCT rule admits a Scheduled Payment before cutoff and allows its verified commitment after cutoff. Core's Order reaction checks the Payment's creation time against cutoff, not provider payment-success time. Web creates QR Ph methods with `expiry_seconds` capped at 30 minutes and the one-hour local action expiry; neither is capped to the cycle cutoff. Core's provider-action expiry updates only local D1 action status. The PaymentProvider port and PayMongo adapter have no Payment Intent cancel operation, and the adapter does not recognize `qrph.expired` as a financial event. Official PayMongo documentation describes single-use dynamic QR Ph expiry (30 minutes by default, configurable 60–9000 seconds), a `qrph.expired` event, and `POST /v1/payment_intents/{id}/cancel` with a secret key. The public cancel reference does not establish that an in-flight bank payment can always be recalled; provider behavior and account capability still need sandbox verification. No actual provider call, customer message, production D1 write, source behavior change or deployment occurred.

Answer to the owner: an unpaid checkout never creates an Order, but current code may create one from a Payment started before cutoff and confirmed after it. A hard paid cutoff would require server-owned cancellation of unresolved Payment Intents at cutoff, QR lifetime capped to remaining cycle time where the provider supports it, a guarded classification of provider-success timing, and an explicit treatment of an already processing or later captured payment (normally refund without Order if the new policy excludes it). A displayed QR disappearing or local action expiring is not proof that funds did not move; the five retained uncertainties still require financial reconciliation, but under a new hard-cutoff policy they need not necessarily hold up purchasing once late captures have a safe refund path. The exact meaning of “paid by cutoff” (provider paid instant versus receipt of our webhook) and the treatment of a pre-cutoff payment observed late remain material owner decisions. This checkpoint records a proposed correction, not an approved replacement for PRODUCT's existing late-commitment rule.

Completion level: **1 of 1 QR/cutoff feasibility questions answered**; **0 implementation slices or provider cancellations performed**. Next action if the owner confirms the hard paid cutoff: update PRODUCT/API/state rules and implement one guarded Core/provider/Web/payment-reaction slice with provider cancellation race, delayed webhook, refund, and purchase-readiness tests before a separately authorized release.

## Prior audit — SCHEDULED-E2E-AUDIT-1 (2026-09-26)

Active plan: `docs/product/COMMERCE_ALIGNMENT_E2E_PLAN.md`, **Phase 7 — Complete journeys and activation evidence**; the audit retains **Phase 5 — Scheduled operations** and **Phase 6 — Delivery and customer alignment** obligations. Stable ID: `SCHEDULED-E2E-AUDIT-1`. The owner challenged why post-cutoff purchasing considers earlier payment attempts and requested a comprehensive Scheduled-mode E2E audit. Acceptance for this audit: trace the customer-to-delivery path and the live blocked Order, inspect the cutoff/payment/purchase/receiving/packing guards, execute relevant local Worker/D1 and browser checks, distinguish actual provider acceptance, and record prioritized findings without changing production business state.

Observed synchronized `main`/`origin/main` at `7b1ce681392fd86dd1323c2f68b6a6fdeef2940b`, with only unrelated untracked `docs/freshmarkets-shopify-admin-plan.zip` preserved. The detailed, redacted evidence and prioritized findings are in [the Scheduled-mode audit](../SCHEDULED_MODE_E2E_AUDIT_20260926.md). At 2026-09-26 06:01 UTC, the production cycle was early-closed but its original cutoff remained Sep 26 11:59 PM Asia/Manila. Its one paid Order had exact demand for one Abiu and no purchase, receiving session or accepted cycle goods. Five other pre-cutoff Payments remained `REQUIRES_ACTION`, all with expired actions, exhausted lookup recovery and open financial cases (three lookup unavailable, two unresolved). The reconciliation job was running successfully but its latest four runs selected zero lookups. This is a live operational blocker after the original cutoff unless financial evidence changes; action expiry alone is not payment failure. The production Admin Fulfillment, Receiving, Delivery weeks and Payments views were inspected read only. No live Payment recheck, provider call, purchase, receipt, packing command, D1 write, message, courier operation, migration or deployment was performed.

Local verification at the starting HEAD: focused Core Worker/D1 Vitest passed **146/146 tests across 8 files**. Managed local Web/Core/D1 Playwright with an initially absent, dedicated `e2e-scheduled-audit-20260926` state, fake provider ingress, one worker and zero retries completed **9 passed / 7 failed across 16 tests**. The full Scheduled browser journeys stopped on obsolete article locators despite correct demand being visible in a table; mobile weekly pages overflow, and the remaining failures are calendar-day selector ambiguity, payment-detail overlay interaction, and second-site receiving selection. These failures remain open, not waived as application acceptance. Actual PayMongo/Lalamove account acceptance and post-cutoff production behavior were not tested. Source inspection also found that verified post-expiry unpaid-window lookup completion leaves canonical `REQUIRES_ACTION`, which the purchase readiness predicate still blocks; this liveness mismatch needs a product-consistent fix and a purchase regression test. No source behavior changed in this audit; only the evidence report and checkpoint were added.

Completion level: **1 of 1 requested Scheduled-mode audit completed** at the investigation level; **0 of 1 full Scheduled browser journeys accepted**, **0 of 1 actual-provider Scheduled journeys accepted**, and Phase 5/6/7 acceptance gaps remain open. Next action: resolve the five live payment uncertainties through the authorized Payments/provider path, then prove and repair the verified-unpaid purchase-readiness rule and rerun the failed complete browser journeys. This audit does not authorize a new cutoff/finality policy or a production operation.

## Prior production release — OPERATIONAL-PUSH-DEPLOY-1 (2026-09-26)

Active plan: `docs/product/COMMERCE_ALIGNMENT_E2E_PLAN.md`, **Phase 7 — Complete journeys and activation evidence**; focused owner: `docs/product/FULFILLMENT_DISPATCH_ALIGNMENT_PLAN.md`, **FDP-4 — Make new orders visible without manual page refresh**. Stable ID: `OPERATIONAL-PUSH-DEPLOY-1`. The owner explicitly requested deployment of the pushed event-driven Fulfillment/POS refresh. Acceptance for this release: apply the required production D1 schema, deploy paired Core/Web Workers in dependency order, verify production readiness and stream access, and retain the limits of live behavioral evidence.

Started with synchronized `main`/`origin/main` at `22e086bc359628d13d7eebf273c6d5a3ca1eeee4`; only unrelated untracked `docs/freshmarkets-shopify-admin-plan.zip` was present and remains untouched. The source implementation and local/browser checks are recorded in the prior implementation section below. No source behavior was changed in this release; this checkpoint records the production operation. Initial read-only production D1 migration/query calls returned transient Cloudflare authorization errors, but the same account and target succeeded on retry before any remote write. The target was production D1 `freshmarkets-core-production` (`66f3668a-35ed-42fa-bd03-bab73cbb05ee`), about 48.7 MB before migration. Its migration history ended at `0101`; `0102_operational_queue_indexes.sql` and `0103_operational_revisions.sql` were pending. The prior release had deliberately deferred `0102`; Wrangler applies pending files in order, so this deployment window included that index-only migration immediately before `0103`.

Preflight: `pnpm migration:check`, `pnpm --filter @freshmarkets/core build`, `CLOUDFLARE_ENV=production pnpm --filter @freshmarkets/web build`, and strict production Core/Web Wrangler dry runs passed on `22e086bc`. The generated Web config selected `freshmarkets-web-production`, `freshmarkets.ph`, and the production Core service binding. Cloudflare D1 Time Travel returned both a current bookmark and a bookmark at the requested timestamp one hour earlier, confirming a recent platform-managed restore point before the migration. `pnpm --filter @freshmarkets/core exec wrangler d1 migrations apply freshmarkets-core-production --remote --env production --config wrangler.jsonc` applied `0102` and `0103` successfully. A subsequent remote migration list reported no pending migrations, and read-only schema inspection found the queue indexes, `operational_revision`, its pending index, and the Order trigger. The public Core health bridge stayed HTTP 200 after schema application.

Core production deploy with `--env production --strict --message "Operational event refresh 22e086bc"` succeeded as version `6e33e4cb-9864-49d7-b7f1-f18bea0f11a6` at 100% traffic. Its `/health` and `/ready`, and Web's `/api/core-health`, returned HTTP 200 before the Web release. The production Web build deployed with `--strict --message "Operational event refresh 22e086bc"` as version `62af3cd9-4b81-4529-bd01-1b62c46d2fa1` at 100% traffic on `freshmarkets.ph`. Afterward, public `/health`, `/api/core-health`, and `/admin/fulfillment` returned HTTP 200. A same-origin unauthenticated WebSocket upgrade to `/api/admin/operational-stream` returned 401; a cross-origin upgrade returned 403. The first in-app browser inventory returned no open tab, so the initial release check did not include authenticated browsing. No actual paid Order, provider transaction, courier booking, customer communication, or production business-row test write was made. D1 row/cost reduction has not yet been measured against comparable post-release traffic.

Owner correction and authenticated browser follow-up: the in-app browser subsequently exposed the owner's existing signed-in production tab. Read-only navigation loaded `/admin/fulfillment` and `/admin/point-of-sale` under the Central Cebu scope, with the same paid Scheduled Order visible on both surfaces. POS briefly displayed `Updates delayed` during initial connection, then cleared it and showed `1 on this page`; the client implementation clears that stale state on WebSocket `onopen` (or a successful activity read after the socket is connected). The browser's captured warning/error log list was empty. This accepts an authenticated page/initial-stream smoke check, but no second staff session changed an Order during observation, so event delivery and resulting queue revalidation remain unproven in production. The tab was returned to Fulfillment.

Completion level: **1 of 1 requested production release completed**, **1 of 1 authenticated page/initial-stream smoke checks accepted**, and **0 of 1 cross-session event journeys accepted** at the Phase 7 evidence level. Remaining work: observe a new committed paid Order and one material staff transition from another signed-in staff session, then compare production D1 query/row metrics after comparable traffic. Next action: perform that cross-staff acceptance when a real authorized change occurs and record measured D1 cost; retain wider Phase 7 provider and full-journey gaps separately.

## Prior implementation — OPERATIONAL-PUSH-IMPLEMENT-1 (2026-09-26)

Active plan: `docs/product/COMMERCE_ALIGNMENT_E2E_PLAN.md`, **Phase 7 — Complete journeys and activation evidence**; focused owner: `docs/product/FULFILLMENT_DISPATCH_ALIGNMENT_PLAN.md`, **FDP-4 — Make new orders visible without manual page refresh**. Stable ID: `OPERATIONAL-PUSH-IMPLEMENT-1`. The owner authorized the event-driven Fulfillment/POS refresh design. Acceptance: a newly committed paid Order becomes visible without manual reload; material receiving, preparation, cancellation, issue and delivery changes made by another staff session revalidate the scoped queue; only newly detected paid Orders produce the new-order toast; idle visible tabs stop their eight-second D1 reads, with reconnect/focus and bounded failure recovery.

Observed `main` at starting HEAD `c8e2a9f844b82f601f322dec0472ee782d609320`; unrelated untracked `docs/freshmarkets-shopify-admin-plan.zip` is preserved. Local work adds migration `0103_operational_revisions.sql` with a transaction-coupled location revision, an authenticated Core WebSocket route and hibernatable location hub, post-commit publication and minute-job retry, a Web Worker upgrade bridge, and one visible-location refresh owner with 60-second disconnected fallback. A revision carries no customer facts or authorization. The owning FDP-4, architecture, API and data-model sections are updated. No remote D1 migration, live provider operation, production deployment or personal settings change has occurred.

Verification against starting HEAD plus the complete OPERATIONAL-PUSH-IMPLEMENT-1 working-tree scope: `pnpm check` passed exit 0, including formatting, naming, terminology, harness, migration, commit, architecture, readiness, lint, types, **1,733 Core tests/212 files**, **706 Web tests/160 files**, package tests and both builds (Core Wrangler dry run and Web vinext). `pnpm --filter @freshmarkets/web check:vinext` passed 16 supported/0 issues; `git diff --check` passed. Five focused Core files passed 58 tests after the dispatch guard correction, and the failed-read Web unit passed 5/5. The managed local two-browser journey and authenticated WebSocket bridge passed 2/2 on the same happy-path code; the later failed-read retry change was separately verified by unit test and aggregate. The first aggregate was stopped after exposing D1 `meta.changes` interaction with the dispatch trigger and a Core composition guard; both were corrected using `RETURNING id` and Core RPC-context publication before this final gate. These are local Worker/D1/browser results, not deployed-provider acceptance. Production request/row metrics and total-cost comparison remain unmeasured.

Post-push efficiency correction at observed `main`/`origin/main` `7c89e7fd`: the Web owner now skips both activity reads and stream connections when the selected staff context lacks `fulfillment.read` and `delivery.read`. Its focused six tests, all **707 Web tests/160 files**, Web typecheck, lint and vinext build passed on the follow-up working tree. Core and migration code did not change; the complete `pnpm check` result above remains evidence for the preceding implementation commit, with this narrower Web gate independently verified.

Completion level: **1 of 1 FDP-4 event-transport implementation slices locally complete**; its remote migration, deployment and live-site acceptance remain at the Phase 7 release level alongside earlier external gaps. The implementation and follow-up commits contain their respectively tested scopes. Next action after push, under a separately authorized rollout: apply `0103` to the target D1, deploy Core with its new Durable Object migration, deploy Web, then verify the live Fulfillment/POS stream and D1 usage. No actual payment or courier provider operation is needed for this local slice.

## Prior owner follow-up — OPERATIONAL-REFRESH-SCOPE-1 (2026-09-26)

Active plan: `docs/product/COMMERCE_ALIGNMENT_E2E_PLAN.md`, **Phase 7 — Complete journeys and activation evidence**, affecting `docs/product/FULFILLMENT_DISPATCH_ALIGNMENT_PLAN.md`, **FDP-4 — Make new orders visible without manual page refresh**. Stable ID: `OPERATIONAL-REFRESH-SCOPE-1`. The owner asked whether POS-style event-driven refresh should cover only newly paid orders. Acceptance for this design clarification: distinguish arrival notices from the changes needed to keep the shared operational queue and action availability current across staff.

Observed synchronized `main`/`origin/main` at `3ffa9ec0`; unrelated untracked ZIP preserved. Current POS renders the same Fulfillment workspace and the shared refresh owner already drives its queue revalidation. FDP-4 explicitly requires relevant progress changed by other staff in addition to new paid orders. Recommendation for owner review, **not implemented or approved policy**: only verified newly committed paid orders trigger the new-order toast. Location-scoped queue invalidation also covers changes that materially alter visible work or legal actions: committed payment, accepted cancellation/shortage resolution, Scheduled receiving readiness, preparation progress and dispatch/delivery state. Local commands update their own view immediately; cross-staff invalidation carries only an opaque revision, and the authoritative scoped Core read determines the resulting view. Avoid publishing unrelated catalog/analytics or every intermediate technical event. Batch nearby invalidations where safe; reconnect/focus reconciles missed changes. No source behavior, schema, production data, provider or deployment change occurred. Completion level: **1 of 1 event-scope design questions answered; 0 refresh changes implemented**. Next action on implementation authorization: enumerate exact Core write paths and test another staff session observing each material transition without a duplicate paid-order notice.

## Prior owner follow-up — OPERATIONAL-KV-COMPARISON-1 (2026-09-26)

Active plan: `docs/product/COMMERCE_ALIGNMENT_E2E_PLAN.md`, **Phase 7 — Complete journeys and activation evidence**, affecting `docs/product/FULFILLMENT_DISPATCH_ALIGNMENT_PLAN.md`, **FDP-4 — Make new orders visible without manual page refresh**. Stable ID: `OPERATIONAL-KV-COMPARISON-1`. The owner asked whether Workers KV would reduce D1 calls or the total bill and requested the strongest engineering recommendation. Acceptance for this read-only comparison: distinguish raw D1-call reduction from freshness, consistency, operational complexity and total metered work.

Observed synchronized `main`/`origin/main` at `6cc32ba7`, with unrelated untracked ZIP preserved. The current repo has no KV binding for this path; `ARCHITECTURE.md` reserves optional KV for workloads that tolerate stale data. Official Cloudflare KV documentation says cached values can be stale for 60 seconds or more across locations, missing keys can be cached, and one hot key is limited to one write per second. Polling a KV revision would remove the D1 activity query but retain Worker requests/KV reads, add a non-atomic D1-to-KV dual write and potentially delay POS changes; caching a queue would duplicate sensitive operational data and still need invalidation. Current KV and D1 pricing use different units/included allowances, so fewer D1 calls alone do not establish a lower bill. Recommendation for the owner's event-driven requirement remains one D1-authoritative revision row per location, direct hibernatable Durable Object invalidation, authoritative queue fetch only on actual change/open/reconnect and the existing minute recovery owner. Before claiming cheapest total cost or implementing, obtain real production `rows_read`/`rows_written`, request counts and active-tab/change rates, then compare the measured model against a simple indexed D1-revision poll as a lower-complexity baseline. No application, schema, production data, provider or deployment change occurred. Completion level: **1 of 1 KV alternatives reviewed; 0 refresh changes implemented**. Next action on an implementation request: measure the baseline and build the selected event-driven path with complete change coverage and failure reconciliation.

## Prior owner follow-up — OPERATIONAL-REVISION-D1-EFFICIENCY-1 (2026-09-26)

Active plan: `docs/product/COMMERCE_ALIGNMENT_E2E_PLAN.md`, **Phase 7 — Complete journeys and activation evidence**, affecting `docs/product/FULFILLMENT_DISPATCH_ALIGNMENT_PLAN.md`, **FDP-4 — Make new orders visible without manual page refresh**. Stable ID: `OPERATIONAL-REVISION-D1-EFFICIENCY-1`. The owner asked for the most D1-efficient event-driven refresh design. Acceptance for this design review: minimize idle D1 reads, storage and redundant queue re-reads without weakening cross-staff correctness.

Observed synchronized `main`/`origin/main` at `9c3969e4`; unrelated untracked ZIP preserved. The existing Fulfillment/POS queue is bounded and location scoped; migration `0102_operational_queue_indexes.sql` indexes its commitment sort and location/order lookup, but actual `rows_read` for production traffic has not been measured. Refined recommendation for owner review, **not implemented or approved policy**: use one `operational_revision` row per location instead of an append-only change log. Each relevant successful Core transaction increments its revision and marks it pending within the same D1 batch. Core publishes the latest opaque revision directly to a location-keyed hibernatable Durable Object and acknowledges publication on that row. The existing minute scheduler uses an indexed pending lookup to recover failed publications; multiple pending changes coalesce to the newest revision. A visible Fulfillment/POS page reads its current bounded queue only on a newer revision, initial open or reconnect/focus; hidden pages defer the read. No per-tab timer queries D1, no new Queue and no replicated order data in the hub. This trades one small revision update and a publication acknowledgment per actual change for a bounded global recovery lookup, subject to measured D1 row accounting. It must cover paid Order commitment, receiving, fulfillment and delivery effects and handle mixed-version release and authorization. No code, schema, live D1, provider or deployment change occurred. Completion level: **1 of 1 D1-efficiency design questions answered; 0 event-driven changes implemented**. Next action on an implementation request: capture current D1 `rows_read` for activity/queue paths, then build and validate the complete revision/push/recovery slice before replacing the eight-second refresh.

## Prior owner follow-up — OPERATIONAL-REFRESH-COST-1 (2026-09-26)

Active plan: `docs/product/COMMERCE_ALIGNMENT_E2E_PLAN.md`, **Phase 7 — Complete journeys and activation evidence**, affecting `docs/product/FULFILLMENT_DISPATCH_ALIGNMENT_PLAN.md`, **FDP-4 — Make new orders visible without manual page refresh**. Stable ID: `OPERATIONAL-REFRESH-COST-1`. The owner asked whether the proposed event-driven design still polls D1 and what Cloudflare resources it would bill. Acceptance: state idle/event/recovery reads separately and give a cost-conscious refinement without claiming a free or implemented system.

Observed synchronized `main`/`origin/main` at `0818f9d1`, with the unrelated untracked ZIP preserved. Current Core already runs a once-a-minute scheduled job, including D1 notification-outbox publication; its existing notification feed omits ordinary picking/packing changes. Refined proposal, **not an approved implementation rule**: write one indexed location-scoped operational change record atomically with each relevant Core transition; after commit, publish its opaque identity directly to a location-keyed hibernatable Durable Object WebSocket hub, avoiding a new Queue. The existing minute scheduler scans only unpublished change records as a failure-recovery path. Browsers fetch authoritative queues on initial open, received newer change, reconnect/focus or explicit refresh; they and the hub perform no timed D1 reads while idle. A bounded recovery D1 lookup still runs even when no browser is connected; removing it would weaken delivery after a post-commit publication failure. Indexed D1 rows read/written, Worker/Cron and Durable Object requests, active compute and storage remain metered; exact billing depends on actual usage and plan. Cloudflare's current official D1, Durable Object and Workers pricing pages were checked. No source behavior, remote state or provider operation changed. Completion level: **1 of 1 cost/recovery questions answered; 0 event-driven changes implemented**. Next action: if implementation is requested, measure current D1 row metrics, define complete event coverage, then build and verify the change record, hub, auth and recovery path before replacing the eight-second loop.

## Prior owner request — OPERATIONAL-REFRESH-ALTERNATIVE-1 (2026-09-26)

Active plan: `docs/product/COMMERCE_ALIGNMENT_E2E_PLAN.md`, **Phase 7 — Complete journeys and activation evidence**, with the affected initial transport in `docs/product/FULFILLMENT_DISPATCH_ALIGNMENT_PLAN.md`, **FDP-4 — Make new orders visible without manual page refresh**. Stable ID: `OPERATIONAL-REFRESH-ALTERNATIVE-1`. The owner challenged the eight-second Fulfillment/POS polling design and asked for an alternative or improvement. Acceptance for this read-only design review: identify the wasted work, distinguish a safe interim improvement from an event-driven replacement, and preserve Core authority and cross-staff freshness.

Started on synchronized `main`/`origin/main` at `cf669a4c`; unrelated untracked `docs/freshmarkets-shopify-admin-plan.zip` was preserved. Current Web increments its refresh revision on every successful eight-second activity read, so each mounted Fulfillment/POS view re-reads the full scoped queue even if the activity response did not change. The activity response's `latest` is only the newest bounded notification, not a revision for every fulfillment/receiving/delivery change; gating queue reads on that value alone would miss other staff updates. Approximately 450 activity and 450 queue reads occur per continuously visible location tab per hour before focus/reconnect traffic. This is a source-derived rate, not measured production traffic.

Recommended design for owner review, **not an approved implementation rule**: Core atomically records a location-scoped operational revision/intent with each relevant successful business transition; a recoverable publisher delivers opaque revision invalidations to a location-keyed hibernatable Durable Object WebSocket hub. The authenticated Web route checks current scope; the browser fetches authoritative Core queues only on a newer revision and reconciles on initial connect/reconnect/focus. Duplicates coalesce, missed push is repaired from durable revision, and socket failure shows stale status with explicit refresh or a separately chosen slow fallback. Push carries no customer/order payload and never authorizes a business write. A smaller interim option is to add the complete Core revision first and stop full queue re-reads when it is unchanged, retaining one lightweight poll; comparing only notification `latest` would be unsafe. Cloudflare's current official Durable Object WebSocket and Queue documentation supports hibernation and at-least-once delivery; cost, authorization expiry, all write paths and mixed-version deployment need implementation review. No code, test, deployment, provider transaction or remote write occurred. Completion level: **1 of 1 architecture alternatives reviewed; 0 refresh changes implemented**. Next action: on an explicit behavior-change request, define the durable revision coverage and implement/test the event-driven path across Core, Web and runtime bindings; wider Phase 7 acceptance remains open.

## Latest owner request — STOREFRONT-ANNOUNCEMENT-REVISION-1 (2026-09-26)

Active focused plan: `docs/product/STOREFRONT_ANNOUNCEMENT_POPUP_PLAN.md`, **Storefront announcement popup — plan and assets**, within `docs/product/COMMERCE_ALIGNMENT_E2E_PLAN.md`, **Phase 7 — Complete journeys and activation evidence**. The owner approved a revised scheduled-order announcement and requested it on every eligible home visit, one combined shopper/market image with a larger mid-thigh-cropped model and smaller softly blurred produce boxes, a gently floating grass mascot, and a shorter centered guest Sign in to checkout pill. Acceptance: approved headline/body/CTA without the old eyebrow or note; desktop/mobile image and dialog framing; every-home-visit display, dismissal and focus behavior; centered cart drawer and Cart page actions. No new delivery or checkout policy was requested.

Started on synchronized `main`/`origin/main` at `a87e104e` with only unrelated untracked `docs/freshmarkets-shopify-admin-plan.zip`, which was preserved. Source and focused specifications are committed at `a5eb8db5`; no local business data/schema or remote resource changed. Web now renders the popup on every eligible home opening, uses the combined `welcome-market-scene.webp`, approved static copy and the existing separate mascot with an 8px/3s vertical animation disabled under reduced motion. The shopper and branded tote dominate the revised scene; the produce boxes are small in the softly blurred right background. The popup retains its dedicated ordered-page shell for later pages. Guest checkout actions are capped at 240px and centered in the cart drawer and Cart page. Core's retired announcement-copy selector and home-contract field were removed; Core still owns actual available cycle, delivery window/fee and checkout admission.

Verification on the working tree before `a5eb8db5`: `pnpm check` passed, including contracts/Core/Web typechecks, static guards, both builds, Web **703/703** unit tests and Core Worker/D1 **1,731/1,731** tests. `pnpm --filter @freshmarkets/web check:vinext` reported 100% compatible. After the final centered-action wrapper and journey-test adjustment, `pnpm --filter @freshmarkets/web typecheck`, `pnpm format:check`, `pnpm lint` and `git diff --check` passed; lint retained two existing unrelated unused-variable warnings in the address-book test. Managed local Web/Core/D1 Chromium browser runs with `E2E_START_STACK=1` and disposable state passed popup **2/2**, first-visit/cart carryover **2/2** at 1440px and 390px, and guest cart drawer centering **1/1**. An initial browser run exposed that the always-show popup blocked an old test's immediate location click; its journey now closes the announcement first. The first centering assertion compared element bounds during the drawer slide at different times; the final assertion measures both in one frame and passed. Desktop 1440x900 and mobile 390x844 screenshots were visually inspected: the model/tote and smaller background boxes are legible, mascot sits at the image/content seam, and Close and CTA remain in view. The 240px Cart page pill was also asserted in both first-visit journeys. This local verification did not use an actual provider transaction or a production browser.

The owner then explicitly requested commit, push and deployment. `main` and `origin/main` were synchronized at `94e50986` before release; only the unrelated untracked ZIP remained. Pre-release Core `/health` and `/ready`, Web `/health` and `/api/core-health` returned HTTP 200 with production Core ready. Core/Web `wrangler types --check`, `vinext check` (100% compatible), production `CLOUDFLARE_ENV=production pnpm --filter @freshmarkets/web build`, `node scripts/verify-worker-readiness.mjs`, and strict production Core/Web Wrangler dry runs passed. The generated Web config targeted `freshmarkets-web-production`, `https://freshmarkets.ph` and `freshmarkets-core-production#CoreEntrypoint`; the combined scene and mascot were present in built assets. No migration or remote data reset was needed.

Production Core deployed first with `pnpm --filter @freshmarkets/core exec wrangler deploy --config wrangler.jsonc --env production`, version `df2efcf7-ba2f-4c7a-b018-cc10c4790126`. Core `/health` and `/ready` and Web `/api/core-health` remained HTTP 200 with ready/ok production status. Production Web deployed with `pnpm --filter @freshmarkets/web exec wrangler deploy --config dist/server/wrangler.json`, version `885910b0-0a4d-4c26-99eb-24b10c8bd66c` on `freshmarkets.ph`. Post-release Core health/readiness and Web-to-Core health returned HTTP 200; the new WebP scene and PNG mascot each returned HTTP 200 with the expected content type. Read-only Chromium tests against `https://freshmarkets.ph` passed **2/2** popup cases, including Escape, reopening after homepage refresh, the mobile action and button radius. Anonymous production screenshots at 1440x900 and 390x844 were visually inspected: the larger shopper/tote, smaller soft market boxes, separate mascot, copy, Close and CTA were all visible in the viewport. No production Cart action, authenticated Admin session, purchase, payment, courier operation, outbound message or remote D1 mutation was used in release verification.

Completion level: **1 of 1 requested source/asset revisions committed and pushed; 1 of 1 paired production releases deployed and publicly browser-checked; 0 actual provider transactions**. Next action: continue the outstanding Phase 7 commerce/provider acceptance at its own authorization and operational gates. The unrelated untracked ZIP remains preserved.

## Latest owner request — STOREFRONT-ANNOUNCEMENT-RELEASE-1 (2026-09-26)

Active focused plan: `docs/product/STOREFRONT_ANNOUNCEMENT_POPUP_PLAN.md`, **Storefront announcement popup — plan and assets**, within `docs/product/COMMERCE_ALIGNMENT_E2E_PLAN.md`, **Phase 7 — Complete journeys and activation evidence**. The owner explicitly requested deployment of the popup and rounded Storefront/Admin buttons. Release acceptance for this slice: build and deploy the contract-compatible Core then Web production Workers from the verified `main` source, confirm readiness and the Web-to-Core binding, observe the popup and both image assets on the live storefront at desktop/mobile widths, and verify dismissal and the button shape without a real purchase/provider operation.

Started on `main`/`origin/main` at `8f13159a` with only unrelated untracked `docs/freshmarkets-shopify-admin-plan.zip`, which was preserved. Compared the prior live source `9f920dc7` to HEAD: the intervening application changes are the popup Core/contract/Web slice, the revised full-body shopper and the button CSS; remaining intervening commits are documentation. No migration or remote data write was needed. Pre-release Core `/health` and `/ready` and Web `/api/core-health` were HTTP 200, with Core ready. The full `pnpm check` aggregate passed: Web **160 files/703 tests**, Core **212 files/1,732 tests**, package tests and static/type/build gates. Lint retained two unrelated unused-variable warnings in the address-book test. Core/Web `wrangler types --check`, `vinext check`, production `CLOUDFLARE_ENV=production pnpm --filter @freshmarkets/web build`, `node scripts/verify-worker-readiness.mjs`, and strict Core/Web production Wrangler deploy dry runs passed. The generated Web configuration targeted `freshmarkets-web-production`, `https://freshmarkets.ph`, and `freshmarkets-core-production#CoreEntrypoint`, with no nested environment. Both popup PNG assets were present in the production build. Earlier managed local Web/Core/D1 browser evidence for the popup and buttons remained 3/3.

Production Core was deployed first with `pnpm --filter @freshmarkets/core exec wrangler deploy --config wrangler.jsonc --env production`, version `39d4162f-a3fc-4402-9eff-f7904bead498`. Core `/health` and `/ready` and Web `/api/core-health` then returned HTTP 200 with ready/ok status. Production Web was deployed with `pnpm --filter @freshmarkets/web exec wrangler deploy --config dist/server/wrangler.json`, version `1b8056fe-2d9f-4b66-873d-72e60135edad` on `freshmarkets.ph`. Deployment history showed both new versions at 100% traffic. Post-release Core `/health`, `/ready`, Web `/api/core-health`, and both announcement PNG URLs returned HTTP 200. A read-only Chromium run against `https://freshmarkets.ph` passed **2/2** `announcement-popup.spec.ts` cases for first visit, image load/framing, Escape/revisit persistence, mobile action and computed storefront button radius. Anonymous production screenshots at 1440x900 and 390x844 were visually inspected: shopper head-to-toe, tote and mascot visible, dialog/CTA within the viewport. The live popup used the safe general wording because the current Core schedule choice did not qualify for the Monday–Friday/Sunday claim. Production Admin button appearance was inferred from the shipped shared CSS and locally exercised Admin journey; no authenticated production Admin visual session was used.

Completion level: **1 of 1 requested production releases deployed and public popup/browser checks passed; 0 actual payment/courier/provider transactions performed**. No production order, outbound message or remote D1 mutation was initiated by this release verification. Next action: verify Admin button appearance during a normal authorized operator session if desired; wider Phase 7 commerce/provider acceptance remains open.

## Latest owner request — STOREFRONT-ADMIN-BUTTON-RADIUS-1 (2026-09-26)

Active focused plan: `docs/product/STOREFRONT_ANNOUNCEMENT_POPUP_PLAN.md`, **Storefront announcement popup — plan and assets**, within `docs/product/COMMERCE_ALIGNMENT_E2E_PLAN.md`, **Phase 7 — Complete journeys and activation evidence**. The owner extended the pill shape to all buttons in Storefront and Admin and asked for a CSS change. Acceptance for this slice: both surfaces' buttons, including buttons with explicit corner classes, use a full pill radius; button-like links using the existing control token stay aligned; fields and non-button containers keep their shapes.

Started on `main` at `bcf0b322` with only unrelated untracked `docs/freshmarkets-shopify-admin-plan.zip`, which was preserved. `apps/web/app/globals.css` now applies `9999px` directly to Storefront/Admin buttons and scopes the existing `--fm-radius-control` token to buttons and links. A descendant selector covers the surfaces without `:has()`; a `body:has(...)` selector also reaches their dialog portals on browsers that support it. DESIGN and the focused popup plan record the owner's expanded button rule. No Core, contract, schema or business behavior changed.

Verified on the working tree: `pnpm --filter @freshmarkets/web typecheck`, `pnpm lint`, focused `pnpm exec oxfmt --check apps/web/app/globals.css`, `pnpm naming:check`, and `git diff --check` passed. Lint reported two pre-existing unused-variable warnings in `apps/web/test/app/account/addresses/address-book-client.test.tsx`. Managed local Web/Core/D1 Chromium Playwright with `E2E_START_STACK=1`, disposable `E2E_STATE_NAME=e2e-button-radius-20260926`, `--workers=1 --retries=0` passed **3/3**: initial administrator setup through Staff & Access, popup first visit/dismissal, and storefront action radius/mobile browse. The final descendant CSS fallback was added after that browser run; it does not change the supported-browser selector's output. No production deployment, remote D1 write, actual provider transaction or outbound message occurred.

The reviewed source is committed directly to `main` and pushed to `origin/main`; the pushed revision contains the locally checked CSS and guidance. Completion level: **1 of 1 requested button-radius source slices implemented and locally checked; 0 production releases requested or deployed**. Next action: pursue any separately authorized release and production visual check. Wider Phase 7 journey/provider acceptance remains open.

## Latest owner request — STOREFRONT-ANNOUNCEMENT-IMPLEMENT-1 (2026-09-26)

Active focused plan: `docs/product/STOREFRONT_ANNOUNCEMENT_POPUP_PLAN.md`, **Storefront announcement popup — plan and assets**, within `docs/product/COMMERCE_ALIGNMENT_E2E_PLAN.md`, **Phase 7 — Complete journeys and activation evidence**. The owner authorized implementing the X-style popup and clarified that the Grab-inspired storefront button radius needs a simple CSS change. Acceptance for this slice: welcome dialog before home browsing, complete transparent shopper and small mascot, accessible dismiss/shop path with campaign-revision persistence and later-page structure, accurate conditional Monday–Friday/Sunday copy, and pill-shaped storefront actions without changing Admin or fields.

Started on `main` at `27ae6145`; the unrelated untracked `docs/freshmarkets-shopify-admin-plan.zip` was preserved. Core now derives `announcementSchedule` from the current Global mode/state and all currently offered Open Scheduled cycles with active zones, using each cycle's local opening, cutoff and Sunday-only window. The shared home contract exposes this presentation choice, not a checkout decision. Web renders the first page from a separate versioned announcement module only on an unseen home visit. A first-party revision cookie suppresses it on reload. The X-style native dialog has a white brand/Close bar, soft-green image field containing the complete full-body shopper and grass mascot, short copy and one green pill CTA. It supports Escape, focus management and a future Next/Back page sequence. The generic copy appears unless the current Core facts support the literal weekly claim. CSS applies the existing radius token as a pill only on storefront buttons and links that use it; inputs, other containers and Admin keep their prior shapes. The focused plan, DESIGN and API contract document the implemented behavior.

Verified on this working tree: `pnpm typecheck`, `pnpm lint`, `pnpm naming:check`, `pnpm architecture:check`, `pnpm terminology:check`, focused `pnpm exec oxfmt --check` and `git diff --check` passed. Lint reported two pre-existing unused-variable warnings in `apps/web/test/app/account/addresses/address-book-client.test.tsx`. `pnpm --filter @freshmarkets/core test -- src/catalog/storefront-announcement.test.ts` passed **1/1** for local weekday eligibility. Managed Web/Core/D1 Playwright with `E2E_START_STACK=1`, disposable `E2E_STATE_NAME=e2e-announcement-final-20260926b`, `--workers=1 --retries=0` passed **2/2** in Chromium: first visit, image load and uncropped media bounds, Escape, reload dismissal, mobile CTA/scroll and computed storefront action radius. The earlier desktop/mobile screenshot review caught a clipped shopper; the final run uses the corrected `object-fit: contain` framing. Managed D1 showed the safe general wording; no live Scheduled Sunday cycle or actual provider transaction was exercised. No production deployment, remote D1 write, live order, outbound message or release verification occurred.

The reviewed source is committed directly to `main` and pushed to `origin/main`; the pushed revision contains the locally tested implementation. Completion level: **1 of 1 requested popup-and-radius source slices implemented and locally browser-verified; 0 production releases requested or deployed**. Next action: pursue any separately authorized release and production browse check. Wider Phase 7 journey/provider acceptance remains open.

## Latest owner request — STOREFRONT-ANNOUNCEMENT-STYLE-1 (2026-09-26)

Active focused plan: `docs/product/STOREFRONT_ANNOUNCEMENT_POPUP_PLAN.md`, **Storefront announcement popup — plan and assets**, within `docs/product/COMMERCE_ALIGNMENT_E2E_PLAN.md`, **Phase 7 — Complete journeys and activation evidence**. The owner supplied an X announcement-modal screenshot, asked to style the planned welcome popup after it, and asked whether Grab's rounded buttons should extend across the storefront. Follow-up asked to check Grab's own documentation. Acceptance for this planning slice: record the X hierarchy, a FreshMarkets popup CTA shape, and an evidence-based boundary for any wider button treatment.

Observed `main` at `b36d6b6c` with only unrelated untracked `docs/freshmarkets-shopify-admin-plan.zip`, which was preserved. The focused popup plan and DESIGN now adapt the X image-first vertical card with the full-body transparent shopper, small mascot and green pill action. Mobbin's Grab iOS order screens visually show prominent rounded order actions. Figma's published Grab Duxton account describes shared components with configurable corner radius for local/function needs, but gives no universal pill specification; Grab's published product experimentation and user-feedback accounts describe testing action placement and aligning buttons within a particular order flow. Sitewide button-radius change is a proposal, not implemented or accepted. Current `--fm-radius-control` is 4px and shared with other controls, so a broad change would need a storefront-specific action style and review across cart/checkout/dialogs/mobile.

Verification: inspected the supplied X screenshot, Mobbin Grab screen previews, official published Grab/Figma accounts, current storefront button component and CSS token; documentation diff checked with `git diff --check`. No popup UI or sitewide radius code changed, and no browser/provider/deployment acceptance occurred. Completion level: **1 of 1 reference-and-plan revisions prepared; 0 of 1 popup runtime implementations accepted; 0 sitewide button changes approved or implemented**. Next action: implement and browser-verify the X-style announcement from the focused plan, preserving cycle-accurate delivery copy; pursue a separate storefront button review only after a sitewide shape decision. Wider Phase 7 acceptance remains open.
...
Started on synchronized `main`/`origin/main` at `908fa4a4`; concurrent unrelated storefront popup plan/assets commit `4a9ff7f3` was preserved. Unrelated untracked `docs/freshmarkets-shopify-admin-plan.zip` was untouched. The Core cycle command, contracts and Web admin UI now support early close with current Global authorization, expected version, conditional D1 update, audit, immutable receipt and idempotent replay. PRODUCT, API, state and design guidance record the approved distinction between ordering status and customer cutoff. The live cycle has **not yet been changed**; no purchase, payment, receiving or stock command was executed.

Verification on the committed source: focused Core integration 48/48, local managed Web/Core/D1 browser early-close test 1/1, `pnpm typecheck`, naming, terminology, architecture, lint and `git diff --check` passed. Full `pnpm check` passed with Core Worker/D1 Vitest **1,731/1,731**, Web Vitest **703/703**, static checks, builds and typechecks; lint reported two existing unrelated unused-variable warnings. The browser test used a disposable cycle and local payment behavior, not actual provider acceptance. The source and initial checkpoint were committed as `9f920dc7` and pushed to `origin/main`; the unrelated untracked ZIP was excluded.

Production-target Web build and `node scripts/verify-worker-readiness.mjs` passed; generated config targeted `freshmarkets-web-production`, `freshmarkets.ph`, `ENVIRONMENT=production`, and `freshmarkets-core-production#CoreEntrypoint`, without nested environment. Strict Core and Web Wrangler dry runs passed with Wrangler 4.127.1. Core deployment version `6a294513-1623-478f-a7b2-05643b7859dd` and Web deployment version `363ba911-7079-4018-9b55-521ed6fc0f32` each reached 100% traffic. Web `/health` and `/api/core-health`, direct Core `/health` and `/ready` returned HTTP 200 with Core ready. No schema migration was needed.

At about 2026-09-26 02:13 Asia/Manila, authenticated production Global UI showed `Sunday delivery · Sep 27` Open, Central Cebu, with Sep 26 11:59 PM cutoff. The owner-authorized **Close ordering now** command succeeded. A fresh browser reload and re-open of that exact cycle showed `Cutoff Reached`, no close button and the unchanged `Sat, Sep 26, 11:59 PM` cutoff, with the early-close explanation. No Confirm purchase, receiving, stock, payment or provider transaction was invoked. The live payment reconciliation and post-cutoff receiving journey remain separate Phase 7 obligations. Completion level: **1 of 1 implementation slices locally verified, 1 of 1 paired releases deployed and healthy, and 1 of 1 specified live early-close commands executed and UI-verified**. Next action: after Sep 26 11:59 PM Asia/Manila, inspect pending payment resolution for this week, then use the normal purchase and receiving workflow only when its gates are satisfied.
...
Observed synchronized `main`/`origin/main` at `690373a3`; unrelated untracked `docs/freshmarkets-shopify-admin-plan.zip` was preserved. On 2026-09-26 at 01:18:24 Asia/Manila, the Sep 26 23:59 cutoff was still later that day. Authenticated read-only production Web inspection of the Sep 27 week showed the same cutoff, a pending-payment confirmation banner, and the Central Cebu Abiu row at 1 paid piece, `not purchased`, accepted 0, rejected 0. Source inspection found that Confirm purchase requires the cutoff to have passed, no unresolved Scheduled payment commitments, permitted scope and an eligible cycle/requirement state. Receiving actions require an `ORDERED` or partially received requirement, created by purchase confirmation. The planned purchase time is schedule context, not an automatic state transition.

Verification: local Manila time command and read-only `https://freshmarkets.ph/admin/procurement` UI; inspected `scheduled-week.ts`, `receiving-actions.ts`, weekly and receiving Web pages, `STATE_MACHINES.md` and `API_CONTRACTS.md`. No code behavior changed; no local tests, provider acceptance, live purchase/receiving command, stock mutation, deployment or remote D1 write occurred. Completion level: **1 of 1 reported receiving blockers diagnosed; 0 live receiving transitions attempted**. Next action: after Sep 26 23:59 Manila time, wait for payment confirmation to resolve, confirm the actual supplier purchase on the weekly row, then start receiving at Central Cebu; if the banner remains after reconciliation, inspect the pending payment state read-only before any intervention. Wider Phase 7 journey/provider acceptance remains open.

## Latest owner request — DELIVER-TO-FIRST-VISIT-INLINE-RELEASE-1 (2026-09-25)

Active plan: `docs/product/COMMERCE_ALIGNMENT_E2E_PLAN.md`, **Phase 7 — Complete journeys and activation evidence**. Stable ID: `DELIVER-TO-FIRST-VISIT-INLINE-RELEASE-1`. The owner authorized production deployment of the committed first-visit inline Deliver to prompt. Acceptance: production Web serves the clear Set delivery location action instead of auto-opening the dialog, keeps the prompt/button green consistent, preserves the existing production Core binding and health, and reaches 100% Web traffic.

Release source was synchronized `main`/`origin/main` at `a43f176088cec1dde077ef9e4ebfb88fb1515f8d`; only unrelated untracked `docs/freshmarkets-shopify-admin-plan.zip` was preserved. Before deployment, Web version `eb5300e3-6968-4abf-b6ce-285c26ae649b` and Core version `83d47789-2a8e-4c27-bed3-6b928b57991f` each had 100% traffic. Web `/health` and `/api/core-health` and direct Core `/health` and `/ready` returned HTTP 200; Core reported ready. Required production Web secret names were present without reading values.

`pnpm check` passed on the release source, including Web Vitest **703/703**, Core Worker/D1 Vitest **1,729/1,729**, packages, typechecks, static checks and builds; lint reported two unrelated existing unused-variable warnings. `CLOUDFLARE_ENV=production pnpm --filter @freshmarkets/web build`, `node scripts/verify-worker-readiness.mjs` and strict Web Wrangler deploy dry run passed with Wrangler 4.127.1. The generated config targeted `freshmarkets-web-production`, `ENVIRONMENT=production`, `https://freshmarkets.ph`, the `freshmarkets.ph` Custom Domain and `freshmarkets-core-production#CoreEntrypoint`, with no nested environment. The previous local desktop/mobile Worker/D1 browser evidence is under `DELIVER-TO-FIRST-VISIT-INLINE-1`.

Strict Web deployment with message `Deliver to inline prompt a43f1760` produced version `724b8b68-5321-4505-92f6-1e0ccf23cdd0` on `freshmarkets.ph` at 100% traffic. Core remained `83d47789-2a8e-4c27-bed3-6b928b57991f` at 100%. Post-release Web `/`, `/health`, `/api/core-health` and direct Core `/health`, `/ready` returned HTTP 200, with Core ready. Read-only fresh-browser production checks at 390x844 in Chromium 151, Firefox 153 and WebKit 26.5 found zero initial Deliver to dialogs, the visible inline prompt, matching `rgb(0, 177, 79)` prompt border/button, heading focus after clicking, and prompt persistence after Escape; no page errors occurred. Production desktop and mobile Chromium screenshots were visually inspected. Historical browser versions, live location confirmation and actual providers were not production-tested. No Core deployment, live order/stock command, provider transaction, remote D1 write/migration or outbound message occurred.

Completion level: **1 of 1 authorized Web releases deployed at 100% traffic and published first-visit prompt verified; 0 historical browser versions tested**. Next action: if an older-browser support target is required, identify exact engine/version and run that real browser; continue the wider Phase 7 journey/provider acceptance separately.

## Latest owner request — DELIVER-TO-FIRST-VISIT-INLINE-1 (2026-09-25)

Active plan: `docs/product/COMMERCE_ALIGNMENT_E2E_PLAN.md`, **Phase 7 — Complete journeys and activation evidence**. Stable ID: `DELIVER-TO-FIRST-VISIT-INLINE-1`. After reverting the first inline attempt, the owner clarified that a new visitor should receive a clear signal to click instead of an automatically opened green-outlined Deliver to popup. Acceptance: no automatic selector on first browse; a conspicuous inline prompt and green Set delivery location button below the header; the button and header control open the existing selector; dismissal leaves the prompt available; confirmed location removes it, and location-gated browsing remains safe.

Started on synchronized `main`/`origin/main` at `9f9938ff`; the unrelated untracked `docs/freshmarkets-shopify-admin-plan.zip` was preserved. Web now renders an inline Where should we deliver? prompt on the home, product and cart routes when no confirmed browsing-point cookie exists. The prompt explains local prices/availability and uses the same `--fm-storefront-action` green for its border, icon and Set delivery location button. Its event opens the existing Deliver to selector. Selection emits a local change event so the prompt disappears immediately after confirmation; route and focus checks refresh browser state. The selector no longer opens automatically, and its always-mounted heading receives initial focus so the entire popup is not green outlined. The existing serviceability, map/current-location/saved-address confirmation and Core write path are unchanged. DESIGN records the approved first-visit behavior.

The tested source was the working tree based on `9f9938ff`. Focused Web Vitest **15/15 passed** for the dialog, prompt and shell layout. Web typecheck, focused Oxlint, focused Oxfmt check, naming, terminology and `git diff --check` passed. Managed local Web/Core/D1 browser tests with `E2E_START_STACK=1`, existing disposable `E2E_STATE_NAME=e2e-inline-location`, `--workers=1 --retries=0` passed **6/6** prompt/confirmation cases at 1440x844 and 390x844 across installed Chromium, Firefox and WebKit. They verified no initial dialog, visible prompt, exact `rgb(0, 177, 79)` equality for prompt border/button, heading focus after click, confirmed address, prompt removal and reload persistence. The 390px first-visit cart/guest carryover journey passed **1/1**, including safe browsing without a location and recovery after confirmation. Local screenshots were inspected at both widths. The browser tests mock address resolution and are not actual Google/provider or historical-browser-version acceptance. No production release, live order/stock command, provider transaction, remote D1 write/migration or outbound message occurred.

Completion level: **1 of 1 requested source UI slices locally verified; 0 production releases requested or deployed; 0 historical browser versions tested**. Next action: review the pushed source on a separately authorized Web production release, then check a fresh visitor at desktop/mobile widths and any specifically targeted older browser versions; wider Phase 7 journey/provider acceptance remains open.

## Latest owner request — DELIVER-TO-MOBILE-LOCATION-ERROR-1 (2026-09-25)

Active plan: `docs/product/COMMERCE_ALIGNMENT_E2E_PLAN.md`, **Phase 7 — Complete journeys and activation evidence**. Stable ID: `DELIVER-TO-MOBILE-LOCATION-ERROR-1`. After the current-location production release, the owner reported that an Android tap showed “Location permission was not granted” without a visible permission prompt. Acceptance: diagnose what the published site can establish, report the browser error accurately, keep map recovery and explicit destination confirmation, and publish the focused Web correction under the existing deployment request.

Started on synchronized `main`/`origin/main` at `da71f0ac` with only unrelated untracked `docs/freshmarkets-shopify-admin-plan.zip` preserved. The published HTTPS response allowed self geolocation via `Permissions-Policy`. The existing tap called `navigator.geolocation.getCurrentPosition` directly, but its error callback ignored the error code and called every failure a permission denial. Web now distinguishes code 1 (access blocked), code 2 (position unavailable) and code 3 (timeout), with actionable device/browser guidance and map recovery. It never displays the browser's raw error message. Success still proceeds through reverse address lookup, Core serviceability, a visible pin and explicit Deliver here confirmation. No Core, contract, schema or provider integration changed. DESIGN records the error feedback rule. Android Chrome may omit a new prompt when site, browser-app or device location access is already blocked; a site cannot force the native prompt after that decision.

Focused AddressEditor Vitest **25/25 passed**, including success, denial, position unavailable, timeout and stale callback cases. Web typecheck, focused formatting, lint, naming, terminology and `git diff --check` passed; lint reported two unrelated existing unused-variable warnings in an address-book test. The reviewed source was committed as `a20f83e0` and pushed to `origin/main`. Production `CLOUDFLARE_ENV=production pnpm --filter @freshmarkets/web build`, `node scripts/verify-worker-readiness.mjs` and strict Web Wrangler deploy dry run passed with Wrangler 4.127.1. Generated config targeted `freshmarkets-web-production`, its `freshmarkets.ph` Custom Domain, `https://freshmarkets.ph` and `freshmarkets-core-production#CoreEntrypoint`, without a nested environment. Pre-release Web `/health` and `/api/core-health` were HTTP 200.

Strict Web deployment produced version `cec07e61-1cba-4664-aeb0-f555f90580c6` on `freshmarkets.ph` at 100% traffic; Core was not deployed. Post-release Web `/`, `/health` and `/api/core-health` returned HTTP 200. A read-only production Chromium check at 390x844 with location permission denied showed the blocked-access guidance and map recovery in the still-open Deliver to dialog. This proves the published denied-permission feedback, not the actual cause on the owner's Android device or a physical permission sheet. No live order/stock command, provider transaction, remote D1 write/migration or outbound message occurred.

Completion level at the initial correction: **1 of 1 focused Web corrections deployed and production denied-permission feedback verified**. The owner later identified the phone setting; see the follow-up below. Wider Phase 7 journey/provider acceptance remains open.

Owner follow-up: the owner confirmed that device Location was turned off. The blocked-access message now explicitly says to turn on device Location before retrying, alongside site and browser permission guidance. This identifies the reported phone setting but does not prove a complete live pin/confirmation journey. Focused AddressEditor Vitest **25/25**, Web typecheck, focused formatting and `git diff --check` passed for this copy change. It was committed as `d8a56ae2` and pushed to `origin/main`. Concurrent unfinished Deliver to edits in the shared checkout were excluded by building an isolated worktree at exactly `d8a56ae2`. `pnpm install --frozen-lockfile`, production Web build, readiness verifier and strict Wrangler dry run passed there; generated config retained `freshmarkets-web-production`, `freshmarkets.ph`, `https://freshmarkets.ph`, and `freshmarkets-core-production#CoreEntrypoint`. An initial isolated build lacked local build inputs and was discarded; the successful rebuild used temporary local inputs, which were removed from the isolated checkout after verification without printing or committing their values.

Strict Web deployment produced version `eb5300e3-6968-4abf-b6ce-285c26ae649b` at 100% traffic; Core was unchanged. Post-release Web `/`, `/health` and `/api/core-health` returned HTTP 200. A read-only production Chromium check at 390x844 with location permission denied found the new device Location guidance, site/browser permission guidance and map recovery. No live order/stock command, provider transaction, remote D1 write/migration or outbound message occurred. Completion level: **1 of 1 owner-reported device causes identified and reflected in the deployed recovery copy; 0 physical-device end-to-end confirmations performed**. Next action: the owner can turn on device Location and retry Use current location on Android, then confirm the entrance pin with Deliver here; wider Phase 7 provider and journey acceptance remains open.
...
Observed `main` at `da71f0ac`; unrelated untracked `docs/freshmarkets-shopify-admin-plan.zip` and concurrent partial edits in `apps/web/components/storefront/address/address-editor.tsx`, its test, and `docs/design/DESIGN.md` were preserved. Source inspection found the popup is deliberately opened on `/`, `/products/*`, and `/cart` if no browsing-point cookie and no per-tab dismissal marker exist. This condition is independent of browser version; a separate browser/profile starts without the site's storage. The native `showModal()` focuses the dialog itself while lazy address content loads, and the global `:focus-visible` rule draws its 3px green outline around the popup. A read-only production browser visit at 390x844 reproduced the open dialog and green dialog outline in installed Chromium 151, Firefox 153, and WebKit 26.5; each had no page errors. No historical browser executable was tested. Vite 8's documented default production range is Chrome/Edge 111+, Firefox 114+, Safari 16.4+; older engines can have unrelated script compatibility failures, and Firefox before 98 or Safari before 15.4 lacks native `showModal()`. The dialog also uses `100dvh`, which older engines can ignore. Brave desktop/Android shares Chromium's engine; iOS Brave uses WebKit.

Suggested source change, not yet authorized as an implementation request: keep the first-visit location requirement if intended, place an always-mounted focus target such as the popup heading or a visible Close button before lazy content, focus it when opening, and avoid drawing the global focus outline on the dialog container while retaining visible focus for keyboard controls. Add an explicit Close action and a `vh` height fallback before `dvh`. Set and test an explicit minimum supported-browser policy before adding legacy bundles/polyfills; the current Vite target is not a promise of support below its range. Verify first visit, dismiss/reload, keyboard focus and selected-location visits in supported Chromium, Firefox and WebKit, and test any promised older versions on real engines. No source code, production deployment, provider transaction or remote data change was performed for this investigation.

Completion level: **1 of 1 diagnosis and recommendation delivered; 0 historical-browser version tests and 0 fixes implemented**. Next action: if the owner requests the fix, implement the popup focus/close/fallback changes and run browser acceptance at the chosen supported versions.

## Latest owner request — DELIVER-TO-CURRENT-LOCATION-RELEASE-1 (2026-09-25)

Active plan: `docs/product/COMMERCE_ALIGNMENT_E2E_PLAN.md`, **Phase 7 — Complete journeys and activation evidence**. Stable ID: `DELIVER-TO-CURRENT-LOCATION-RELEASE-1`. The owner requested production deployment of the pushed Deliver to current-location action, then clarified that iOS and Android should ask for location permission. Acceptance: the production Web release serves the action above Choose map, Web/Core stay healthy with the new Web version at 100% traffic, and the tap requests browser/device location permission before any coordinate is used.

Release source was synchronized `main`/`origin/main` at `e76f0a8bcf65e91fca48f56f018d0df69b6dc132`; only unrelated untracked `docs/freshmarkets-shopify-admin-plan.zip` was preserved. Previous production Web version was `b63fb633-d93a-44ff-9c32-840f7d563973` and Core was `83d47789-2a8e-4c27-bed3-6b928b57991f`, both at 100%. This Web release also carries the already pushed Sonner presentation slice from `0c74664e`; no Core code, schema or provider integration changed. Pre-release Web `/health` and `/api/core-health`, direct Core `/health` and `/ready` returned HTTP 200, with Core ready. Required Web/Core production secret names were present; values were not read.

`CLOUDFLARE_ENV=production pnpm --filter @freshmarkets/web build`, `node scripts/verify-worker-readiness.mjs` and `pnpm --filter @freshmarkets/web exec wrangler deploy --config dist/server/wrangler.json --dry-run --strict` passed with Wrangler 4.127.1. Generated config targeted `freshmarkets-web-production`, the `freshmarkets.ph` Custom Domain, canonical `https://freshmarkets.ph`, and `freshmarkets-core-production#CoreEntrypoint`, without a nested environment. Strict Web deployment with message `Deliver to current location e76f0a8b` produced version `64603c50-57d6-460a-8034-e3c0ab63b6a5` on `freshmarkets.ph`. Deployment lists showed that Web version at 100% traffic and Core unchanged at 100%. Post-release Web `/`, `/health`, `/api/core-health` and direct Core `/health`, `/ready` returned HTTP 200; Core remained ready.

A read-only production mobile-viewport browser inspection at 390x844 found exactly one enabled Use current location action in the Deliver to dialog, before Choose map. The published HTTPS response returned `Permissions-Policy: camera=(), microphone=(), geolocation=(self), payment=(self)`. Source inspection confirms that the action calls `navigator.geolocation.getCurrentPosition` synchronously on tap, with no location request at page load; the browser requests permission when needed on iOS Safari and Android Chrome. Existing focused tests cover success, denial and explicit Deliver here confirmation. No new source change was needed for the owner's mobile-permission clarification. Physical iOS/Android permission sheets, live reverse geocoding/serviceability, and Sonner appearance were not production-tested. No live order/stock command, provider transaction, remote D1 write/migration or outbound message occurred.

Completion level: **1 of 1 authorized Web releases deployed, healthy and published-control verified; 0 physical-device permission journeys tested**. Next action: on a real iOS or Android device with fresh location permission, tap Use current location and verify the OS/browser prompt, address/pin and explicit Deliver here confirmation; separately verify Sonner success/error appearance and continue wider Phase 7 journey/provider acceptance.
...
## Earlier external blockers (historical)
...
The current next action is recorded at the top of this checkpoint. Staging Google OAuth has no
remaining action. Production instance and credential provisioning remains a separate future release
task.
