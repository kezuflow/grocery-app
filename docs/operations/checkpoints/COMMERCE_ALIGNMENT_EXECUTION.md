...
**CA-7.8 — Customer account access and contact completion** is the sole active implementation task. Source: [COMMERCE_ALIGNMENT_E2E_PLAN.md](../../product/COMMERCE_ALIGNMENT_E2E_PLAN.md), **Phase 7 — Complete journeys and activation evidence**, section E and ENGINEERING transaction requirements.

Acceptance: account links expose the owner-supplied support address, existing password recovery and sign-out. Network errors remain visible/retryable; successful password reset invalidates the one-use token and old credentials, and sign-out invalidates the session. Verify real Better Auth Worker/D1 behavior and desktop/mobile access flows. No outbound messages or actual email-provider acceptance.
...
- Branch `main`; observed HEAD and origin/main `4ba6f214`, CA-7.7 committed/pushed. CA-7.8 starts from this revision. Preserve all excluded owner changes.
...
Earlier obligations preserved: actual permanent-geocoding account/storage acceptance and the eligibility of retained pre-fix provider-derived records remain external; no existing saved records were rewritten. CA-7.6 delivered name/account phone/default address/deactivation; address create/edit safeguards passed CA-7.7. Account recovery and customer-launch/storefront/Admin mocks with retired membership/default-location expectations still need acceptance. Earlier phase/provider obligations remain below.
...
| CA-7.8 | Active: customer recovery/sign-out and approved support contact. Customer-facing closure deferred by owner. |
...
**One next action:** commit/push CA-7.8, then align retired membership/default-location/minimum expectations and reconcile final phase coverage. Earlier obligations remain assigned.

CA-7.8 locally verified on working tree based on 4ba6f214: auth 5/1, static/types/Web build and desktop/mobile account-access browser 2/2 passed. First browser exposed the existing sign-out empty POST returning 415; JSON request fixed it and both widths passed. Account closure draft removed per owner. No schema/provider/outbound effects. Exact commands/scope in history; no process remains. Commit/push follows.
