...
- Branch `main`; observed HEAD `ae869ce2`. Current CA-7 Problems draft spans Core/contracts/Web and owning specs; exact scope is below. Preserve all unrelated/protected changes outside commits.
...
**CA-7 / Phase 7 / GD-D11 Problems draft on ae869ce2:** optional affected items; new customer reports require delivered status; report replay preserves the original response after handling; selected lines and report insertion are guarded. Existing Admin pages/nav say Problems and show New / Being handled / Resolved. Only CLAIM/RESOLVE remain callable; retained INVESTIGATING/ESCALATED rows remain visible in Being handled and resolvable. Recipient phone joins existing customer email and Order links. Handling revalidates Global orders.manage and commits report/audit/frozen result together; legacy reference-only receipts remain distinguishable. No automatic refund, payment or delivery change. Customer browser retains the exact request/key after a lost response.

Partial files: Core admin/application/{finance-commands,finance-reads,order-issue-policy,get-admin-context,admin-finance.integration.test,order-issue-policy.test}.ts; Core index.ts and orders/{domain/order-issue{,.test},application/{get-customer-order-detail,list-customer-order-issues{,.integration.test},submit-customer-order-issue{,.integration.test}}}.ts; contracts admin-finance{,.test}.ts; existing Admin issues pages/actions route, customer Order page, issue-status badge, order-issue-form{,.test}.tsx and Instant browser journey; API_CONTRACTS/DATA_MODEL/STATE_MACHINES plus this checkpoint. No migration or new operator workspace.
...
**One next action:** commit/push the verified CA-7 Problems scope, then implement GD-D19 notifications from durable facts: pickup/delivery customer updates and scoped dashboard notices, with Problems restricted to administrators. The current projection checks obsolete DISPATCHED and can miss intermediate delivery states; no dashboard notification read was found in the Admin context. Preserve Queue/attempt/unknown-outcome safeguards. No provider/account acceptance or outbound message is claimed.
