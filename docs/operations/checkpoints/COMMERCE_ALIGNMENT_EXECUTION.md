...
Focused AddressEditor Vitest **25/25 passed**, including success, denial, position unavailable, timeout and stale callback cases. Web typecheck, focused formatting, lint, naming, terminology and `git diff --check` passed; lint reported two unrelated existing unused-variable warnings in an address-book test. This is source and simulated-browser-error evidence, not a physical Android diagnosis. Production release and device/browser permission confirmation remain to be recorded after deployment. No live order/stock command, provider transaction, remote D1 write/migration or outbound message occurred.

Completion level: **1 of 1 focused source corrections verified; production correction and physical Android cause remain open**. Next action: review and commit the diff, build and deploy the Web correction, verify production denied-location feedback and health, then record release evidence; use the owner's Android browser/site-permission details if provided to narrow the device cause.
