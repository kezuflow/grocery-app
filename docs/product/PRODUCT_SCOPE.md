...
- Admin Overview, Customers, Orders, Products, Inventory, Promotions, Memberships, Payments, Delivery, Analytics, and Staff & Access primary workspaces using purpose-built Core commands/read models. Categories live under Products; the default Inventory flow is dated stock in/out. Procurement, Receiving, and Fulfillment remain contextual advanced workflows over their independent Core state machines.
...
37. Admin product lists resolve price/availability in an explicit authorized market/location context, report authoritative catalog-readiness totals, and load protected product media only through a same-origin Web adapter over a Core-authorized content read; R2 keys never reach Web or browser contracts.
