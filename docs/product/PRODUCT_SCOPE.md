...
Browse marketplace
 ...
 -> use Instant pay-as-you-go or activate the one-month promotional trial/paid membership for Scheduled
 -> build cart
 -> choose a valid Cebu serviceable address
 -> pass global selling OPEN and receive the globally active Instant courier choices or Scheduled window
 -> pay
 ...
 -> pick/pack
 -> dispatch through an enabled external courier
 -> completed order
```
...
- Instant checkout lists enabled, currently quoteable external delivery partners after address/serviceability and lets the customer select an opaque provider offer. Scheduled checkout lists only the delivery window while Lalamove supplies the future quotation; scoped store operations later choose an enabled external provider and immediate or supported scheduled pickup.
...
- Separate provider-neutral Payments boundary for membership and grocery payments. Development uses PayMongo sandbox; application-accessible mock providers/simulation flows are excluded. Deterministic fakes remain test-only. The PayMongo adapter provides signed event ingestion, canonical state translation, idempotent provider resources, settlement-cost evidence, and reconciliation.
...
- Admin Overview, Customers, Orders, Products, Inventory, Promotions, Payments, Delivery, Analytics, and Staff & Access primary workspaces using purpose-built Core commands/read models. The selector presents Global and Central Cebu for the current release. Global Products owns identity, Categories, media, and Sell variants only; Central Cebu Operations contains Products, Inventory, and external Delivery, with Products focused on manually managed exact local retail price, local selling status, and the shared Product inventory pool. No sourcing selector, price fallback, Rider/batch/internal-map navigation, or internal delivery-fee configuration is exposed.
...
- A global `/admin/commerce-configuration` workspace for Membership Price plus separate selling-state and fulfillment-mode controls. It never exposes Service Fee configuration, arbitrary history editing, or committed-snapshot mutation.
...
- Time-driven execution via Cloudflare Cron Triggers dispatching an explicit scheduled-job registry of idempotent Core commands (checkout/payment-hold expiry, trial expiry, scheduled membership cancellation, provider-inbox/reconciliation redrive, provider-action expiry, cycle cutoff/advancement/closeout, and notification outbox redrive). PayMongo—not Cron—owns paid subscription invoice and retry timing.
...
4. Core permits authenticated Instant checkout without membership and requires eligible membership for Scheduled. It rejects checkout when selling is paused, for a mode-specific entitlement failure, non-serviceable coordinate, unavailable mode/option/provider quotation, invalid or expired Instant hold, invalid/closed/cutoff Scheduled window, invalid exact-location price, unavailable SKU, Promotion conflict/ineligibility, or below-minimum basket.
...
26. Cart prices do not lock price or inventory. Before payment, Core resolves replay first and revalidates selling state, exact price, discount, Instant stock/hold or Scheduled window/cutoff, serviceability, provider quotation, and mode-specific membership. A changed total or quotation creates no payment until explicit acceptance.
27. Delivery price comes from verified external-provider quotation evidence. Instant lets the customer choose an enabled quote; Scheduled uses a Lalamove future quote while showing only the window. The Order snapshots the accepted quote, final courier payable/variance is separate, and the customer is never charged again.
...
31. Customer checkout lists opaque Core-routed options only after a confirmed serviceable address and nonempty current cart. Instant lists enabled external partners and snapshots the selected partner/service without fallback; Scheduled lists only a cycle/window with Lalamove pricing and leaves final external-provider/pickup choice to scoped store operations. Stale address/cart/provider/quotation/cycle evidence fails closed and no customer selects a hub.
...
40. Active delivery is external-provider only and presents normalized progress without Rider assignment, batches, route planning, or a custom driver map. Internal `BAG`/`BOX` classification is not sent to Lalamove; rejected package/handling enrichments and store pickup instructions remain outside recipient remarks.
