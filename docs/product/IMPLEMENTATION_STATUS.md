...
Remaining Phase 1 work includes complete provider normalization/shared webhook-refresh-inbox recovery, concurrent packing coverage, Scheduled goods separation and purchase reachability, and bounded end-to-end paid-operation recovery. Webhook retry tests do not establish an automatic reconciliation scheduler or refresh parity; those paths remain open. Later-phase dependencies must be reconciled before accepting the phase. Only fresh Worker/verifier test databases are classified disposable; existing local and remote environments remain preserved.
