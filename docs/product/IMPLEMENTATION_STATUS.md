...
### 2026-09-08 — Location dispatch readiness and connected setup (in progress)

Global location readiness/promise commands and Web settings are implemented with current authority/prerequisite checks, location/readiness/geography versions, unstarted-Quote invalidation, required audit and frozen replay receipts in one D1 batch. New commerce in both modes requires dispatch readiness; Scheduled does not require an Instant minutes promise. Provider-confirmed Instant commitment ignores retired order-count settings and later readiness changes while preserving exact hold/reservation checks. No schema migration or retained database change.

Executed evidence: 48 paid-commitment/mode/Quote Worker tests, 45 affected commerce/payment/Quote tests, 12 new readiness Worker tests; desktop/mobile readiness recovery browser tests passed (2 tests, zero retries). A connected mobile UI journey created and activated a customer site, saved hours, pickup profile and service area, then enabled readiness and verified reload persistence (1 test, zero retries; screenshot inspected). Web 389, contracts 68, types/lint/format/naming/architecture/readiness, migrations, harness, Core dry-run build and vinext checks pass.

The full Core run exposed 21 failures: obsolete capacity expectations, fixtures without declared dispatch readiness, cascading clock/state expectations and a trailing-comment seed-loader defect. After fixture/test correction, all 84 tests in the eight affected files pass without removing schema assertions. A fresh aggregate gate remains pending. Staff-detail scope editing now uses named locations and validates loaded DTOs; its updated browser journey is running. No whole commerce phase or real provider/account acceptance is claimed.
Final setup-slice gate: `pnpm check` completed successfully (`readiness-check.log`): 1341 Core tests/181 files, 389 Web tests, 68 contract tests, shared tests, harness/migration/convention/type/lint checks and both builds. The new command-driven Worker journey also proves confirmed-address routing and local staff assignment to the newly configured Scheduled site without physical inventory. Desktop/mobile verified-invitee and staff-scope browser flows pass (2 tests, zero retries; mobile screenshot inspected). This saves the setup milestone; actual provider activation and the later catalog/media, warehouse, Scheduled operations, delivery/membership and final-journey acceptance remain open.
2026-09-09 audit and product-media slice (uncommitted): traced 48 main commits since 619df3c and preserved all local media changes. The execution checkpoint had stale pending/failure descriptions and was corrected; historical reports do not accept any whole commerce phase. Current catalog creation/updates still require transaction-time authority/effect review. Promotion media, warehouse transfers, full Scheduled operations, delivery alignment and membership retirement remain open; browser inspection confirms the cart still advertises the retired trial.

Product media now has durable one-object upload identity/digest, frozen replay receipts, published same-origin R2 reads, catalog/cart projection and bounded cleanup plus Global operator observation/discard/redrive. Migration 0084 preserves retained associations and queues inactive cleanup; no retained database was migrated/reset. Unknown storage responses never authorize blind replacement. Web freezes file/body/key and recovery intents. Recovery read failure/stale-response regressions were reproduced and fixed.

Executed evidence: corrected desktop/mobile managed media journeys passed two tests with zero retries (media-audit-browser.log), through real Web/Core/D1/local R2 upload, lost-response retry, Global exact-location price, local activation, cart and anonymous rendering, removal and conditional-public-read rejection. Cart population uses the real HTTP command, not the add-to-cart UI; this is not checkout/provider acceptance. Mobile cart/recovery images inspected. Latest focused media Worker run passed 17 tests including historical minimal-audit receipt recovery (media-audit-worker.log); prior affected media/cart/catalog run passed 37, Web suite 394, current recovery-panel tests 2, contracts 68 and workspace types pass. Current aggregate gate is pending. No actual R2 account, OAuth, email, payment or Lalamove acceptance is inferred.
Final media-slice aggregate passed: Core 1358 tests/182 files (342.55s), Web 396/98, contracts 68/19, shared/harness/migrations/conventions/types/lint and both builds (media-audit-check.log). Vinext and catalog-generation checks pass. Final 17-test media Worker recheck passed after using the real fixture auth identity in historical audit evidence. No whole phase or actual provider acceptance claimed. Continue catalog transaction repairs before promotion publication.

2026-09-09 category command recovery verified after pushed media commit dc74065. Six regression failures reproduced ignored required category/audit/receipt effects, current-scope revocation, lost batch response and mutable replay. New category commands guard current Global authority, legal hierarchy/version, all effects and frozen receipts atomically; interrupted unapplied historical claims can recover, while already-applied legacy reference receipts require history review. Shared schemas align Core and bounded Web adapters. Category forms freeze the complete request through unknown responses, validate DTOs and expose paginated parent choices with failure/stale-read handling.
Validation: 29 focused Worker/D1 tests/2 files pass, including reachable create/edit/status, interrupted claim and concurrent identical retry. Web 398 tests/99 files, workspace types/lint/architecture/readiness/naming, Core dry-run build and managed Web build pass. Desktop/mobile real Web/Core/D1 category creation, edit and deactivation each recover a lost response with the identical body/key (2 browser tests, zero retries, category-browser.log); mobile screenshot inspected. Last full aggregate is the preceding media slice, not this category repair. No schema/RPC signature change, retained DB migration/reset, provider operation or whole-phase acceptance. Next: Product/variant/local activation correctness and recovery before promotion images and later authorized phases.
### 2026-09-09 — Product command recovery audit repair

Product create/edit/status now guard current Global catalog authority, mutable creation eligibility, the complete Product/pool/detail/audit effect set and original command result atomically. Shared validated Web boundaries retain complete saved requests, including after scope refresh; product category choices paginate and setup reads expose errors/retry. No schema or RPC signature change.

Evidence: 12 Product Worker/D1 tests pass; affected Category/catalog/media run had 57 passing tests plus one new test assertion using the wrong audit column, corrected and passing in that Product rerun. Web 398/99, workspace types/lint, conventions/architecture/readiness/format and Core dry-run build pass. Four executed desktop/mobile Category/Product creation/edit/status browser journeys pass with zero retries, preserving body/key after lost responses; mobile Product screenshot inspected. Logs: product-worker-final.log, product-category-worker.log, product-web-tests.log, product-category-browser.log and product-core-build.log in the task artifact directory. Last aggregate predates this repair; SKU/unit/local activation/price audit repairs, promotion media and later commerce acceptance remain open. No retained database migration/reset or actual provider acceptance.
### 2026-09-09 — Owner-directed removal of image-recovery feature

Removed the separate product image-recovery panel, Web route, Core operator methods and shared recovery DTOs/validators. Image administration remains ordinary upload/preview/update/remove with normal progress/errors; the upload button silently repeats the same request once after a transport failure. Kept internal publication integrity, duplicate prevention and bounded background deletion; removed only tests for the retired operator actions. No schema change or retained database reset/migration.

Validation: 35 focused Worker tests, 396 Web tests, 68 contract tests, workspace types/lint, naming/architecture/readiness and Core dry-run build pass. Two executed managed desktop/mobile browser journeys pass with zero retries, covering upload with lost-response handling, storefront/cart rendering and removal. Mobile screenshot inspected. First attempt exited during stack startup before tests; diagnostic rerun passed. Logs: image-simplification-worker.log, image-simplification-web.log, image-simplification-contracts.log, image-simplification-browser-debug.log and image-simplification-core-build.log in the task artifact directory. Broader implementation is paused for owner review of the 12 workflows; incomplete Unit/SKU transaction work remains preserved separately. No whole-phase acceptance or actual provider acceptance.

### 2026-09-09 — Resumed workflow agreements and new-chat handoff (unfinished)

Saved main is 4b611c5, including removal of image-recovery administration. Owner authorized ordinary CRUD simplifications and continued all remaining commerce work, then requested a handoff to a new Astra High chat. Latest refund decision: eligible Instant-before-acceptance and Scheduled-before-cutoff cancellations initiate automatic PayMongo refunds; staff-reviewed post-delivery refunds are issued from FreshMarkets dashboard through PayMongo, not restricted to PayMongo dashboard. Keep detailed financial diagnostics and mandatory transaction notifications.

Uncommitted changes align canonical policies, Scheduled cutoff cancellation, payment evidence at staff acceptance, fulfillment labels and refund-processing copy. Focused Worker verification executed 85 tests: 84 pass, one existing Scheduled OUT_FOR_DELIVERY-before-cutoff rejection expectation conflicts with the changed policy and remains unresolved. No new Web/browser/provider/build/aggregate acceptance. Preserved Unit/SKU transaction repair still contains eleven known failing regressions. No schema or retained-data mutation and no live provider action. No new commit or phase acceptance. Read the active handoff at docs/operations/checkpoints/COMMERCE_ALIGNMENT_EXECUTION.md before resuming; it supersedes older pause/status entries.


### 2026-09-09 - Cancellation and catalog transaction repair verified

Scheduled customer cancellation uses its snapshotted cutoff even after real staff acceptance; Instant acceptance requires matching successful payment evidence and closes customer cancellation. Refund initiation remains processing until provider confirmation. PayMongo uses the reason supported by both official guides; runbook now covers one-time commerce and staff refunds instead of retired subscriptions.

Unit, Variant, availability and Global exact-location price commands now atomically recheck scope and mutable inputs, require every write/audit/receipt, and replay frozen target-specific results. Distinct location prices allocate versions within their transaction. Bounded shared Web validation, exact variant quantities, named price choices, Save price effective-now and secondary scheduling/history complete this repair. Eleven original regressions reproduced; expanded 21-effect/race/replay suite passes.

Validation: pnpm check passed Core 1405/185, Web 396/98, contracts 68/19, shared/harness/migrations/conventions/lint/types and both builds. Catalog generation and vinext checks pass. Seven initial browser checks passed; two new variant cases had an incorrect final UI locator, corrected and passing desktop/mobile with zero retries. Real local media, variant/price/activation and staff refund paths are covered; customer cancellation messaging and price presentation also have controlled-DTO browser evidence. Variant screenshots inspected. Logs: commerce-check.log, commerce-browser.log, catalog-variants-browser.log under the current checkpoint artifact directory. No schema/retained-data mutation, deployment or actual provider acceptance; no whole commerce phase accepted. Continue promotion media/authoring, warehouse, Scheduled operations, delivery, membership removal and complete acceptance.
...
### 2026-09-09 - Customer-aware promotion preview and selection

Admin selects Customers by email/phone for preview and grants. Optional customer preview requires Global customer-read access and uses checkout audience/grant/usage policy without consuming redemptions; amount-only estimates explicitly do not assert customer eligibility. Grants recheck matching active commerce-principal access inside the complete transaction. Verified 66 focused Core tests, 400 Web, 68 contracts, types/lint/conventions/static guards and both builds. Nine real promotion browser journeys pass with zero retries, including mobile named selection and a lost grant response. Audience authoring/campaign media and later commerce acceptance remain open. No new schema, retained database reset or actual provider operation.


### 2026-09-09 - Controlled promotion audience authoring

Added draft audience GET/PATCH and Core RPCs for the five approved condition types, named Customer/segment selection, conjunctive matching and complete transactional replacement with immutable replay. Active customer/commerce-principal and segment references are checked inside the command transaction. Malformed/retired rules fail closed and cannot be activated without explicit draft replacement. No schema change. Verified 99 focused Core tests, 13 final audience tests, 400 Web and 68 contracts; types/lint/conventions/static guards, builds and binding freshness pass. Nine browser journeys pass with zero retries, including lost-response audience save and minimum/target-aware preview. Actual Instant Quote receives the authored campaign claim/version. Campaign media and later commerce acceptance remain unfinished; no whole phase or actual provider acceptance claimed.
