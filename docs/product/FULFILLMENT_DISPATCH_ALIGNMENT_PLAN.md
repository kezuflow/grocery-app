...
The owner's 2026-09-26 follow-up supersedes the initial 5–10-second polling transport. Use a D1-backed location revision committed with relevant operational writes, then publish that opaque revision to a location-keyed hibernatable Durable Object WebSocket hub. A shared visible Admin owner revalidates on newer revision, initial open, focus/reconnect and local commands. The existing minute scheduler retries unpublished revisions; a disconnected visible tab uses a slower recovery read and displays stale status. The hub never owns an Order, action decision or customer payload. Workers KV is not a live operations signal.

Use one shared refresh owner per selected scope rather than separate loops in the bell, Orders and Delivery. Only verified newly committed paid Orders cause a new-order toast. The queue also refreshes for other staff's material preparation, receiving, cancellation, issue and dispatch/delivery progress, without repeatedly loading the entire analytics overview. Preserve previous data during revalidation; cancel/ignore stale responses after account/location changes; use backoff and a visible stale/error indication when offline.
