...
Status: implementation resumed after the 2026-09-09 owner workflow review. Normal CRUD simplification, automatic eligible customer refunds and FreshMarkets dashboard execution through PayMongo of reviewed post-delivery refunds are authorized. Phase 3 has local implementation/acceptance evidence; Phase 4 is in progress. Earlier Phase 0–2 acceptance obligations and provider/release evidence remain open. Use the [active execution checkpoint](../operations/checkpoints/COMMERCE_ALIGNMENT_EXECUTION.md) for the reconciled task ledger, tested revisions and next action, and follow the saved plan in dependency order. The historical plan below does not override current canonical policy or owner decisions.
...
- Customers with nested Memberships, plus Promotions, Inventory, Delivery, Operational Exceptions, Analytics, Staff, Roles, Audit Log, and Fulfillment Mode Settings list/detail/configuration flows as applicable. The privacy/account-closure schema, contracts, lifecycle, and audit evidence remain preserved, but the standalone Privacy Requests page and customer-detail controls are deferred until an owner-approved operational and legal procedure exists. Inventory uses explicit Add stock/Remove stock actions with dated ledger history; Procurement, Receiving, and Fulfillment remain contextual advanced routes rather than primary navigation.
