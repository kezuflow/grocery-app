...
FreshMarkets uses DoorDash as a usability reference only. The brand, copy, product photography, icons, colors, and implementation remain original. The storefront is a grocery marketplace with scheduled Cebu delivery and subscription-gated checkout, not an instant restaurant marketplace.
...
- [DoorDash iOS cart](https://mobbin.com/screens/ef91d789-3440-4e08-9b48-1bceba6855ed) uses a bottom-anchored Continue CTA, line-item steppers, and optional recommendations. FreshMarkets uses a full-screen cart/bottom-sheet variant with minimum-order and cycle context.
- [DoorDash iOS checkout](https://mobbin.com/screens/85e47483-f648-40c3-8d89-ec4e77240217) separates map/address, delivery time, instructions, cart summary, and the next action into a single-column progression. FreshMarkets substitutes scheduled cycles and subscription eligibility.
...
Reject or change: DoorDash red branding and logos; restaurant/store marketplace switching; instant ETA promises; pickup and customer hub selection; restaurant ratings/tips; arbitrary meal customizations; free-merchandise implications from membership; generic cross-store carts; and freely editable paid orders. FreshMarkets must preserve fixed sellable variants, subscription eligibility, serviceability, cycle cutoff/capacity, Core pricing, and locked paid orders with additive amendments only.
...
6. Build address picker and delivery-cycle presentation using existing serviceability/cycle contracts; add cutoff/full/error states.
...
- Notifications are deferred from the immediate implementation phase.
