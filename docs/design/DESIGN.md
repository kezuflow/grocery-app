...
Owner correction, 2026-09-13, retained under the 2026-09-24 Shopify presentation:
Locations and Service Areas are distinct destinations under pinned Settings, visible in Global and
selected-location navigation when Core authorizes Locations. The Settings grouping does not
require an additional Settings capability.
...
Delivery weeks appears under Orders for authorized Global and selected-location views; Receiving appears under Products only for an authorized selected location. They are separate links because receiving permission can be held without delivery-week read permission. Warehouse transfers is visible where Core authorizes it in Global or selected-location scope. The Global Products submenu retains Catalog overview for its controlled-unit reference. Web preserves every Core-authorized destination when regrouping so a second display list cannot silently hide a permitted workspace. Overview exception links open the existing location-scoped Operational exceptions page; Global readers select a location there to inspect its queue.
...
Owner-approved Shopify-style Admin direction, 2026-09-24: replace the earlier orange rail and universal master-detail presentation with the supplied [Spring ’26 reference](references/shopify-approved-reference.png) and the selected [Mobbin reference ledger](SHOPIFY_ADMIN_REFERENCES.md). FreshMarkets owns its implementation and identity. This visual approval does not add Shopify's draft orders, financial products, messaging, POS, supplier tools or other unsupported commands. Product preview controls approved in PRODUCT remain available in their authoritative Global or selected-location context.
...
- Present Shopify-like groups over Core-authorized destinations: Home, Orders, Products, Customers, Discounts, Content, Finance, Analytics, Sales channels and Apps, with Settings pinned. Parent destinations open only when independently authorized; a visible child such as Receiving must not depend on permission for its visual parent. Preserve all existing routes, deep links and scope/capability restrictions. POS and Messaging are visibly disabled non-links marked Coming soon. The active route has a subdued rounded background; icons are small and monochrome. The existing command palette uses the same authorized destinations.
...
| Settings | Existing redirect, configuration pages and detail/step routes | A visual group never grants access; child-only capability remains reachable, dirty editors guard scope/navigation, Scheduled cycles remain calendar-led. |
