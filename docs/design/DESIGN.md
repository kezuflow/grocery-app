...
Procurement appears in Operations for authorized Global and selected-location views; Receiving appears only for an authorized selected location. They are separate links because receiving permission can be held without delivery-week read permission. Warehouse transfers is visible where Core authorizes it in Global or selected-location scope. The Global Products submenu retains Catalog overview for its controlled-unit reference. Web preserves Core's navigation order so a second display list cannot silently hide a permitted workspace. Overview exception links open the existing location-scoped Operational exceptions page; Global readers select a location there to inspect its queue.
