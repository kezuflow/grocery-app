...
Owner correction, 2026-09-12: the rest of Product authoring follows the supplied companion references without restoring breadcrumbs or demo facts. Create Product provides a live customer-facing preview from the entered identity, chosen category, main draft image and first selling option; it clearly defers exact-location price and availability until those authoritative records can exist. The embedded creator stays single-column and independently scrollable, while the dedicated fallback may place that preview beside the editor. Edit Product uses a responsive details/images workspace, a visible unsaved-change state and persistent Cancel/Save actions; image cards retain replacement, removal, main-image, order, alt-text and recovery behavior. Global exact-location price inspection remains separate from Product identity: choosing Edit price opens the shared animated/resizable right pane with the selected location/option, current authoritative price, optional effective time and one retained idempotent Save price intent.
...
Owner correction, 2026-09-13: the complete checkout review workspace uses the white storefront background and flat sections separated by simple rules. Delivery details, saved checkout addresses, promotions, total review and the order summary must not use gray-filled card shells, rounded surface containers or card shadows. Form controls and meaningful warning, error and success states may retain their own accessible affordances.

Checkout keeps delivery details, promotions, delivery options and total review together in one review workspace. When the customer adds or corrects a checkout address, guide that address task through exactly three steps: (1) find an address or use current location and confirm the exact entrance pin, (2) complete address and recipient details, and (3) add delivery instructions and save. While active, this guide replaces the complete left checkout workspace rather than nesting another constrained form inside the delivery card; the cart and current total remain visible in the right summary. Step 1 uses one stable, full-width map surface at every selection state; choosing or moving a pin must not change its column count or dimensions. Place separate search and current-location icon buttons over the map. Search toggles an accessible, origin-anchored panel and returns focus to its trigger when dismissed; its results scroll over the map rather than reflowing it. Current location remains a direct independent action and must not require opening search. Saving returns to a compact confirmed-address summary and unlocks delivery choices. Existing serviceable saved addresses remain directly selectable and do not force the guide. In the Details step, offer valid account and saved-address phone numbers as deduplicated choices while retaining a clear “Use a different number” path. Accept ordinary Philippine mobile entry in local or international form, group it with spaces while typing, and normalize the saved command to canonical `+639…` format. Keep the order summary sticky on wide screens; each item shows media, product and fixed-pack identity, an editable quantity control and a right-aligned line total. A quantity change must release any active quote before mutating the cart, then require delivery and total review again.
...
Owner approval, 2026-09-11, as updated by the 2026-09-13 Global-area model: new address creation at /account/addresses uses three steps: Find address/map, Address and recipient details, Delivery instructions. Show progress and Back/Continue; preserve input between steps, validate the confirmed coordinate against active Global areas and nearest fulfillment-location assignment, then contact details, and only save on the final step. The account may retain an outside-area address with explicit unserviceable feedback, but it cannot be selected for checkout. Instructions are optional. Existing address editing retains its current layout.
