...
The checkout payment-method selector uses compact boxed choices in one wrapping row. Each choice displays only its brand mark, readable name and circular selection control; disabled styling and semantics communicate methods that are not active without status pills or descriptive subcopy. Available width determines how many boxes fit, and excess choices flow onto subsequent rows rather than creating horizontal scrolling or reverting to a full-width vertical list.
