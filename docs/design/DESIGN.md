...
The checkout payment-method selector uses full-width flat rows separated by simple rules. Each row displays only a circular selection control, bordered brand mark and readable method name, in that order; disabled styling and semantics communicate methods that are not active without status pills or descriptive subcopy. Do not add category tabs that conceal configured methods or imply additional provider availability.
