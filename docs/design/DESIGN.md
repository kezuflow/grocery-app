...
Order detail uses immutable number/date/items/financial/address/promise snapshots and a customer-safe timeline. The primary progress rail presents Payment, Order placement, Preparation and Delivery in customer-journey order, uses a recognizable semantic icon at every point, and fits its available width without exposing a horizontal scrollbar. Preparation `NOT_STARTED` and Delivery `UNASSIGNED` are initial records, not dated customer progress; show their milestones only after a meaningful state change. Titles name the recorded state, and timestamps and the latest safe update remain authoritative Core-supplied facts. Current Order options contain eligible cancellation and paid additions; after Scheduled packing starts, show Cancel order disabled with its reason. Order follow-up, Buy again and What went wrong appear after delivery completes. View transaction summary stays with Totals; omit the customer View invoice control and empty invoice panel. Preserve separate cancellation/refund progress and support actions. No provider payloads/internal notes/live-driver map; payment initiation or a browser return is not Order success. Customer emails communicate material recorded events, with scoped staff dashboard notices; no extra SMS/push channel or every-packing-step email.
