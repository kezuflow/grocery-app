...
The checkout payment-method selector uses compact boxed choices in one wrapping row. Each choice keeps its brand, readable name, availability and selection state together; available width determines how many boxes fit, and excess choices flow onto subsequent rows rather than creating horizontal scrolling or reverting to a full-width vertical list.
