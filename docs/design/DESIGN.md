...
- Command palette: Ctrl/Cmd+K opens a cmdk palette over the same capability-filtered navigation the shell renders; it only navigates and never widens access. The desktop search trigger with its shortcut hint occupies the flexible header center; mobile keeps the Sheet navigation search.
