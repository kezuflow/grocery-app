...
Warehouse Transfers appears in Global and authorized location scopes. Global creates a draft from named active warehouses, destinations and product pools, then dispatches it. The destination accepts checked quantities through an ordinary action; only acceptance credits physical stock. The list filters by status/scope and opens a detail with planned, accepted and outstanding transit quantities, reasons and immutable receipt history. Draft cancellation has no stock effects. Keep retries behind the original action and preserve its request after an unknown response. Discrepancy/loss/verified-return handling remains part of the required warehouse workflow and must keep non-sellable goods separate from accepted inventory.
