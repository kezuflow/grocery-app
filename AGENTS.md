...
- Customer cancellation is one coordinated Order operation. Instant locks at `FULFILLMENT_PENDING` and retains the snapshotted FreshMarkets Service Fee only for customer-caused cancellation; Scheduled locks at the earlier of its cutoff or `FULFILLMENT_PENDING`, refunds the original payment plus every committed paid addition, and never permits an addition to be canceled independently. FreshMarkets-caused cancellation refunds the full remaining applicable set. Post-lock exception refunds require global `refunds.manage`, a reason, and immutable audit evidence without reopening customer cancellation.
- Customer fulfillment mode is exactly `INSTANT` or `SCHEDULED`. One versioned global configuration selects exactly one active mode for all new commerce; `WEEKLY` is the initial Scheduled cadence, never a fulfillment mode. A later global switch invalidates uncommitted commerce but never rewrites a committed Order's fulfillment snapshot.
- Sourcing mode is not configurable domain state. Supply behavior is derived from the global fulfillment mode: `INSTANT` uses exact-location physical stock and holds/reservations; `SCHEDULED` uses cycles, cutoff, capacity, committed demand, and procurement planning.
...
- Authoritative price belongs to a sellable SKU at one exact fulfillment location. Market/global price fallback is forbidden; missing or invalid exact-location price is unavailable, never silently zero.
...
- Core authoritatively validates coordinates, serviceability polygons, delivery zone, the one global fulfillment mode, resolved location/promise, mode-specific inventory or cycle/cutoff/capacity, cart, exact-location SKU prices, promotions/stacking, minimum order, mode-specific membership entitlement, Service Fee evidence where applicable, and payment readiness. Overlapping eligible geofences resolve to the closest dispatch origin by exact Haversine distance with stable location-ID tie-break; route/driving-time providers never select the owning location.
